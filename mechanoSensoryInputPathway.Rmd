...
# WPN -> R1
WPNpathConnections = getConnectionTable_forSubset(unique(allPathFromWPN_to_R1$from),unique(allPathFromWPN_to_R1$to))
typesTable <- getTypesTable(unique(WPNpathConnections$databaseType.to))

if (splitLR){
  # Subdevide types and build a custom types table
  WPNpathConnections = lrSplit(WPNpathConnections, nameCol="name.to",typeCol="type.to")
  WPNpathConnections = lrSplit(WPNpathConnections, nameCol="name.from",typeCol="type.from")
  typesTable = lrSplit(typesTable, nameCol="name",typeCol="type")
}
...
```{r}
myPathT2T = full_join(myAPNPathT2T, myWLLPathT2T,myWPNPathT2T)
```
...
if(splitLR){
  dev.print(pdf, paste0("../neuprintR_analysis_plots/mechanoPW/connectivityGraph_APN_or_WPN_or_WLL_to_R1b_LR.pdf"), width=10, height=10)
}else{
  dev.print(pdf, paste0("../neuprintR_analysis_plots/mechanoPW/connectivityGraph_APN_or_WPN_or_WLL_to_R1b.pdf"), width=10, height=10)
}
```
...
```{r}
#APN -> R3
APNpathConnections = getConnectionTable_forSubset(unique(allPathFromAPN_to_R3$from),unique(allPathFromAPN_to_R3$to))
typesTable <- getTypesTable(unique(APNpathConnections$databaseType.to))
...
# WLL -> R3
WLLpathConnections = getConnectionTable_forSubset(unique(allPathFromWLL_to_R3$from),unique(allPathFromWLL_to_R3$to))
typesTable <- getTypesTable(unique(WLLpathConnections$databaseType.to))
...
# WPN -> R3
WPNpathConnections = getConnectionTable_forSubset(unique(allPathFromWPN_to_R3$from),unique(allPathFromWPN_to_R3$to))
typesTable <- getTypesTable(unique(WPNpathConnections$databaseType.to))
...
if (splitLR){
  ...
  path2R3_5 = all_simple_paths(myGraph, V(myGraph)[c("PDL27e_pct_R","PDL27e_pct_L")], to = V(myGraph)[c("R3a_b_R","R3a_b_L")], mode = "out")
}else{
  path2R3_1 = all_simple_paths(myGraph, V(myGraph)[c("PDM14m_pct")], to = V(myGraph)[c("R3a_b")], mode = "out")
...
onpath = V(myGraph)[unique(c(unlist(path2R3_1),unlist(path2R3_2),unlist(path2R3_3),unlist(path2R3_4),unlist(path2R3_5)))]
...
if(splitLR){
  dev.print(pdf, paste0("../neuprintR_analysis_plots/mechanoPW/connectivityGraph_APN_or_WPN_or_WLL_to_R3a_b_LR.pdf"), width=10, height=10)
}else{
  dev.print(pdf, paste0("../neuprintR_analysis_plots/mechanoPW/connectivityGraph_APN_or_WPN_or_WLL_to_R3a_b.pdf"), width=10, height=10)
}
```
...
if(splitLR){
  dev.print(pdf, paste0("../neuprintR_analysis_plots/mechanoPW/connectivityGraph_APN_to_WLL_to_R1_or_R3_LR.pdf"), width=10, height=10)
}else{
  dev.print(pdf, paste0("../neuprintR_analysis_plots/mechanoPW/connectivityGraph_APN_to_WLL_to_R1_or_R3.pdf"), width=10, height=10)
}
```
