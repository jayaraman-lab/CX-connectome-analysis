...
saveName = paste0("ER1_a_input_", myROI)
my_types = c("ER1_a") #This should be the post-synaptic partners
...
if(myROI %in% c("AOTU(R)", "LAL(R)")){
  meshOutline <- ahull(x=roiMeshPtsPlane$x,y=roiMeshPtsPlane$y,alpha=140)
}else{
...
if(myROI %in% c("AOTU(R)", "LAL(R)")){
  meshOutline <- ahull(x=roiMeshPtsPlane$z,y=roiMeshPtsPlane$y,alpha=140)
}else{
...
if(myROI %in% c("AOTU(R)", "LAL(R)")){
  meshOutline <- ahull(x=roiMeshPtsPlane$x,y=roiMeshPtsPlane$z,alpha=140)
}else{
...
```{r}
minSyn = 10
inputTypes = my_synapses %>% filter(prepost == 1 ) %>% group_by(partnertype) %>% count() %>% filter(n>=10)
...
for (partner in c(arrange(inputTypes, desc(n))$partnertype[-1])){
  ...
  mycols = paletteer_c("pals::ocean.phase", n=1+length(unique(data$id)))[-1]
  
  xyproj <- ggplot(toplot_2D) + geom_point(aes(x=x, y=y, color=id, size=1, alpha=0.8)) + 
     geom_point(data=toplot_2D %>% group_by(id) %>% 
                summarize_each(mean), aes(x=x, y=y, color=id, size=2)) +
  ...
  zyproj <- ggplot(toplot_2D) + geom_point(aes(x=z, y=y, color=id, size=1, alpha=0.8)) + 
    geom_point(data=toplot_2D %>% group_by(id) %>% 
                 summarize_each(mean), aes(x=z,y=y, color=id, size=2)) +
    #scale_color_manual(values = colPal, breaks = unique(toplot_2D$id)) + 
  ...
  xzproj <- ggplot(toplot_2D) + geom_point(aes(x=x, y=z, color=id, size=1, alpha=0.8)) + 
    geom_point(data=toplot_2D %>% group_by(id) %>% 
                 summarize_each(mean), aes(x=x,y=z, color=id, size=2)) +
    #scale_color_manual(values = colPal, breaks = unique(toplot_2D$id)) + 
    geom_path(data=outline_xz, aes(x=c1, y=c2), size = 0.5) + xlim(xrange)  + ylim(zrange) +
    coord_fixed() +theme_void() + guides(size=FALSE, alpha=FALSE) + lims(x=xrange,  y=zrange)
...
for (partner in c(arrange(inputTypes, desc(n))$partnertype[-1])){
  ...
  xyproj <- ggplot(toplot_2D) + geom_point(aes(x=x, y=y, color=partnerid, size=1, alpha=0.8)) + 
     geom_point(data=toplot_2D %>% group_by(partnerid) %>% 
                summarize_each(mean), aes(x=x, y=y, color=partnerid, size=2)) +
     #scale_color_manual(values = colPal, breaks = unique(toplot_2D$id)) + 
     geom_path(data=outline_xy, aes(x=c1, y=c2), size = 0.5) + xlim(xrange)  + ylim(yrange) +
     coord_fixed() + theme_void() + guides(size=FALSE, color=FALSE, alpha=FALSE) + lims(x=xrange,  y=yrange)
  ...
  zyproj <- ggplot(toplot_2D) + geom_point(aes(x=z, y=y, color=partnerid, size=1, alpha=0.8)) + 
    geom_point(data=toplot_2D %>% group_by(partnerid) %>% 
                 summarize_each(mean), aes(x=z,y=y, color=partnerid, size=2)) +
    #scale_color_manual(values = colPal, breaks = unique(toplot_2D$id)) + 
    geom_path(data=outline_zy, aes(x=c1, y=c2), size = 0.5) + xlim(zrange)  + ylim(yrange) +
    coord_fixed() +theme_void() + guides(size=FALSE, color=FALSE, alpha=FALSE) + lims(x=zrange,  y=yrange)
  ...
  xzproj <- ggplot(toplot_2D) + geom_point(aes(x=x, y=z, color=partnerid, size=1, alpha=0.8)) + 
    geom_point(data=toplot_2D %>% group_by(partnerid) %>% 
                 summarize_each(mean), aes(x=x,y=z, color=partnerid, size=2)) +
    #scale_color_manual(values = colPal, breaks = unique(toplot_2D$id)) + 
    geom_path(data=outline_xz, aes(x=c1, y=c2), size = 0.5) + xlim(xrange)  + ylim(zrange) +
    coord_fixed() +theme_void() + guides(size=FALSE,  alpha=FALSE) + lims(x=xrange,  y=zrange)
