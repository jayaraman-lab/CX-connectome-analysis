...
```
```{r}
labelRois <- selectRoiSet(roiH,default_level = 1,exceptions = list("CX"=2))
```
...
```{r}
haneschEB_R <- haneschPlot(EBSummary %>% filter(type %in% EBTypes$type & (!grepl("_L.*",type))),grouping="supertype2",flip=T,roiSelect = EBRois,roiLabel = labelRois)
```

```{r}
haneschEB_R <- haneschEB_R + scale_color_brewer(type = "div",palette = "Paired",name="Brain region")
```
...
labelRoisNO <- selectRoiSet(roiH,default_level = 1,exceptions = list("NO"=2,"AB(R)"=2),exceptionLevelMatch = 2)
```
...
```{r}
haneschNOR <- haneschPlot(NOSummary %>% filter(type %in% NORTypes$type & (!grepl("_L.*",type))),grouping="supertype2",flip=T,roiSelect = NORois,roiLabel = labelRoisNO,theme=theme_bw(),alphaRois = 0.15)
```
