...
```{r}
NORois <- selectRoiSet(roiH,default_level = 2,exceptions = list("NO(R)"=4,"LAL(R)"=4))
```
...
haneschNOR <- haneschPlot(NOSummary %>% filter(type %in% NORTypes$type & (!grepl("_L.*",type))),grouping="supertype2",flip=T,roiSelect = NORois,roiLabel = labelRois)
```
