...
```{r}
haneschEB_R <- haneschPlot(EBSummary %>% filter(type %in% EBTypes$type & (!grepl("_L.*",type))),grouping="supertype2",flip=T,roiSelect = EBRois,roiLabel = labelRoisEB)
haneschEB_R
```
...
```{r}
NOTypesTable <- lateralizeInputOutputList(NOTypesTable)
```
...
labelRoisNO <- selectRoiSet(roiH,default_level = 0,exceptions = list("NO"=2,"AB(R)"=2),exceptionLevelMatch = 2)
```
...
```{r}
haneschNOR <- haneschPlot(NOSummary %>% filter(type %in% NORTypes$type),grouping="supertype2",flip=T,roiSelect = NORois,roiLabel = labelRoisNO,theme=theme_bw())
```
...
labelRoisPB <- selectRoiSet(roiH,default_level = 0,exceptions = list("PB"=2,"AB(R)"=2),exceptionLevelMatch = 2)
```
...
```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/PB/RoiInOut/HaneschPBTypes.svg",haneschPB,width = 16,height=8)
```
...
labelRoisFB <- selectRoiSet(roiH,default_level = 0,exceptions = list("FB"=2,"AB(R)"=2),exceptionLevelMatch = 2)
```
...
```{r}
haneschFB <- haneschPlot(FBSummary %>% filter((type %in% FBTypes$type)),grouping = "supertype2",roiSelect=FBRois,flip = T,roiLabel = labelRoisFB)
haneschFB
```
...
```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/FB/RoiInOut/HaneschFBTypes.svg",haneschFB,width = 45,height=8)
```
