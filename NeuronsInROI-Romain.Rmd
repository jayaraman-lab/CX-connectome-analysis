...
```{r}
haneschEB_R <- haneschPlot(EBSummary %>% filter(type %in% EBTypes$type & ((!grepl("_L.*",type) & supertype3 != "EB Columnar")| grepl("_L",type) & supertype3 == "EB Columnar")),grouping="supertype2",flip=T,roiSelect = EBRois,roiLabel = labelRoisEB,interactive=TRUE)
haneschEB_R
```
...
```{r}
interHaneschEB_R <- girafe(ggobj = haneschEB_R, width_svg=14,height_svg=6,
  options = list(
...
```{r}
htmlwidgets::saveWidget(interHaneschEB_R,"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/EB/RoiInOut/HaneschEBTypes_Right.html")
...
```{r}
haneschNOR <- haneschPlot(NOSummary %>% filter(type %in% NORTypes$type),grouping="supertype2",flip=T,roiSelect = NORois,roiLabel = labelRoisNO,interactive=TRUE)
```
...
```{r}
htmlwidgets::saveWidget(interHaneschNOR,"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/NO/RoiInOut/HaneschNORTypes.html")
...
```{r}
haneschPB <- haneschPlot(PBSummary %>% filter(type %in% PBTypes$type),roiSelect = PBRois,flip = T,grouping = "supertype2",roiLabel = labelRoisPB,interactive=TRUE)
haneschPB
```
```{r}
...
```{r}
htmlwidgets::saveWidget(interHaneschPB,"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/PB/RoiInOut/HaneschPBTypes.html")
...
```{r}
haneschFBtR <- haneschPlot(FBSummary %>% filter((type %in% FBTypes$type) & supertype2 == "FBt" & !grepl("_L",type)),roiSelect=FBRois,flip = T,roiLabel = labelRoisFB,grouping="supertype1",interactive = TRUE)
haneschFBtR
```
...
```{r}
htmlwidgets::saveWidget(interHaneschFBtR,"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/FB/RoiInOut/HaneschTangentialRight.html")
...
```{r}
haneschFBOutLay <- haneschPlot(FBSummary %>% filter((type %in% FBTypes$type) & (supertype3 == "FB Output") & !grepl("_R",type)),roiSelect = FBRois,flip=T,roiLabel = labelRoisFB,grouping="supertype2",interactive=TRUE)
haneschFBOutLay
```
...
```{r}
htmlwidgets::saveWidget(interHaneschFBQL,"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/FB/RoiInOut/HaneschFB-QsLeft.html")
...
```{r}
haneschFBColL <- haneschPlot(FBSummary %>% filter((type %in% FBTypes$type) & (supertype3 == "FB Columnar") & !grepl("_R",type)),roiSelect = FBRois,flip=T,roiLabel = labelRoisFB,grouping="supertype2",interactive=TRUE)
haneschFBColL
```
```{r}
...
```{r}
htmlwidgets::saveWidget(interHaneschFBColL,"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/FB/RoiInOut/HaneschFBColumnarLeft.html")
...
```{r}
haneschDeltas <- haneschPlot(FBSummary %>% filter((type %in% FBTypes$type) & (supertype3 == "FB Interneuron") & !grepl("_R",type)),roiSelect = FBRois,flip=T,roiLabel = labelRoisFB,grouping="supertype2",interactive=TRUE)
haneschDeltas
```
```{r}
...
```{r}
htmlwidgets::saveWidget(interHaneschDelta,"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/FB/RoiInOut/HaneschFBDeltas.html")
htmlwidgets::saveWidget(interHaneschDelta,paste0(haneschSaveFolder,"FB/FBDeltas.html"))
```

```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/FB/RoiInOut/HaneschFBDeltas.svg",haneschDeltas,width = 10,height=7)
ggsave(paste0(haneschSaveFolder,"FB/FBDeltas.svg"),haneschDeltas,width = 10,height=7)
```
...
```
```{r}
fbSummary <- plot_grid(haneschFBColL,haneschDeltas,haneschFBOutLay,nrow = 1)
```

```{r}
ggsave(paste0(haneschSaveFolder,"FB/FBSummary.svg"),fbSummary,width = 30,height=7)
```
