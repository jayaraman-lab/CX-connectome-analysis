...
my_neurons = getBodyIdsForList(c("EL","EPG","EPGt","PEG","PEN_a","PEN_b"))
...
```{r}
rot = c(0,180,0) #BU (45,180, 0) #EB: (0,0,180)
...
```{r}
synprojFront = ggplot() + 
  stat_density_2d(data=synPtsPlane, aes(x=x, y=y, fill=type, alpha = stat(nlevel)), bins=3, geom = "polygon") +
...
```{r}
synprojFront = ggplot() + 
  stat_density_2d(data=synPtsPlane, aes(x=x, y=y, color=simpleType), 
                  bins=2, geom ="contour", show.legend=TRUE) + 
  #stat_density_2d(data=synPtsPlane, aes(x=x, y=y, fill=simpleType, alpha = stat(nlevel)), bins=3, geom = "polygon") +
