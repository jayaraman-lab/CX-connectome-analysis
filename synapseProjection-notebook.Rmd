...
```{r}
rot = c(45,180, 0)#BU (45,180, 0) #EB: (0,0,180)
...
```{r}
synprojFront = ggplot() + 
  stat_density_2d(data=synPtsPlane %>% filter(type %in% typeFilter), aes(x=x, y=y, fill=type, alpha = stat(nlevel)), bins=3, geom = "polygon") +
...
```{r}
synprojFront = ggplot() + 
  #stat_density_2d(data=synPtsPlane %>% filter(type %in% typeFilter), aes(x=x, y=y, color=simpleType), bins=2, geom ="contour", show.legend=TRUE) + 
  stat_density_2d(data=synPtsPlane %>% filter(type %in% typeFilter), 
                  aes(x=x, y=y, fill=simpleType, alpha = stat(nlevel)), bins=4, geom = "polygon") +
...
```{r}
synprojSide = ggplot() + 
  stat_density_2d(data=synPtsPlane %>% filter(type %in% typeFilter), aes(x=z, y=y, fill=type, alpha = stat(nlevel)), bins=5, geom = "polygon") +
