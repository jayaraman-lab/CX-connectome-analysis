...
saveName = paste0("contra_in_", myROI)
my_types = c("ExR","AOTU")
my_types = paste0(my_types, ".*")
cmapName = "colorMap_ExR_and_AOTU46"
...
```{r}
#Filter out synapses that are to TuBu
#my_synapses = my_synapses %>% mutate(partnerType =neuprint_get_meta(as.numeric(partner))$type ) %>% 
#  filter(partnerType %in% c("TuBu10",paste0("TuBu0",seq(9))))
```
...
```{r}
rot = c(45,0, 0) #BU (45,180, 0) EB: (0,0,180) AOTU: (0,180,0)
...
```{r}
synprojSide = ggplot() + 
  stat_density_2d(data=dataFilt, aes(x=x, y=z, fill=type, alpha = stat(nlevel)), bins=nlevels, geom = "polygon") +
...
ggsave( paste("synapseDistributions_",saveName,'_top_level.pdf', sep=''), plot = synprojSide, device='pdf', path = "../neuprintR_analysis_plots/",
    scale = 1.5, width = plotW*0.8, height = plotH*1.2, units ="cm", dpi = 600, limitsize = TRUE)
