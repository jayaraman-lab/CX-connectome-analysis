...
```{r}
myROI = "BU(R)"
saveName = paste0("ringNeurons_in_", myROI)
my_neurons = getBodyIdsForList(c("R1.*","R2.*","R3.*","R4.*","R5.*","R6.*"))
```
...use this list to get synapse locations
```{r}
my_synapses = neuprint_get_synapses(as.numeric(my_neurons$bodyid),myROI) %>%
                          mutate(partnerName=neuprint_get_meta(as.numeric(partner), all_segments = TRUE)[["name"]],
...
```{r}
rot = c(45,180,0) #z,y,x # BU (45, 0, 180) #EB: (0,0,180) and eigen vectors
...
nlevel = 4
synprojFront = ggplot() + 
  stat_density_2d(data=synPtsPlane, aes(x=x, y=y, fill=type, alpha = stat(nlevel)), bins=6, geom = "polygon") +
  facet_wrap(~simpleType, ncol=5) +
  coord_fixed(ratio = 1) + theme_void() + guides(fill=FALSE) +
  geom_path(data=outline, aes(x=c1, y=c2), size = 0.5)

print(synprojFront)
ggsave( paste("synapseDistributions_",saveName,'_front_simpletype.pdf', sep=''), plot = synprojFront, device='pdf', path = "../neuprintR_analysis_plots/",
    scale = 1.5, width = plotW, height = plotH, units ="cm", dpi = 600, limitsize = TRUE)
```
...
nlevel = 4
synprojFront = ggplot() + 
  stat_density_2d(data=synPtsPlane, aes(x=x, y=y, fill=simpleType, alpha = stat(nlevel)), bins=5, geom = "polygon") +
...
```{r}
synprojSide = ggplot() + 
  geom_hex(data=synPtsPlane, aes(x=z, y=y), bins=nbins) +   #fill=type, color=type
  scale_fill_gradientn(colours = brewer.pal(5,"BuPu"), breaks=c(0,Cmax), limits=c(0, Cmax), oob = scales::squish) +
...
```{r}
synprojSide = ggplot() + 
  stat_density_2d(data=synPtsPlane, aes(x=z, y=y, fill=type, alpha = stat(nlevel)), bins=5, geom = "polygon") +
...
```{r}
# get outline
meshOutline <- ahull(x=roiMeshPtsPlane$dist,y=roiMeshPtsPlane$z,alpha=70)
outline = data.frame(meshOutline$arcs)
#outline = bind_rows(outline, outline[1,]) # close the shape
```
...
synprojSlice = ggplot() + 
  ...
  geom_hex(data=synPtsPlane, aes(x=syn_dist, y=z,fill=type, alpha=..density..), bins=50) +
...
Cmax = 200
synprojSlice = ggplot() + 
  geom_hex(data=synPtsPlane, aes(x=syn_dist, y=z), bins=30) +
  scale_fill_gradientn(colours = brewer.pal(5,"BuPu"), breaks=c(0,Cmax), limits=c(0, Cmax), oob = scales::squish) +
  facet_wrap(~type, ncol=5) +
  coord_fixed(ratio = 1) + theme_void() + guides(fill=FALSE) +
  geom_path(data=outline, aes(x=c1, y=c2), size = 0.5)
...
nlevel = 3
synprojSlice = ggplot() + 
  stat_density_2d(data=synPtsPlane, aes(x=syn_dist, y=z, fill=type, alpha = stat(nlevel)), bins=4, geom = "polygon") + scale_alpha(range = c(0.2, 1)) +
  facet_wrap(~type, ncol=5) +
  coord_fixed(ratio = 1) + theme_void() + guides(fill=FALSE) +
  geom_path(data=outline, aes(x=c1, y=c2), size = 0.5)
print(synprojSlice)
