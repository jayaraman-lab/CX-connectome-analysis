...
if (useCuratedList) {
  ...
  postName = "ExR, PEN and EPG"#paste0(ROI, " neurons")
  
  postNeuron = c('ExR',  'PEN', 'EPG')
...
if (useCuratedList) {
  ...
  preNeuron = c('ExR',  'PEN', 'EPG')
  preIDs = getBodyIdsForList(preNeuron, field="type")
  preName = paste0(ROI, "neurons")
  saveName = paste0(ROI,"neurons")
  
} else{
  preName = "ExR, PEN and EPG"#paste0(ROI,"Neuron Partners")
  saveName = "ExR_PEN_EPG"#paste0(ROI,"Inputs")
  ...
  preNeuron = unique( c(unique(partnersInRoi$from.type) , unique(partnersInRoi$type)) )

  parnerPlot = ggplot(myConnections %>% filter(from.type %in% preNeuron), aes(x=from.type, fill=from.type)) + geom_bar() + theme_classic() + 
...
for (connectionMeasure in connectionMeasures){
  ...
  print(conMatPlot)
  ggsave( paste("connectivityMatrix_",saveName,'_in_',slctROI,'_',connectionMeasure,'_byName.pdf', sep=''), plot = conMatPlot, device='pdf', path = "../neuprintR_analysis_plots/",
...
cutoff = 0 # for weight 0.05, 0.01, 0
