...
saveName = paste0("RN_to_EPG-EP_",slctROI) #RN_TB_ExR #RN_ExR_col_in_
...
if (useCuratedListPost) {
  ...
  if(useCustomPostSelection){
    ...
    postName = paste0("EPG, EL in ",slctROI) #RN, columnar and ExR
...
if (useCuratedListPre) {
  ...
  if(useCustomPreSelection){
    preNeuron = c("R1","R2","R3","R4","R5","R6") # postNeuron #c("TuBu.*")#
    preName = paste0("RN in ",slctROI) # postName#
...
dev.print(pdf, paste0("../neuprintR_analysis_plots/connectivityGraph_",graphName,'_in_',slctROI,
                      "_",connectionMeasure, round(100*cutoff),"perc_inOutRatio.pdf"), width=20, height=12)


# V2: 
for (i in seq(1, length(V(myGraph)$name))) {
  ratio = weightRatio %>% filter(type == V(myGraph)$name[i]) %>% select(ratio)
  vertexSize[i] = ratio$ratio * vertexBaseSize + 5
}
...
dev.print(pdf, paste0("../neuprintR_analysis_plots/connectivityGraph_",graphName,'_in_',slctROI,
                      "_",connectionMeasure, round(100*cutoff),"perc_byEPG.pdf"), width=20, height=12)
```
...
roiInfo = lrSplit(roiInfo, nameCol="name",typeCol="type")
...
inlayer = c("GLN","LN1", "LN2", "LN3", "LNa","LCNpm", "LCNp")
