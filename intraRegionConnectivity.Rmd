...
saveName = paste0("col2ExR_", ROI)#paste0("all_in_", ROI) #RN_TB_ExR #RN_ExR_col_in_
...
useCustomPostSelection = T
useCustomPreSelection = T
usenbag = FALSE
inCX = FALSE
...
if (useCuratedListPost) {
  ...
  if(useCustomPostSelection){
    postNeuron = c("EL","EPG","EPGt","PEG","PEN_a","PEN_b")
      #
    postName = "col"
...
maxEdge = max(E(myGraph)$width)
minEdge = min(E(myGraph)$width)
legend(x=-0.8,y=1, legend = edgeNorm*seq(round(minEdge, digits = 0),round(maxEdge, digits = 0),length.out=5), 
       lwd	 = seq(round(minEdge, digits = 0),round(maxEdge, digits = 0),length.out=5),  bty = "n", seg.len=0.5)

dev.print(pdf, paste0("../neuprintR_analysis_plots/connectivityGraph_",graphName,'_in_',slctROI,
                      "_",connectionMeasure, round(100*cutoff),"perc_layered.pdf"), width=8, height=8, useDingbats=FALSE)
```
