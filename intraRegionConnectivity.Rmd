...
if (useCuratedList) {
  ...
  synCutoff = 3
  relWeightCutoff = .03 # percent input or output in that region
  ...
  partnersInRoi =  myConnections %>% 
    ...
    group_by(type) %>% 
    filter( (weightRelative >= relWeightCutoff))
...
graphData = reorganizeGraphData(connectData$from,
                                connectData$to,
                                connectData$type.from,
                                connectData$type.to,
                                connectData$weight,
...
dev.print(pdf, paste0("../neuprintR_analysis_plots/connectivityGraph_",saveName,'_in_',roiName,
