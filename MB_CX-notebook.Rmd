...
StrongMBON_CX_TypeToTypeConnections <- MBON_CX_TypeToTypeConnections  %>% filter(weightRelative > 0.01)
...
# Get the connection table from StrongMBON2CX_Target to downstream subset in the FB
ROI <- "FB"
conTab <- getConnectionTable_forSubset(StrongMBON2CX_TargetIds$bodyid,
                                       StrongMBON2CX_TargetsCnctTable$to,
                                       ROI)

conTab_Clean <- getTypeToTypeTable(conTab)
xyLookup = rbind(xyLookupTableInner(),xyLookupTableCol())

# Graph the TypeToType ConnTable using the combined lookupTable
gg_MBONtargetTab <- graphConTab(conTab_Clean,xyLookup,FALSE,TRUE)
gg_MBONtargetTab <- gg_MBONtargetTab + xlim(-0.75,0.75) + ylim(0.5,3.5) + scale_y_reverse()
print(gg_MBONtargetTab)
ggsave("gg_MBONtargetTab.pdf", plot=gg_MBONtargetTab, device="pdf", path=PlotDir, scale=1, 
       width=11, height=8, units="in", dpi=300, limitsize=TRUE)
