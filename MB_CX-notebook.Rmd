...
### Find all CX neurons with input/output in the MB
```{r}
# Get roiInfo for all CX neurons
allCXneuronsROIinfo <- neuprint_get_roiInfo(allCXneurons$bodyid)
allCXneuronsPostInMBinfo <- allCXneuronsROIinfo  %>% filter(MB*.post > 5)
allCXneuronsPostInMB <- allCXneurons %>% filter(bodyid %in% allCXneuronsPostInMBinfo$bodyid)

# allCXneuronsPreInMBinfo <- allCXneuronsROIinfo  %>% filter("MB(L).pre" + "MB(R).pre" > 5)
# allCXneuronsPreInMB <- allCXneurons %>% filter(bodyid %in% allCXneuronsPreInMBinfo$bodyid)
