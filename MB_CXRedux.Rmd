...
StrongMBON2nonCXwCXRTgtInpCombSort <- StrongMBON2nonCXwCXRTgtInpCombSort_CosDist[[3]]
StrongMBONnonCXTargetR2CXRtab <- StrongMBONnonCXTargetR2CXR_CosDist[[3]]

# Make a combined table
# StrongMBON2nonCXwCXRTgtInpCombSort$type.from <- as.vector(StrongMBON2nonCXwCXRTgtInpCombSort$type.from)
# StrongMBON2nonCXwCXRTgtInpCombSort$type.to <- as.vector(StrongMBON2nonCXwCXRTgtInpCombSort$type.to)
# StrongMBONnonCXTargetR2CXRtab$type.from <- as.vector(StrongMBONnonCXTargetR2CXRtab$type.from)
# StrongMBONnonCXTargetR2CXRtab$type.to <- as.vector(StrongMBONnonCXTargetR2CXRtab$type.to)
...
types <- unique(c(unique(as.vector(StrongMBON2nonCXwCXRTgtTable$type.from)), unique(as.vector(StrongMBON2nonCXwCXRTgtTable$type.to)),
...
numMidNodes <- length(unique(as.vector(StrongMBON2nonCXwCXRTgtTable$type.to)))
...
ggsave("StrongMBON2nonCXR2CXRTgtComboPath.svg", plot=StrongMBON2nonCXR2CXRTgtComboPath, device="svg", path=PlotDir, scale=1, 
       width=12, height=15, units="in", dpi=300, limitsize=FALSE)
...
```
```{r}
# Add a cluster.toFrom column in StrongMBONnonCXTargetR2CXRtab for clustering based on inputs to the type.from types
StrongMBON2nonCXwCXRTgtInpCombSort_CosDistClusterByOut <- StrongMBON2nonCXwCXRTgtInpCombSort_CosDist[[2]]
StrongMBON2nonCXwCXRTgtInpCombSort_CosDistClusterByOutVec <- StrongMBON2nonCXwCXRTgtInpCombSort_CosDistClusterByOut[[4]]
StrongMBONnonCXTargetR2CXRtab <- mutate(StrongMBONnonCXTargetR2CXRtab, cluster.toFrom = StrongMBON2nonCXwCXRTgtInpCombSort_CosDistClusterByOutVec[as.vector(StrongMBONnonCXTargetR2CXRtab$type.from)])

# Re-plot the connectivity matrix based on cluster.toFrom for rows and cluster.to for columns
plotStrongMBONnonCXTargetR2CXRtab <- plotConnectivityMatrix(StrongMBONnonCXTargetR2CXRtab,byGroup="type",connectionMeasure="weightRelative")

plotStrongMBONnonCXTargetR2CXRtab <- plotStrongMBONnonCXTargetR2CXRtab + facet_grid(rows=vars(cluster.toFrom),cols=vars(cluster.to),scale="free",space="free") + 
...
print(plotStrongMBONnonCXTargetR2CXRtab)
ggsave("StrongMBONnonCXTargetR2CXRtab_cosDistClustConnMat_rowsByMBONto.eps", plot=plotStrongMBONnonCXTargetR2CXRtab, device="eps", path=PlotDir, scale=1, 
         width=24, height=16, units="in", dpi=300, limitsize=FALSE)

```

### Plot an example of indirect connections
```{r}
midTypes <- c("CRE042_R","CRE054_R","LHPV4m1_R","LHPV5e3_R","SIP029_R","SIP087_R");
StrongMBON2nonCXwCXRTgtTableFilt <- StrongMBON2nonCXwCXRTgtTable %>% filter(type.to %in% midTypes)
StrongMBONnonCXTgtR2CXRTableFilt <- StrongMBONnonCXTgtR2CXRTable %>% filter(type.from %in% midTypes)
...
numMidNodes <- length(unique(as.vector(StrongMBON2nonCXwCXRTgtTableFilt$type.to)))
numCXtargets <- length(unique(as.vector(StrongMBONnonCXTgtR2CXRTableFilt$type.to)))
xyLookup = data.frame(type = types, x = c(rep(-1,times = numMBONs), rep(0,times = numMidNodes), rep(1,times = numCXtargets)), 
                      y = c(seq(-0.8,0.8,length.out = numMBONs), seq(-1,1,length.out = numMidNodes), seq(-1,1,length.out = numCXtargets)))
...
StrongMBONTargetInpC <- StrongMBON2CXTargetR_Type2Type_CosDist[[3]]
...
### Pull out connections based on PN->MBON input modality percentages
```{r}
# Get the PN2MBON input modality table
PN2allMBON_InputPct <- getPN2MBONinputTable()

# Filter and analyze the MBON to CX direct and indirect connections based on input modality
visualMBON2CX <- filterConnTabsByInputMod(PlotDir,PN2allMBON_InputPct,"Visual",25,"MBON",StrongMBONTargetInpC,StrongMBON2nonCXwCXRTgtInpCombSort,StrongMBONnonCXTargetR2CXRtab)
thermoHygroMBON2CX <- filterConnTabsByInputMod(PlotDir,PN2allMBON_InputPct,"ThermoHygro",20,"MBON",StrongMBONTargetInpC,StrongMBON2nonCXwCXRTgtInpCombSort,StrongMBONnonCXTargetR2CXRtab)
# olfMBON2CX <- filterConnTabsByInputMod(PlotDir,PN2allMBON_InputPct,"Olfactory",75,"MBON",StrongMBONTargetInpC,StrongMBON2nonCXwCXRTgtInpCombSort,StrongMBONnonCXTargetR2CXRtab)
