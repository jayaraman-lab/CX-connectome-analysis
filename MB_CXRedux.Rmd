...
StrongMBON2CX_2ndTargetsCnctTable <- getConnectionTable(StrongMBON2CX_TargetsCnctTable$to,synapseType="POST")
# head(StrongMBON2CX_2ndTargetsCnctTable)
StrongMBON2CX_2ndTargetsToPostInFB <- getConnectionTable_forSubset(StrongMBON2CX_TargetsCnctTable$to, StrongMBON2CX_2ndTargetsCnctTable$to,"FB")
...
### Cluster MB->CX secondary targets by the correlation of their weightRelative connectivity to tertiary targets
```{r}
library(reshape2)
...
# Order the secondary targets according to their clustering
StrongMBON2CX_2ndTargetsToPostInFB_Type2TypeTab$type.from <- factor(StrongMBON2CX_2ndTargetsToPostInFB_Type2TypeTab$type.from, levels = hc$labels[hc$order])
# Pull out the reordered matrix
Data4Corr <- dcast(StrongMBON2CX_2ndTargetsToPostInFB_Type2TypeTab %>% select(type.from,type.to,weightRelative),type.from~type.to)
...
library(corrplot)
postscript(file = paste(PlotDir,"corrplotStrongMBON2CX_2ndTargetsToPostInFB_Type2Type.eps",sep="/"),width=24,height=24)
corrplot(cor(Data4Corr), order="hclust", hclust.method="ward.D2", tl.cex=0.5, tl.col="black")
dev.off()
