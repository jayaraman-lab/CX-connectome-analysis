...
```{r}
numLookup <- colNronOutsByID$id.from %>% unique() %>% lapply(function(x) strsplit(as.character(x),'-')[[1]][1]) %>% unlist() %>% table() %>% as.data.frame()
colnames(numLookup) <- c('name','freq')
```
```{r, fig.width=10, fig.height = 10}
colNronOutsByID$freq <- colNronOutsByID$id.from %>% lapply(function(x) numLookup[numLookup$name == strsplit(as.character(x),'-')[[1]][1],]$freq) %>% unlist()
...
FBNO_OutPlt <- ggplot(colNronOutsByID_meanNorm %>% filter(roi %in% c('FB','NO'))) + 
  ...
  facet_grid(rows = vars(roi)) +
  theme_paper() +
  coord_cartesian(xlim = c(1,12), ylim = c(0,2), expand = FALSE, clip ='off') +
  xlab('numerosity factor') + 
  ylab('normalized total synapse count') +
  ggtitle('outputs') +
  theme(plot.margin = margin(0.25,0.25,0.25,0.25,'cm'),
...
Find the number of neurons per glomerulus
```{r}
numLookup <- colNronInsByID$id.to %>% unique() %>% lapply(function(x) strsplit(as.character(x),'-')[[1]][1]) %>% unlist() %>% table() %>% as.data.frame()
colnames(numLookup) <- c('name','freq')
```
```{r, fig.width=10, fig.height = 10}
colNronInsByID$freq <- colNronInsByID$id.to %>% lapply(function(x) numLookup[numLookup$name == strsplit(as.character(x),'-')[[1]][1],]$freq) %>% unlist()
...
PBFBNO_InPlt <- ggplot(colNronInsByID_meanNorm %>% filter(roi %in% c('PB','FB','NO'))) + 
  ...
  facet_grid(rows = vars(roi)) +
  theme_paper() +
  coord_cartesian(xlim = c(1,12), ylim = c(0,4), expand = FALSE, clip ='off') +
  xlab('numerosity factor') + 
  ylab('normalized total synapse count') +
  ggtitle('inputs') +
  theme(plot.margin = margin(0.25,0.25,0.25,0.25,'cm'),
...
EPG_ptPlt <- ggplot(EPGDat) + 
  geom_point(aes(x=freq,y=totalSyns), size = 1, alpha = 0.25) + 
  geom_point(data = EPGDat_mean, aes(x = freq, y = meanSyns, fill =as.factor('mean')), size = 2, color = 'red', alpha=0.5) +
...
design <- "
  123344
"
statPlts <- EBEtc_InPlt + EBPBEtc_OutPlt +
  PBFBNO_InPlt + FBNO_OutPlt +
