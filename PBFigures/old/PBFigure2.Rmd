...
Plot a cosine fit for the D7 inputs onto PEG neurons from EPG neurons assuming a von Mises input
```{r}
typesToPlot <- c("PEG","PFGs")
...
tpPlts <- list()
for (t in 1:length(typesToPlot)){
   # Get the von Mises profiles
  vMOut <- EPGToD7Transform(typesToPlot[t],actProfvM)
  
  # Get the profile stats
  D7OutputShape_stats <- D7ShapeMeanSD(vMOut)
  
  g_D7ToTp_vM <- ggplot(D7OutputShape_stats) + geom_line(aes(x=glom,y=mean,group=1),color='black') +
  geom_line(aes(x=glom,y=mean-sd,group=1),color='gray') +
  geom_line(aes(x=glom,y=mean+sd,group=1),color='gray')
  ...
  g_D7ToTp_vM <- g_D7ToTp_vM + geom_line(data=dffit,aes(x=pts,y=wMean,group=1),color='red')
  g_D7ToTp_vM <- g_D7ToTp_vM + theme_paper() + xlab('EPG input position') + ylab('# of synapses') + 
    scale_y_continuous(expand = c(0, 0)) + ggtitle(paste0(typesToPlot[t],'-von Mises'))
  
  tpPlts[[t]] <- g_D7ToTp_vM
  
  
  # Get the impulse profiles
  impOut <- EPGToD7Transform(typesToPlot[t],actProfImp)
  
  # Get the profile stats
  D7OutputShape_stats <- D7ShapeMeanSD(impOut)

  g_D7ToTp_imp <- ggplot(D7OutputShape_stats) + geom_line(aes(x=glom,y=mean,group=1),color='black') +
  geom_line(aes(x=glom,y=mean-sd,group=1),color='gray') +
  geom_line(aes(x=glom,y=mean+sd,group=1),color='gray')
  ...
  g_D7ToTp_imp <- g_D7ToTp_imp + geom_line(data=dffit,aes(x=pts,y=wMean,group=1),color='red')
  g_D7ToTp_imp <- g_D7ToTp_imp + theme_paper() + xlab('EPG input position') + ylab('# of synapses') + 
    scale_y_continuous(expand = c(0, 0)) + ggtitle(paste0(typesToPlot[t],'-impulse'))
  
  tpPlts[[t + 2]] <- g_D7ToTp_imp
}

pdf('C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\FirstDraftFigs\\PB\\PEGPFGFits.pdf')
grid.arrange(grobs=tpPlts,nrow=2)
dev.off()
```