...
print(conmatPlot)
save_plot('C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\PB\\ForPaper\\EPGD7IbSpsPConn.pdf',conmatPlot)
```
...
conmatPlot <- ggplot(PBRegInputs) + 
  scale_fill_gradient2(low="thistle", mid="blueviolet", high="black", midpoint =0.5*max(PBRegInputs$regInput),
                       limits=c(0,max(PBRegInputs$regInput))) +
...
g_D7ToEPG <- g_D7ToEPG + theme_t() + xlab('EPG input position') + ylab('# of synapses') + 
  scale_y_continuous(expand = c(0, 0))
print(g_D7ToEPG)
```
