...
sTCols <- data.frame(
  type = c("Delta7","EPG","EPGt","SPS-PB","LPsP","P","PEG","PEN","PFGs","PFL","PFN","PFR","SIFamide"),
  colorID = c(28,12,32,21,30,31,33,16,27,7,26,1,2)
)
...
-tangential
```{r}
tanTypes <- c("P1-9", "Delta7", "P6-8P9", "LPsP", "SpsP")
...
gPBGloms <- ggplot(synsByType) + geom_tile(aes(x = PBGlom,y=type, fill=log10(syns+1))) + 
  ...
  scale_fill_viridis_c(option = "B", direction = -1, limits=c(0, 5)) + theme_cowplot()
...
pdf("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\PB\\ForPaper\\PBOverview_1.pdf", width = unit(8,"in"), height = unit(6,"in"))
...
```{r}
EPG_CT_PB <- getConnectionTable(getBodyIdsForList(c("EPGt")),"POST","PB")
...
EPG_CT_EB <- getConnectionTable(getBodyIdsForList(c("EPGt")),"PRE","CX")
...
Previously uncharacterized neuron types
 - Summary of inputs and outputs (switch from bar graph to network style diagrams)
 - P6-8P9s as D7s