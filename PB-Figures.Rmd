...
Plot a von Mises profile in the EPGs
```{r}
# Create a von Mises profile in the EPGs
...
EPGProf <- data.frame(nameid = PBRename(EPG_2_Delta7$name.from,EPG_2_Delta7$from) %>% unique(), act = EPGact, glom = allEPGGloms)
EPGProf$glom <- factor(EPGProf$glom, levels = gloms)
...
pdf("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\PB\\ForPaper\\ActivityThroughPB.pdf", width = unit(3,"in"), height = unit(6,"in"))
plot_grid(g_EPGact_in_EB,g_EPGact_in_PB,g_D7act,g_EPGinAct,align="v",ncol=1,rel_heights = c(0.8,1,1.2,1))
dev.off()
```
...
conTab <- getConnectionTable(getTypesTable(PBTypes)$bodyid,"POST","PB")
conTab <- conTab %>% filter(!is.na(type.to) & !is.na(name.to))
  
conTab$name.to <- factor(conTab$name.to,
        levels = glomSort(conTab$name.to))

conTab$nameid <- PBRename(conTab$name.from,conTab$from)
conTab$partnerid <- PBRename(conTab$name.to,conTab$to)

conmatPlot_EPG <- ggplot(conTab %>% filter(type.from == "EPG" & type.to != "Delta7")) + 
...
print(conmatPlot_EPG)

conmatPlot_PFNv <- ggplot(conTab %>% filter(type.from == "PFNv" & type.to != "Delta7")) + 
  scale_fill_gradient2(low="thistle", mid="blueviolet", high="black", midpoint =0.5*max(conTab$ROIweight),
                       limits=c(0,max(conTab$ROIweight))) +
  geom_tile(aes(partnerid,nameid,fill=ROIweight)) +
  theme_cowplot() + coord_fixed(ratio = 1) + 
...
print(conmatPlot_PFNv)

conmatPlot_Other <- ggplot(conTab %>% filter(!(type.from %in% c('EPG','Delta7','EPGt','IbSpsP','LPsP','SpsP','P6-8P9','P1-9','PFNv')) &
                                               type.to != "Delta7")) + 
  scale_fill_gradient2(low="thistle", mid="blueviolet", high="black", midpoint =0.5*max(conTab$ROIweight),
                       limits=c(0,max(conTab$ROIweight))) +
  geom_tile(aes(partnerid,nameid,fill=ROIweight)) +
