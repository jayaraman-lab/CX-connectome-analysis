...
for (nnn in 1:length(unique(RingNeuronSynsAll_BU$name)) ){
  ...
  contour_95 <- with(kd, contourLines(x=eval.points[[1]], y=eval.points[[2]], z=estimate, levels=cont["25%"])[[1]])
...
ppp=ggplot() + geom_polygon(data=BU_Outline_Smooth, aes(x = x, y = -z), colour='black', fill=NA, size = 0.75 ) + 
  geom_polygon(data=ContourAll, aes(x=x, y=-y,color=name, fill = name, alpha=0.2)) + theme_bw()
