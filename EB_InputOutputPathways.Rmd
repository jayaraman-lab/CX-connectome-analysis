...
Rneuron_Connect_df=filter(roi_Connect_df, startsWith(unlist(roi_Connect_df$type), "R"))
...
p1<-ggplot(R_R_plotdata, aes(x=roi_names, y=type, size=count)) + geom_point(aes(color=roi)) +
  facet_grid(cols=vars(prepost)) +  theme_bw() + theme(axis.text.x = element_text(angle = 90))  + guides(fill=FALSE)
print(p1)
ggsave(paste(PlotDir, "RingNeuron_","inputOutputRegions_perType.pdf",sep=""), plot = p1, device='pdf', scale = 1, width = 14, height = 8, units ="in", dpi = 600, limitsize = TRUE)

# Plot a subset of the data only in some ROIs
SynThresh=30
...
p2<-ggplot(R_R_plotdataSubset, aes(x=roi_names, y=type, size=count)) + geom_point(aes(color=roi)) + 
  facet_grid(cols=vars(prepost)) + theme_bw() + theme(axis.text.x = element_text(angle = 90))  + 
  guides(fill=FALSE) + ggtitle(paste("Ring Neuron Input/Output ROIs with #synapses > ",as.character(SynThresh),sep=""))
print(p2)
ggsave(paste(PlotDir, "FIGURE_RingNeuron_","inputOutputRegions_perType.pdf",sep=""), plot = p2, device='pdf', scale = 1, width = 14, height = 8, units ="in", dpi = 600, limitsize = TRUE)
...
ExRneuron_Connect_df=filter(roi_Connect_df, startsWith(unlist(roi_Connect_df$type), "ExR"))
...
# Get right ExRs and the left ExRs
R_ExR_plotdata=filter(ExRneuron_Connect_bytype_df, neuron_hemi=='R')
L_ExR_plotdata=filter(ExRneuron_Connect_bytype_df, neuron_hemi=='L')
...
print(p4)
ggsave(paste(PlotDir, "ExR_ALLThresh_","InOutRegions_perType.pdf",sep=""), plot = p4, device='pdf', scale = 1, width = 20, height = 8, units ="in", dpi = 600, limitsize = TRUE)
...
p6<-ggplot(ExRneuron_Connect_bytype_dfThresh_NoLayers, aes(x=roi_names, y=type_name, size=count)) + geom_point(aes(color=roi)) +
  ...
  ggtitle(paste("ALL ExR  Input/Output ROIs with #synapses after count thresholding at ",as.character(SynThresh),sep=""))
print(p6)
ggsave(paste(PlotDir, "ExR_ALLThresh_NoLayers_","InOutRegions_perType.pdf",sep=""), plot = p6, device='pdf', scale = 1, width = 20, height = 8, units ="in", dpi = 600, limitsize = TRUE)
...
p7<-ggplot(ExRneuron_Connect_bytype_dfThresh_EBLayers, aes(x=roi_names, y=type_name, size=count)) + geom_point(aes(color=roi)) +
  ...
  ggtitle(paste("ALL ExR  Input/Output ROIs with #synapses after count thresholding at ",as.character(SynThresh),sep=""))
print(p7)
ggsave(paste(PlotDir, "ExR_ALLThresh_EBLayers_","InOutRegions_perType.pdf",sep=""), plot = p7, device='pdf', scale = 1, width = 12, height = 6, units ="in", dpi = 600, limitsize = TRUE)


# Plot the ExRs again, but only in FB
ExRneuron_Connect_bytype_dfThresh_EBLayers=filter(ExRneuron_Connect_bytype_dfThresh, (startsWith(ExRneuron_Connect_bytype_dfThresh$roi,"FB")))

p8<-ggplot(ExRneuron_Connect_bytype_dfThresh_EBLayers, aes(x=roi_names, y=type_name, size=count)) + geom_point(aes(color=roi)) +
  facet_grid(cols=vars(prepost)) +  theme_bw() + theme(axis.text.x = element_text(angle = 90))  + guides(fill=FALSE) +
  ggtitle(paste("ALL ExR  Input/Output ROIs with #synapses after count thresholding at ",as.character(SynThresh),sep=""))
print(p8)
ggsave(paste(PlotDir, "ExR_ALLThresh_FBLayers_","InOutRegions_perType.pdf",sep=""), plot = p8, device='pdf', scale = 1, width = 12, height = 6, units ="in", dpi = 600, limitsize = TRUE)
...
print(p10)
ggsave(paste(PlotDir, "NonR_ALLThresh_","InOutRegions_perType.pdf",sep=""), plot = p10, device='pdf', scale = 1, width = 20, height = 8, units ="in", dpi = 600, limitsize = TRUE)
...
p12<-ggplot(NonR_Connect_bytype_dfThresh_NoLayers, aes(x=roi_names, y=type_name, size=count)) + geom_point(aes(color=roi)) +
  ...
  ggtitle(paste("ALL Non R  Input/Output ROIs with #synapses after count thresholding at ",as.character(SynThresh),sep=""))
print(p12)
ggsave(paste(PlotDir, "NonR_ALLThresh_NoLayers_","InOutRegions_perType.pdf",sep=""), plot = p12, device='pdf', scale = 1, width = 20, height = 8, units ="in", dpi = 600, limitsize = TRUE)
...
p13<-ggplot(NonRneuron_Connect_bytype_dfThresh_EBLayers, aes(x=roi_names, y=type_name, size=count)) + geom_point(aes(color=roi)) +
  ...
  ggtitle(paste("ALL Non R  Input/Output ROIs with #synapses after count thresholding at ",as.character(SynThresh),sep=""))
print(p13)
ggsave(paste(PlotDir, "NonR_ALLThresh_EBLayers_","InOutRegions_perType.pdf",sep=""), plot = p13, device='pdf', scale = 1, width = 12, height = 6, units ="in", dpi = 600, limitsize = TRUE)


# Plot the NonRs again, but only in FB
NonRneuron_Connect_bytype_dfThresh_EBLayers=filter(NonR_Connect_bytype_dfThresh, (startsWith(NonR_Connect_bytype_dfThresh$roi,"FB")))

p14<-ggplot(NonRneuron_Connect_bytype_dfThresh_EBLayers, aes(x=roi_names, y=type_name, size=count)) + geom_point(aes(color=roi)) +
  facet_grid(cols=vars(prepost)) +  theme_bw() + theme(axis.text.x = element_text(angle = 90))  + guides(fill=FALSE) +
  ggtitle(paste("ALL Non R  Input/Output ROIs with #synapses after count thresholding at ",as.character(SynThresh),sep=""))
print(p14)
ggsave(paste(PlotDir, "NonR_ALLThresh_FBLayers_","InOutRegions_perType.pdf",sep=""), plot = p14, device='pdf', scale = 1, width = 12, height = 6, units ="in", dpi = 600, limitsize = TRUE)
