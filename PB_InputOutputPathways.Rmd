...
ggsave(paste(PlotDir, "A_Figure_PBAll_InputOutput_perType_Absolute.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
...
ggsave(paste(PlotDir, "R_Figure_PBAll_InputOutput_perType_Relative.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
       width = 16, height = 16, units ="in", dpi = 600, limitsize = TRUE)
...
Input_SynThresh=12          # 12 is good
Input_RelSynThresh=0.01     # 0.01 is good
Output_SynThresh=12          # 4 is good
Output_RelSynThresh=0.005     # 0.1 good



ROISubset_Thresh=filter(ROISubset, ((ROISubset$RelativeCount  > Input_RelSynThresh | ROISubset$count>Input_SynThresh)              & ROISubset$prepost == "Input ROI")  |
                                   ((ROISubset$count > Output_SynThresh            | ROISubset$RelativeCount>Output_RelSynThresh)  & ROISubset$prepost == "Output ROI") )
...
InputHistDat= rbind( subset(ROISubset, (prepost == "Input ROI")) %>% mutate(TTT='ALL'), 
                     subset(ROISubset_Thresh, (prepost == "Input ROI")) %>% mutate(TTT='Threshold'))
OutputHistDat= rbind(subset(ROISubset, (prepost == "Output ROI")) %>% mutate(TTT='ALL'), 
                     subset(ROISubset_Thresh, (prepost == "Output ROI")) %>% mutate(TTT='Threshold'))
...
ggsave(paste(PlotDir, "A_Figure_PBAll_InputOutput_perType_RelativeThresh.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
       width = 16, height = 16, units ="in", dpi = 600, limitsize = TRUE)
...
ggsave(paste(PlotDir, "R_Figure_PBAll_InputOutput_perType_AbsoluteThresh.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
       width = 16, height = 16, units ="in", dpi = 600, limitsize = TRUE)
...
plot_ly(data = subset(ROISubset, prepost=="Input ROI"),  x = ~roi_names, y = ~type_name, z = ~count, type = "heatmap")
plot_ly(data = subset(ROISubset, prepost=="Input ROI"),  x = ~roi_names, y = ~type_name, z = ~RelativeCount, type = "heatmap")
