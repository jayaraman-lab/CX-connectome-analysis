...
filterROISummary <- function(ROIsummary,nBagROI,roiTypes,rois,renaming=cxRetyping){
  roiConnections <- neuronBag(filter(nBagROI$names, type %in% roiTypes$type),slctROI = unique(filter(ROIsummary,roi %in% rois$roi)$roi),verbose=TRUE,selfRef=TRUE,renaming=renaming)
...
haneschBUR_ttFCustom <- haneschPlot(BUSummary_ttFiltered %>% filter(type %in% BURTypes$type & !(databaseType %in% c("CRE013","LHCENT11","LHPV5l1","ExR7","SMP238"))),grouping="supertype2",flip=T,roiSelect = BURois,roiLabel = labelRoisBU,interactive=TRUE,showCount = TRUE,theme=theme_paper_grid())
haneschBUR
haneschBUR_ttF
...
save_plot(paste0(haneschSaveFolder,"BU/BU_RTypes_filteredCustom.svg"),haneschBUR_ttFCustom,ncol=2,nrow=1.5,base_width = 8.5/2)
```
...
#### Restrict to "full" PB neurons
```{r}
PBsummary <- PBSummary %>% filter((type %in% PBTypes$type) & ((supertype3 %in% c("PB Interneurons","PB Input")  & grepl("_R",type))  | (supertype3 == "EB Columnar" & !grepl("_R",type)) | (supertype2 %in% c("PFG","PFL","PFR") & !grepl("_R",type)) | (supertype2 == "PFN" & grepl("_R",type))))
PBsummary$supertype2 <- factor(PBsummary$supertype2,levels=c("Δ7","P","LPsPB","SPS-PB","EPGt","EPG","PEN","PEG","PFG","PFL","PFR","PFN"))
PBsummary_filtered <- PBSummary_ttFiltered %>% filter((type %in% PBTypes$type) & ((supertype3 %in% c("PB Interneurons","PB Input")  & grepl("_R",type))  | (supertype3 == "EB Columnar" & !grepl("_R",type)) | (supertype2 %in% c("PFG","PFL","PFR") & !grepl("_R",type)) | (supertype2 == "PFN" & grepl("_R",type))))
PBsummary_filtered$supertype2 <- factor(PBsummary_filtered$supertype2,levels=c("Δ7","P","LPsPB","SPS-PB","EPGt","EPG","PEN","PEG","PFG","PFL","PFR","PFN"))
```
...
haneschPBRedux_filtered <- haneschPlot(PBsummary_filtered %>% filter(type %in% PBTypes$type & !(databaseType %in% "SIFa")),roiSelect = PBRois,flip = T,grouping = "supertype2",roiLabel = labelRoisPB,interactive=TRUE,showCount = T,theme=theme_paper_grid())
haneschPBRedux_filtered
```
```{r}
save_plot(paste0(haneschSaveFolder,"PB/PBTypesRedux.svg"),haneschPBRedux,nrow=1,ncol=1.7,base_width = 8.5/2.5)
save_plot(paste0(haneschSaveFolder,"PB/PBTypesRedux_filtered.svg"),haneschPBRedux_filtered,nrow=1,ncol=1.7,base_width = 8.5/2.5)
```
...
```{r}
haneschFBSleep <- haneschPlot(FBSummary %>% filter(grepl("FB7B|FB7A|FB7K|FB6Z|FB6C_a|FB6E|FB6I|FB6C_b|FB6A|FB6G|FB6H",type) & !grepl("_L",type)),roiSelect=FBRois,flip = T,roiLabel = labelRoisFB,grouping="supertype1",interactive = TRUE,showCount = T,theme=theme_paper_grid()) + scale_size_area(max_size = 9,breaks=scales::breaks_log(n=6),name="# synapses")
haneschFBSleep_ttF <- haneschPlot(FBSummary_ttFiltered %>% filter(grepl("FB7B|FB7A|FB7K|FB6Z|FB6C_a|FB6E|FB6I|FB6C_b|FB6A|FB6G|FB6H",type) & !grepl("_L",type)),roiSelect=FBRois,flip = T,roiLabel = labelRoisFB,grouping="supertype1",interactive = TRUE,showCount = T,theme=theme_paper_grid())+ scale_size_area(max_size = 9,breaks=scales::breaks_log(n=6),name="# synapses")
haneschFBSleep
haneschFBSleep_ttF
```
...
save_plot(paste0(haneschSaveFolder,"FB/FBTangentialsRightSleep.svg"),haneschFBSleep,nrow=1,ncol=1,base_width = 8.5/2)
...
save_plot(paste0(haneschSaveFolder,"FB/FBTangentialsRightSleep_filtered.svg"),haneschFBSleep_ttF,nrow=1,ncol=1,base_width = 8.5/2)
```
