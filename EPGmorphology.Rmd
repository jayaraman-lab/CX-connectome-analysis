...
ggsave('modalityCDF_EL.pdf', plot = modalityCDFPlot, device='pdf', path = "../neuprintR_analysis_plots/")
}
...
medViolinPlot = ggplot(filter(medOfDist)) +
...
ggsave('medianDists_EL.pdf', plot = medViolinPlot, device='pdf', path = "../neuprintR_analysis_plots/")
...
ggsave('rankOrdering_EL.pdf', plot = rankOrderingPlot, device='pdf', path = "../neuprintR_analysis_plots/")
}
...
#ggsave('distDistance.pdf', plot = distDistancePlot, device='pdf', path = "../neuprintR_analysis_plots/")
```
...
rootPtsCoord = lapply(nronList, function(nron) nron$d[rootpoints(nron), c('X','Y','Z')])
...
#ggsave('radPosVsDist_quads.pdf', plot = radPosVsDistPlot, device='pdf', path = "../neuprintR_analysis_plots/")
...
#ggsave('depthVsDist_quads.pdf', plot = depthVsDistPlot, device='pdf', path = "../neuprintR_analysis_plots/")
...
```{r}
xmax = ceiling(max(synapses$radial))
ymax = ceiling(max(synapses$depth))
dx = 0.75
dy = 0.75
...
depthVRadDensPlot = ggplot() +
  stat_density_2d(data = filter(synapses, prepost == 1), aes(x = radial, y = depth, alpha = stat(nlevel)), fill = 'grey', bins = 40, contour_var = 'ndensity', geom = 'polygon')+
  geom_path(data=outline, aes(x=c1, y=c2), size = 0.5) +
  ...
  theme_paper() #+
  #geom_point(data = rootPtsNewCoord, aes(x = radial, y = depth), color = '#5A5156')

print(depthVRadDensPlot)

ggsave('depthVsRadial_EL.pdf', plot = depthVRadDensPlot, device='pdf', path = "../neuprintR_analysis_plots/", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
...
depthVRadMod = ggplot(filter(synapses, prepost == 1, modality %in% c('motor','ms','cv','iv','sleep'))) +
  stat_density_2d(aes(x = radial, y = depth, fill = modality, alpha = stat(nlevel)), bins = 40, contour_var = 'ndensity', geom = 'polygon')+
  geom_path(data=outline, aes(x=c1, y=c2), size = 0.5) +
  scale_fill_manual(values = modalityCMap) +
  theme_paper()
...
ggsave('depthVsRadial_modalities_EL.pdf', plot = depthVRadMod, device='pdf', path = "../neuprintR_analysis_plots/", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
...
ggsave('depthVsRadial_ms_perm23.pdf', plot = depthVRadMod, device='pdf', path = "../neuprintR_analysis_plots/", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
...
ggsave('depthVsRadial_iv_perm23.pdf', plot = depthVRadMod, device='pdf', path = "../neuprintR_analysis_plots/", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
```
