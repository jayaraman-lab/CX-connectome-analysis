...
PlotDir="C:/Users/labadmin/Documents/Cx_EM_Analysis/BradData/FB_Analysis/"
...
p0<-ggplot(roi_Connect_bytype_df, aes(x=roi_names, y=type_name, size=count)) + geom_point(aes(color=roi)) +
  ...
  ggtitle("ALL FB Neuron Input/Output ROIs")
...
FB_Type = FB_Type[ !(FB_Type %in% c("EPGt","EL","ExR8","PDM21a_pct","PEN_b(PEN2)","PVM09w_pct","SA1_a","SA1_b","SA2_a","SIFamide", "TuBu09_c","TuBu04")) ]
unique(FB_Type)
...
ROI_LIST=c("hemibrain","CX","NO","LX","LX(R)","LX(L)","LAL(R)","PB","FB","SNP(R)","SNP(L)","INP","CRE(R)","CRE(L)") # List of parent ROIs to include
...
ROI_LEVELS=c("FBl1(C)","FBl2(C)", "FBl3(C)", "FBl4(C)", "FBl5(C)", "FBl6(C)", "FBl7(C)", "FBl8(C)", "FBl9(C)",
             ...
             "VLNP(R)","VMNP(C)",
              ...
              "ATL(L)","ATL(R)", "CRE(-RUB)(L)", "CRE(-ROB,-RUB)(R)", "ROB(R)" , "RUB(L)" , "RUB(R)" , "IB(C)", "ICL(L)", "ICL(R)", "SCL(L)","SCL(R)",
             "GNG(C)", "PENP(C)",
...
ROISubset$FACET[ startsWith(ROISubset$type_name,"PFN") | startsWith(ROISubset$type_name,"PFR") | startsWith(ROISubset$type_name,"PFG")] <- "PB-FB Columnar Input"
...
ROISubset$FACET[ startsWith(ROISubset$type_name,"PFL") | startsWith(ROISubset$type_name,"FC") | startsWith(ROISubset$type_name,"FR") | startsWith(ROISubset$type_name,"FS")] <- "FB Columnar Outputs"  
...
ROISubset$FACET = factor(ROISubset$FACET, levels=c('FB Tangential','PB-FB Columnar Input', "Delta0s" ,"Delta6s" ,'FB Columnar Outputs', 
                                                   'ExRs FB-EB','OA','AB'))
...
ROISubset$type = factor(ROISubset$type, levels=c( sort(unique(ROISubset$type[ROISubset$FACET == "FB Tangential"]),decreasing = TRUE),
                                                         sort(unique(ROISubset$type[ROISubset$FACET == "PB-FB Columnar Input"]),decreasing = TRUE),
...
### Plot just the FB tangential neurons, using unilateral ROIs (Note, for unpaired structures this underestimtes the total synapses, since we're summing 
### left and right ROIs. But, there are other ROIs that are cut so there is no perfect soultion).
```{r}
...
# Need to sum "CRE(-ROB,-RUB)" and "CRE(-RUB)"
CRETemp=subset(ROISubset_Thresh_FBT_Uni, roi_names %in% c("CRE(-ROB,-RUB)", "CRE(-RUB)"))
CRETemp=aggregate(CRETemp[, c(4,5)], by=list(type=CRETemp$type, prepost=CRETemp$prepost, FACET=CRETemp$FACET), FUN=sum)
CRETemp$roi_names="CRE(-ROB,-RUB)"


# Combien the LAL and MB data with the other rois
ROISubset_Thresh_FBT_Uni=subset(ROISubset_Thresh_FBT_Uni, !(roi_names %in% c("LAL", "LAL(-GA)", "GA","MB", "MB(+ACA)", "CRE(-ROB,-RUB)", "CRE(-RUB)")))
...
ROISubset_Thresh_FBT_Uni=rbind(ROISubset_Thresh_FBT_Uni, CRETemp)

# Manually set levels for x Axis
ROI_LEVELS2=c("FBl1","FBl2", "FBl3", "FBl4", "FBl5", "FBl6", "FBl7", "FBl8", "FBl9",
             "EB","NO","LAL(+GA)","AB", "VLNP","VMNP", "SMP", "SIP", "SLP", "CRE(-ROB,-RUB)","ROB","RUB", "ATL", "SCL","MB")
...
# set levels
ROISubset_Thresh_FBT_Uni$FACET = factor(ROISubset_Thresh_FBT_Uni$FACET, levels=c("FB9","FB8","FB7","FB6","FB5","FB4","FB3","FB2","FB1"))
ROISubset_Thresh_FBT_Uni$type= as.character(ROISubset_Thresh_FBT_Uni$type)
ROISubset_Thresh_FBT_Uni$type = factor(ROISubset_Thresh_FBT_Uni$type, levels=c(sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB1"]),decreasing = FALSE),
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB2"]),decreasing = FALSE),
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB3"]),decreasing = FALSE),
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB4"]),decreasing = FALSE), 
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB5"]),decreasing = FALSE),
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB6"]),decreasing = FALSE),
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB7"]),decreasing = FALSE),
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB8"]),decreasing = FALSE),
                                                         sort(unique(ROISubset_Thresh_FBT_Uni$type[ROISubset_Thresh_FBT_Uni$FACET == "FB9"]),decreasing = FALSE)) )

ROISubset_Thresh_FBT_Uni$color=ROISubset_Thresh_FBT_Uni$type
ROISubset_Thresh_FBT_Uni$color=as.character(ROISubset_Thresh_FBT_Uni$color)
ROISubset_Thresh_FBT_Uni$color = factor(ROISubset_Thresh_FBT_Uni$color, levels=c( rev(sort(unique(ROISubset_Thresh_FBT_Uni$color)) ) ))

# Plot all of the ROIs (and sub ROIs) that all FB neurons innervate
p0<-ggplot(ROISubset_Thresh_FBT_Uni, aes(x=roi_names, y=type, size=count))   +  
  facet_grid(cols=vars(prepost), rows=vars(FACET), scales="free_y", space="free_y") + 
  geom_point(shape = 21, colour = "black", aes(fill = color, stroke=1.5)) +  scale_color_brewer( direction = -1) +
...
ggsave(paste(PlotDir, "FIGURE_ROI_FB_Tangential.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
...
ROISubset_Thresh_NonFB=subset(ROISubset_Thresh, ! (FACET=="FB Tangential"| FACET=="FB Columnar Outputs" | FACET=="ExRs FB-EB" | FACET=="OA") ) 
...
# Set levels again
ROISubset_Thresh_NonFB$FACET = factor(ROISubset_Thresh_NonFB$FACET, levels=c('PB-FB Columnar Input', "Delta0s" ,"Delta6s" , "AB"))


# Factor the types for plotting so the color is good
ROISubset_Thresh_NonFB$type = factor(ROISubset_Thresh_NonFB$type, levels=c(sort(unique(ROISubset_Thresh_NonFB$type[ROISubset_Thresh_NonFB$FACET == "PB-FB Columnar Input"]),decreasing = TRUE),
...
# Plot all of the ROIs (and sub ROIs) that all FB neurons innervate
p0<-ggplot(ROISubset_Thresh_NonFB, aes(x=roi_names, y=type_name, size=count))   + 
  facet_grid(cols=vars(prepost), rows=vars(FACET), scales="free_y", space="free_y") + 
  geom_point(shape = 21, colour = "black", aes(fill = type, stroke=1.5)) +
  theme_bw() + theme(axis.text.x = element_text(angle = 90))  + guides(fill=FALSE) +
  ggtitle(paste("FB Neuron Input/Output ROIs"," Thresholded "))
ggsave(paste(PlotDir, "FIGURE_ROI_FB_Intra.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
...
ROISubset_Thresh_Output=subset(ROISubset_Thresh, FACET=="FB Columnar Outputs" | FACET=="ExRs FB-EB" | FACET=="OA")


# Plot all of the ROIs (and sub ROIs) that all FB neurons innervate
p0<-ggplot(ROISubset_Thresh_Output, aes(x=roi_names, y=type_name, size=count))   + 
  facet_grid(cols=vars(prepost), rows=vars(FACET), scales="free_y", space="free_y") + 
  geom_point(shape = 21, colour = "black", aes(fill = type, stroke=1.5)) +
  theme_bw() + theme(axis.text.x = element_text(angle = 90))  + guides(fill=FALSE) +
  ggtitle(paste("FB Neuron Input/Output ROIs"," Thresholded "))
ggsave(paste(PlotDir, "FIGURE_ROI_FB_Outputs.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
       width = 16, height = 16, units ="in", dpi = 600, limitsize = TRUE)
...
# Plot the sum of tangential cells innervating each layer
```{r}


# Plot the percent of tangential cells that innervate each region by layer
ROISubset_Thresh_FBT_Uni_PerLayer=ROISubset_Thresh_FBT_Uni[, c(1,2,3,4)]
ROISubset_Thresh_FBT_Uni_PerLayer=complete(ROISubset_Thresh_FBT_Uni_PerLayer, type, roi_names, prepost)
ROISubset_Thresh_FBT_Uni_PerLayer$count[is.na(ROISubset_Thresh_FBT_Uni_PerLayer$count)]<-0
ROISubset_Thresh_FBT_Uni_PerLayer$type=sapply(ROISubset_Thresh_FBT_Uni_PerLayer$type, substring, 1, 3)
ROISubset_Thresh_FBT_Uni_PerLayer$count[ROISubset_Thresh_FBT_Uni_PerLayer$count>0]=1
ROISubset_Thresh_FBT_Uni_PerLayer=aggregate(ROISubset_Thresh_FBT_Uni_PerLayer[,4], by=list(ROISubset_Thresh_FBT_Uni_PerLayer$type,
                          ROISubset_Thresh_FBT_Uni_PerLayer$roi_names, ROISubset_Thresh_FBT_Uni_PerLayer$prepost), FUN = mean)
colnames(ROISubset_Thresh_FBT_Uni_PerLayer)[c(1,2,3)]<-c("type","roi_names","prepost")
ROISubset_Thresh_FBT_Uni_PerLayer=ROISubset_Thresh_FBT_Uni_PerLayer[!(ROISubset_Thresh_FBT_Uni_PerLayer$count==0),]

# Threshold based on proportion
Prop_Thresh=0.3
ROISubset_Thresh_FBT_Uni_PerLayer=subset(ROISubset_Thresh_FBT_Uni_PerLayer, count>Prop_Thresh)
...
# Plot all of the ROIs (and sub ROIs) that all FB neurons innervate
p0<-ggplot(ROISubset_Thresh_FBT_Uni_PerLayer, aes(x=roi_names, y=type, size=count))   +  
  facet_grid(cols=vars(prepost), scales="free_y", space="free_y") + 
  geom_point(shape = 21, colour = "black", aes(fill = type, stroke=1.5)) +  scale_color_brewer( direction = -1) +
   theme_bw() + theme(axis.text.x = element_text(angle = 90))  + guides(fill=FALSE) +
  ggtitle(paste("FB Tangential Neuron Proportion Input/Output ROIs"," Thresholded "))
ggsave(paste(PlotDir, "FIGURE_ROI_FB_Tangential_Proportion.pdf",sep=""), plot = p0, device='pdf', scale = 1, 
       width = 12, height = 8, units ="in", dpi = 600, limitsize = TRUE)


# compute difference
Tangential_Difference=ROISubset_Thresh_FBT_Uni_PerLayer
Tangential_Difference=complete(Tangential_Difference, type, roi_names, prepost)
Tangential_Difference$count[is.na(Tangential_Difference$count)]<-0

Inputs=subset(Tangential_Difference, prepost=="Input ROI") 
colnames(Inputs)[4]<-"InputProportion"
Inputs=Inputs[,c(1,2,4)]


Outputs=subset(Tangential_Difference, prepost=="Output ROI")
colnames(Outputs)[4]<-"OutputProportion"
Outputs=Outputs[,c(1,2,4)]

InputOutput <- merge(Inputs, Outputs, by=c("type","roi_names")) 

InputOutput$OutMinusIn=InputOutput$OutputProportion - InputOutput$InputProportion
InputOutput$Average= (InputOutput$OutputProportion + InputOutput$InputProportion)/2

InputOutput=subset(InputOutput, OutMinusIn>0 | Average>0)



hanesch <- ggplot(InputOutput,aes(x=roi_names,y=type)) + 
  geom_point(aes(size=Average,fill=OutMinusIn),colour="black",shape=21)+
  ...
  scale_size_continuous(name = "Propotion with synapses") +
  theme_minimal() +  ylab("FB tangetial neurons by layer")
ggsave(paste(PlotDir, "FIGURE_ROI_FB_Tangential_hanesch.pdf",sep=""), plot = hanesch, device='pdf', scale = 1, 
       width = 14, height = 8, units ="in", dpi = 600, limitsize = TRUE)
