...
BisectShape <- function(UpperLower){
  
  Upper=subset(UpperLower, UpLow=="Upper")
  Lower=subset(UpperLower, UpLow=="Lower")

  Mid=data.frame(X=numeric(), Y=numeric())
  for (ppp in 1:length(Upper$c1)){
    Distance=((Lower$c1-Upper$c1[ppp])^2  + (Lower$c2-Upper$c2[ppp])^2 )^0.5
...
Outline_L3_XZ_MID=BisectShape_Ortho(Outline_L3_XZ_Cut_UpLow)
...
ggplot() +  geom_path(data=Outline_L3_XZ, aes(x=c1, y=c2), size = 1, color="red") +
  ...
  geom_path(data=Outline_L3_XZ_MID, aes(x=X, y=Y), size = 1, color="orange") + coord_fixed(ratio = 1)
