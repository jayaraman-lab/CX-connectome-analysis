...
BisectShape <- function(Outline, Axis){
  
  
  if (Axis=="X"){
    colnames(Outline)[colnames(Outline)=="c1"] <- "X"
    colnames(Outline)[colnames(Outline)=="c2"] <- "Y"
  } 
  
  # Get furthest point out on layer outline 
  Pos=subset(Outline, X>0)
  Neg=subset(Outline, X<0)
  Pos_Rad=( abs(Pos$X) + abs(Pos$Y) )
  Neg_Rad=( abs(Neg$X) + abs(Neg$Y) )
  #Neg_Rad=(Neg$X^2 + Neg$Y^2)^0.5
  Pos_IND=which(Pos_Rad==max(Pos_Rad))
  Neg_IND=which(Neg_Rad==max(Neg_Rad))
  
  XXX=seq( Neg$X[Neg_IND] , Pos$X[Pos_IND], by=100)
  YYY_Max=numeric()
  YYY_Min=numeric()
  for (xxx in 1:(length(XXX)-1)){
    TempLines=subset(Outline, X >= XXX[xxx] & X < XXX[xxx+1])
    YYY_Max[xxx]=max(TempLines$Y)
    YYY_Min[xxx]=min(TempLines$Y)
   
  }
  Line_Max=data.frame(X=XXX[1:(length(XXX)-1)], Y=YYY_Max, Half="Upper")
  Line_Min=data.frame(X=XXX[1:(length(XXX)-1)], Y=YYY_Min, Half="Lower")
  Line=rbind(Line_Max, Line_Min)
...
UpperLower_FB <- function(Outline){
  ...
  Lower$UpLow="Lower"
  UpLow=rbind(Upper, Lower)
  return(UpLow)
}


# Cut the tips off the layer
Outline_L3_XZ_Cut=CutTips_FB(Outline_L3_XZ, 0.85)


# Get upper and lower half of layer 
Outline_L3_XZ_Cut_UpLow=UpperLower_FB(Outline_L3_XZ_Cut)



ggplot() +  geom_path(data=Outline_L3_XZ, aes(x=c1, y=c2), size = 1, color="red") +
  geom_path(data=Outline_L3_XZ_Cut, aes(x=c1, y=c2), size = 1, color="black") + 
  geom_path(data=subset(Outline_L3_XZ_Cut_UpLow, UpLow=="Upper"), aes(x=c1, y=c2), size = 1, color="blue")  +
  geom_path(data=subset(Outline_L3_XZ_Cut_UpLow, UpLow=="Lower"), aes(x=c1, y=c2), size = 1, color="green") 
