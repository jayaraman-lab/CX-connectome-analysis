...
```{r}
myTypeList = c("GLNO","LNO1","LNO2","LNO3","LNOa", "LCNOpm","LCNOp", "ER1_a","ER1_b","ER3a_b","ER3a_c", "ER6")
...
```{r}
myTypes_bag_full = create_neuronBag(myTypeList)
...
ggsave(paste("inputAndOutputComposition_",saveName,'.pdf', sep=''),plot = bar, device='pdf', path = "../neuprintR_analysis_plots/",
    scale = 1, width=25, height=12, units ="cm", dpi = 600, limitsize = TRUE)
...
ggsave(paste("inputAndOutputComposition_",saveName,'_v2.pdf', sep=''),plot = bar2, device='pdf', path = "../neuprintR_analysis_plots/",
...
```{r}
myROIs = c("LAL(-GA)(R)","GA(R)")#unique(myTypes_in_plot$roi)[unique(myTypes_in_plot$roi)!="NO"] #c("PB", "EB", "NO(R)") #c("LAL(R)","WED(R)", "GNG")#
myROIsName = paste(myROIs, collapse = '-') #"PB_EB_NOr" #
myROIs
...
myTypeList_lat = unique(typesInput_combiRoi$type.to)
```
...
dev.print(pdf, paste0("../neuprintR_analysis_plots/inputCorrelation_",saveName,'_', myROIsName,'_nBag_tree.pdf'), width=8, height=4)
...
cut = 0.8
tplt <- plot(hcl,hang= -.5,cex = 0.8)# + rect.hclust(hcl, h=cut, border = "red")
dev.print(pdf, paste0("../neuprintR_analysis_plots/inputCorrelation_",saveName,'_', myROIsName,'_onPre_nBag_tree.pdf'), width=8, height=4)
