...
inOutContributionDataPrep <- function(myTypes_in_filt, myTypes_out_filt,  supertypeLevel){
  ...
  if(supertypeLevel == 0){
    #selectSupertypeSet(sttable,default_level = 3,exceptions = list("Terra incognita"=2),exceptionLevelMatch = 3) 
    ...
    inData = left_join(inData,selectSupertypeSet(sttable,default_level = 3,
                        exceptions = list("AOTU"=2,"TuBu"=2,"LAL"=2,"CRE"=2,"PS"=2,"IB"=2,"WED"=2,"MBON"=2, "LNO"=2,"FB output"=2,"ER"=2, "ExR"=2),exceptionLevelMatch = 2) %>% 
...
inOutContributionPlot <- function(data){
  ...
  if(ncol <= 32){
    bar = bar + scale_fill_manual(values=paletteer_d("Polychrome::palette36", n=4+length(unique(data$partner)))[c(-seq(4))])
  }else if(ncol <= 36){
...
for(st in c(-1, 0,2,3)){
  ...
  ggsave(paste("inputAndOutputComposition_",saveName,"_supertype",st,'.pdf', sep=''),plot = bar, device='pdf', path = "../neuprintR_analysis_plots/",
      scale = 1, width=23, height=10, units ="cm", dpi = 600, limitsize = TRUE)
