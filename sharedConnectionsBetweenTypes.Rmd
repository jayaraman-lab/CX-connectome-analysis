...
```{r}
myTypes = c("GLNO","LNO1","LNO2","LNO3","LNOa","LCNOpm","LCNOp") #c("ExR.*")#
side = "_R"
saveName = "LNO_comarison"
...
slctROIs = slctROIs[!grepl("\\(L\\)",slctROIs)]
...
usth = 10
dsth = 10
intypeROI = getROISummary(inputTypes_bag) 

intypeROI = intypeROI %>% filter(roi %in% slctROIs) %>%
  filter(upstream > usth) %>%
  filter(downstream > dsth) %>%
...
outtypeROI = getROISummary(outputTypes_bag) 
