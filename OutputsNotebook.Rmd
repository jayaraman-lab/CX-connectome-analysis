...
```{r}
outputBag <- create_neuronBag("FC[1|2].*|FS.*|^FR.*|ExR5.*|ExR7.*|ExR8.*|PFL.*|PFR_b.*|EL.*|EPG.*|PEG.*|PFG.*",verbose=TRUE)
```
...
```{r}
reducedBag <- filter(outputBag,filterPartners = FALSE,(grepl("_L.*",type) & !startsWith(type,"ExR")) | type == "ExR8_R" | type == "ExR7_R" | type == "ExR5_R" | type =="PFGs_R")
```
...
```{r}
outputsInnervation_L <- haneschPlot(outputsByRoi,roiSelect = roisOut,grouping = "supertype2",roiLabel = roiOutLabels,flip=TRUE,interactive=TRUE)
outputsInnervation_L
```

```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/NewOutputs/CXoutputsHaneschL.svg",outputsInnervation_L,width=14,height=8)
```
...
```{r}
outputsInOutBag <- create_neuronBag(allOuts,selfRef = TRUE,verbose=TRUE)
```
...
```
```{r}
CXtoOutputs <- filter(inputs_toOutputs,customSupertype !="Other") %>% group_by(type.to,databaseType.to) %>% summarize(totalCX = sum(weightRelative)) %>% ungroup() %>% filter(!(databaseType.to %in% CXtypes$databaseType)) %>% arrange(desc(totalCX))
```
