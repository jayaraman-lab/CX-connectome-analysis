...
```{r}
outputBag <- buildInputsOutputsByType("FC[1|2].*|FS.*|^FR.*|ExR7.*|ExR8.*|PFL.*|PFR_b.*",verbose=TRUE)
```
...
```{r}
reducedBag <- filter(outputBag,filterPartners = FALSE,(grepl("_L.*",type) & !startsWith(type,"ExR")) | type == "ExR8_R" | type == "ExR7_R")
```
...
```
```{r}
outputsByRoi <- getROISummary(reducedBag,filter=FALSE)
```


```{r}
outputsInnervation_L <- haneschPlot(outputsByRoi,roiSelect = roisOut,grouping = "supertype2",roiLabel = roiOutLabels,flip=TRUE)
```
...
```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/NewOutputs/CXoutputsHaneschL.svg",outputsInnervation_L,width=12,height=8)
```
...
```{r}
outputsOverview <- supertype(combineRois(outputsInOutBag,c("VLNP(R)","SNP(R)","VMNP(R)","INP","LX(R)","LX(L)"),"Output regions",singleNeuronThreshold=0.001))
...
```{r}
outputTypes <- selectSupertypeSet(CXtypes,default_level = 2,exceptions=list("PFL1"=1,"PFL2"=1,"PFL3"=1,"FS1"=1,"FS4"=1,"ExR7"=1,"ExR8"=1),exceptionLevelMatch = 1)
```


```{r}
inp <- outputsOverview$inputs %>% mutate(customSupertype = outputTypes$type[match(supertype.from1,outputTypes$supertype1)])
inp <- replace_na(inp,list("customSupertype" = "Other"))
```

```{r}
source("R/inProgress.R")
```

```{r}
test <- connectivityMatrix(inp,slctROI="Output regions",allToAll = FALSE,from = "type.from","type.to")
```


```{r}
#Clustering on 
testM <- t(test[rownames(test) %in% outputBag$names$type,])
d <- dist(testM/colSums(test),method="canberra")
hc <- hclust(d)
```

```{r}
inp$typeF <- factor(inp$type.to,levels=row.names(testM)[hc$order])
```


```{r}
inputProportions <- ggplot(inp,aes(x=typeF)) + geom_col(aes(y=weightRelative,fill=customSupertype),alpha=0.8) + scale_fill_manual(values=typesPalette(outputTypes$type),name="Input types",guide=guide_legend(ncol=2)) + theme_classic()  + theme(axis.text.x = element_text(angle = 90)) + labs(y="Relative weight",x="Output type")
inputProportions
```
