...
```{r}
outputBag <- buildInputsOutputsByType("FC.*|FS.*|^FR.*|ExR7.*|ExR8.*|PFL.*|PFR.*|EL.*",big=TRUE,nc=5)
```
...
```{r}
outputsInnervation_L <- haneschPlot(outputsByRoi %>% filter(grepl("_L.*",type) | startsWith(type,"ExR8")),roiSelect = roisOut,grouping = "supertype2")
```

```{r}
hanesch2000(outputBag %>% filter(grepl("_L.*",type) | startsWith(type,"ExR8")),roiSelect = roisOut)
```
...
```{r}
buildInputsOutputsByType(allOuts %>% filter(type == "1746300363"),selfRef=TRUE)
```


```{r}
outputsInOutBag <- buildInputsOutputsByType(allOuts,selfRef = TRUE,big=TRUE,nc=12)
```
...
```{r}
feedbackPathways2 <- outputsInOutBag$outputs %>% filter(databaseType.to %in% CXtypes$n.type & (roi %in% CXRois))

feedbackPathways <- outputsInOutBag$inputs  %>% filter((type.to %in% feedbackPathways2$type.from) & (type.from %in% outputBag$names$type)) %>% ungroup()
#feedbackPathways1 <- allOutsTable %>% filter(type.to %in% feedbackPathways2$type.from)
```

```{r}
outputsOutputsByRoi <- getROISummary(outputsInOutBag,filter=FALSE)
outputsSuperSummary <- compressROISummary(outputsOutputsByRoi,level = 1,stat = sum)
```
...
```{r}
haneschPlus(roiDF = outputsOutputsByRoi,connDF = feedbackPathways, rois=roisOut,groupPartnersBy = "supertype.from2")
#feedbacksPlot
```

```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/NewOutputs/feedbacks.svg",feedbacksPlot,height=8)
```
```{r}
feedbackG <- makeGraph(feedbackPathways,polarity="inputs",ROIs = (roisOut %>% filter(level1 != "CX"))$customRois)
```

```{r}
feedbackGraphPlot <- ggraph(feedbackG$graph,layout="stress") + 
      geom_edge_fan(aes(width=weightRelative,color=supertypeFrom2),alpha=0.5) + 
