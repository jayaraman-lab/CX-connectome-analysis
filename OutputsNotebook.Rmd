...
```{r}
outputBag <- create_neuronBag(CXOutputNeurons,selfRef = TRUE,slctROI=outputRegions,verbose=TRUE,computeKnownRatio=TRUE,omitInputs = TRUE)
...
### Do the same but for connectivity
...
```{r}
outputKind <- ggplot(outputsN1Table,aes(x=type.fromF,y=knownOutputContribution)) + geom_col(aes(fill=category)) + theme_paper_hgrid() + theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5,colour = overlapCols)) + scale_fill_paletteer_d("ggthemes::Classic_Gray_5",name="Output kind") +xlab("") + ylab("Fraction of significant output weights")
outputKind
```

```{r}
N1Mat <- connectivityMatrix(outputsN1Table,slctROIs = "Output_Regions",allToAll = FALSE,from = "type.from",to="type.to",value = "knownOutputContribution")
...
connCl <- hclust(N1Dist)
connClCut <- cutree(connCl,h=0.8)

connClOut <- hclust(N1DistOut)
connClOutCut <- cutree(connClOut,h=0.8)

test <- connectivity_clusters(outputsN1Table,slctROIs = "Output_Regions",clusterSide = "input",distance = cos_dist,knownStats = TRUE,h=0.8)
```

Name the "modules"
```{r}
clConnNames <- sapply(1:max(connClCut),function(i) paste(names(connClCut[connClCut==i]),collapse ="/"))
CXOutOverlap <- mutate(CXOutOverlap,clConnName=clConnNames[cl])
CXOutSyn <- mutate(CXOutSyn,clConnName=clConnNames[cl])

## Also create a palette for it
connCols <- paletteer::paletteer_d("Polychrome::palette36")[connClCut[rownames(N1Mat)[connCl$order]]+2]
names(connCols) <- clConnNames[connClCut[rownames(N1Mat)[connCl$order]]]
```

```{r}
connClPlot <- plot_dist(N1Dist) +
...
```{r}
connClPlotOut <- plot_dist(N1DistOut) +
   theme_paper_map() + scale_fill_distiller(palette = "Greys",name = "Cosine distance") +xlab("")+ylab("") +
   theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5))

connClPlotOut
```
Look at the connectivity matrix like that
```{r}
outMatMelted <- df_conn_mat(N1MatOut,connClOut$order,connCl$order)
outMatMelted <- mutate(outMatMelted,
                       clOut=connClOutCut[as.character(Outputs)],
                       clIn=connClCut[as.character(Inputs)],
                       category=outputsN1Table$category[match(Outputs,outputsN1Table$type.to)])
outMatCompressed <- group_by(outMatMelted,Inputs,clIn,clOut,category) %>% summarise(nOut = sum(clOut[1]==connClOutCut),
                                                                             avgWeightR = sum(value)/(nOut),
                                                                             n_pairs = n(),
                                                                             fracConn = n_pairs/(nOut)) %>% ungroup() 

ggplot(filter(outMatCompressed,category=="output")) + geom_point(aes(x=Inputs,y=as.factor(clOut),size=avgWeightR)) + theme_paper_hgrid() + theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5),axis.text.y=element_text()) + xlab("") + ylab("")+xlab("") + ylab("") + facet_grid(. ~ clIn,scales="free",space="free")

ggplot(filter(outMatMelted,category=="output")) + geom_point(aes(x=Inputs,y=Outputs,size=value)) + theme_paper_hgrid() + theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5),axis.text.y=element_text()) + xlab("") + ylab("")+xlab("") + ylab("") + facet_grid(clOut ~ clIn,scales="free",space="free")

ggplot(filter(outMatMelted,category=="CX loop")) + geom_tile(aes(x=Inputs,y=Outputs,fill=value)) + theme_paper_map() + theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5),axis.text.y=element_text()) + xlab("") + ylab("")+xlab("") + ylab("") + facet_grid(clOut ~ clIn,scales="free",space="free") + scale_fill_gradient2(low="#F7F2F7", mid="blueviolet", high="black", 
                         midpoint =0.5, limits=c(0,1)) 

ggplot(filter(outMatMelted,category=="CX loop")) + geom_tile(aes(x=Inputs,y=Outputs,fill=value)) + theme_paper_map()+ theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5),axis.text.y=element_text()) + xlab("") + ylab("")+xlab("") + ylab("") + facet_grid(clOut ~ clIn,scales="free",space="free") + scale_fill_gradient2(low="thistle", mid="blueviolet", high="black", 
                         midpoint =0.5, limits=c(0,1)) 


ggplot(filter(outMatMelted,category=="output")) + geom_tile(aes(x=Inputs,y=Outputs,fill=value)) + theme_paper_map()+ theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) + xlab("") + ylab("")+xlab("") + ylab("") + facet_grid(clOut ~ clIn,scales="free",space="free") + scale_fill_gradient2(low="thistle", mid="blueviolet", high="black", 
                         midpoint =0.5, limits=c(0,1)) 

ggplot(filter(outMatMelted,category=="Self")) + geom_tile(aes(x=Inputs,y=Outputs,fill=value)) + theme_paper_map()+ theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) + xlab("") + ylab("")+xlab("") + ylab("") + facet_grid(clOut ~ clIn,scales="free",space="free") + scale_fill_gradient2(low="thistle", mid="blueviolet", high="black", 
                         midpoint =0.5, limits=c(0,1)) 
```
...
for (t in unique(CXOutputNeurons$type)){
   plot3d(filter(CXOutSyn,type==t)[,c("x","y","z")],col=paletteer::paletteer_d("Polychrome::palette36")[connClCut[t]+2],add=T)
}
nview3d("ventral")
