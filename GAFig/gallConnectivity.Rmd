...
```{r}
EPGexample = neuprint_read_skeletons(neuprint_search("EPG.*",meta = FALSE))
ELexample = neuprint_read_skeletons(912596952)
PEGsexample = neuprint_read_skeletons(788461444)
PFRaexample = neuprint_read_skeletons(819828716) #PFR_b(PB11b)_L4_C5
FRexample = neuprint_read_skeletons(356670712)  #FR2(FQ4)_C4_L_small
...
```{r}
plot3d(gaMesh, col='maroon', alpha=0.4, add=TRUE)
...
plot3d(EPGexample, col='maroon', add=TRUE, WithNode=F)
plot3d(ELexample, col='tomato', add=TRUE, WithNode=F)
plot3d(PEGsexample, col='purple', add=TRUE, WithNode=F)
...
```{r}
rubSlctTypes = c("PFR")
rubSlctIDs = getBodyIdsForList(rubSlctTypes)
pre = rubSlctIDs$bodyid
post = pre
myConnections = getConnectionTable(pre, "POST", by.roi = TRUE)
myConnections = myConnections %>% filter(to %in% post) 

myConnections = myConnections %>% filter(roi %in% slctROIs) %>% filter(roi != "CX")
...
Since the Gall is a badly defined region, we can't easily collect neurons that go to the Gall based in the EM ROI.
```{r}
gallSlctTypes = c("EPG","PEG","EL","ER6","PFG")
...
```{r}
EL = create_neuronBag("EL.*")
```

```{r}
inOutsideCx = EL$inputs_raw %>% filter(roi != "CX") %>% filter(roi %in% slctROIs) %>% 
  group_by(roi, type.from, type.to) %>% summarise(weight=sum(weight))
inOutsideCx

outOutsideCx = EL$outputs_raw %>% filter(roi != "CX") %>%  filter(roi %in% slctROIs) %>%
  group_by(roi, type.from, type.to) %>% summarise(weight=sum(weight))
outOutsideCx
```