...
```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/Accessory/pyramidPlots/PFN-FBInputsByFBlayer.svg",PFNInputsByLayer,width = 20,height=10)
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/Accessory/pyramidPlots/PFN-FBOutputsByFBlayer.svg",PFNOutputsByLayer,width = 49,height=12)
```
...
Retyping:
Group L1 with R and R1 with L (innervation similar for PFRa)
```{r}
## Divide PFR taking into account the column 1 exception
PFRInOut <- redefineTypeByNameInList(PFRInOut,typeList = c("PFR_a"),pattern = "_L[2-7]|_R1",newPostFixes = c("_L*","_R*"))
```

For PFR_b it gets complicated
```{r}
PFRInOut <- redefineTypeByNameInList(PFRInOut,typeList = c("PFR_b"),pattern = "(^.*_L(?!.*C9))|(^.*C1.*)",perl=TRUE,newPostFixes = c("_L*","_R*"))
```
...
PFROutputsByLayer <-  makePyramidGraph(PFRInOut,fbROIs,plot=TRUE,polarity = "outputs")
```
...
```{r}
PFROutputsRob <- makePyramidGraph(PFRInOut,c("ROB(R)","CRE(-ROB,-RUB)(R)"),plot=TRUE,polarity = "outputs")
PFRInputsRob <- makePyramidGraph(PFRInOut,c("ROB(R)","CRE(-ROB,-RUB)(R)"),plot=TRUE,polarity = "inputs")
```
...
```{r}
ggsave("~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/Accessory/pyramidPlots/PFR-ROBOutputs.svg",PFROutputsRob,width = 30,height=15)
```

Crepine?

```{r}
PFROutputsCre <- makePyramidGraph(PFRInOut,c("CRE(-ROB,-RUB)(R)"),plot=TRUE,polarity = "outputs")
```
