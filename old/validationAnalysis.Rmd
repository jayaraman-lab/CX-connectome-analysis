...
- Path to the folder where the plots will be saved if need be  
```{r}
validationFolder <- #"~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/validation/"
```

- Loading some local helper functions
...
- Updating the figure theme for more space between facets
```{r}
...
We start by figure 4 as figure 3 necessitates loading different versions of the data-set, which requires to specify the data-set every time a call is made.

We want to select neurons innervating selectively one of the PB glomerulus of interest (either the ones that have been proofread extensively or their mirror symmetric).

`subroiSet`, defined in "validationsFunctions.R", returns a table of neurons innervating a given ROI (here a PB glomerulus) with added columns giving the what this innervation represents relative to the innervation in an enclosing ROI (here the PB)
...
- we then compute the connection tables (using `neuronBag`) for all the selected types:
```{r}
