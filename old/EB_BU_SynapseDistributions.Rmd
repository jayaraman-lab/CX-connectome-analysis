...
### Exclude neurons not in known EB types
```{r}
...
# Load known types
load("C:/Users/labadmin/Dropbox/FIBSEM CX Paper Jan 2020/Figures/EB/EB_Types.Rdata")

# Exclude bodies not in known types
ExcludedTypes=unique(NamedBodies$bodytype[!(NamedBodies$bodytype %in% EB_Type)])
NamedBodies=subset(NamedBodies, NamedBodies$bodytype %in% EB_Type)


```


### Get synapse distributions for all ring neurons (or load the previously generated data)
```{r}

# load(paste(SaveDir,"RingNeuronSynapses.RData",sep=""))
...
SynLocs=GetSynapseLocs(RingNeuronBodies$bodyid, "all",TRUE)
RingNeuronSynsAll=SynLocs
remove(SynLocs)


# EB
SynLocs=GetSynapseLocs(RingNeuronBodies$bodyid, "EB", TRUE)
RingNeuron_EBsyns=SynLocs
remove(SynLocs)

# BU(R)
SynLocs=GetSynapseLocs(RingNeuronBodies$bodyid, "BU(R)", TRUE)
RingNeuron_BUsyns=SynLocs
remove(SynLocs)

# GA(R)
SynLocs=GetSynapseLocs(RingNeuronBodies$bodyid, "GA(R)", FALSE)
RingNeuron_GAsyns=SynLocs
remove(SynLocs)


# LAL(-GA)(R)
SynLocs=GetSynapseLocs(RingNeuronBodies$bodyid, "LAL(-GA)(R)", FALSE)
RingNeuron_LALsyns=SynLocs
remove(SynLocs)


# Save synapse locations for later loading
save(RingNeuronSynsAll, RingNeuron_EBsyns, RingNeuron_BUsyns, RingNeuron_GAsyns, RingNeuron_LALsyns,
     UniqueNamedBodies, RingNeuronTypes, RingNeuronBodies, file = paste(SaveDir,"RingNeuronSynapses.RData",sep=""))
