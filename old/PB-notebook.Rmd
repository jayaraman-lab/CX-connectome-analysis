...
ggplot(PBpreNpost, aes(x=relMeanPre, y=relMeanPost)) +
...
connectivityBarPlot <- function(df,prepost){
  ...
  nronType = c("Delta7","EPG","EPGt","LPsP","P1-9","P6-8P9","PEG","PEN_a(PEN1)","PEN_b(PEN2)",
               "PFGs","PFL1","PFL2","PFL3","PFR_a","PFR_b",
               "PFNa","PFNd","PFNm_a","PFNm_b","PFNp_a","PFNp_b","PFNp_c","PFNv",
               "SpsP")
  colScale <- scale_fill_manual(values = paletteer_d("Polychrome::palette36"),drop=TRUE,limits = nronType)
  
  dodge <- position_dodge(width = 0.9)
  limits <- aes(ymax = weightRelative + varWeight,
                ymin = weightRelative - varWeight)
  
  if (prepost == "PRE"){
    p <- ggplot(data = df, aes(x = reorder(as.factor(type.from), -weightRelative), y = weightRelative, fill = type.from))
  } else {
    p <- ggplot(data = df, aes(x = reorder(as.factor(type.to), -weightRelative), y = weightRelative, fill = type.to))
  }
  ...
  p <- p + 
    ...
    scale_y_continuous(expand = c(0, 0)) + colScale
...
```{r}
IbSpsP_Post <- getConnectionTable(getBodyIdsForList(c("IbSpsP")),"POST","PB")
IbSpsP_Post_T2T <- getTypeToTypeTable(IbSpsP_Post)
pIbSpsP <- connectivityBarPlot(IbSpsP_Post_T2T[1:17,],"POST") + ggtitle("IbSpsP outputs")
print(pIbSpsP)
```
...
SpsP_Post_T2T <- getTypeToTypeTable(SpsP_Post)
pSpsP <- connectivityBarPlot(SpsP_Post_T2T,"POST") + ggtitle("SpsP outputs")
print(pSpsP)
...
LPsP_Post_T2T <- getTypeToTypeTable(LPsP_Post)
pLPsP_Post <- connectivityBarPlot(LPsP_Post_T2T,"POST") + ggtitle("LPsP outputs")
print(pLPsP_Post)

LPsP_Pre <- getConnectionTable(getBodyIdsForList(c("LPsP")),"PRE","PB")
LPsP_Pre_T2T <- getTypeToTypeTable(LPsP_Pre)
pLPsP_Pre <- connectivityBarPlot(LPsP_Pre_T2T,"PRE") + ggtitle("LPsP inputs")
print(pLPsP_Pre)
```
...
P19_Post_T2T <- getTypeToTypeTable(P19_Post)
pp19_Post <- connectivityBarPlot(P19_Post_T2T,"POST") + ggtitle("P1-9 outputs")
print(pp19_Post)

P19_Pre <- getConnectionTable(getBodyIdsForList(c("P1-9")),"PRE","PB")
P19_Pre_T2T <- getTypeToTypeTable(P19_Pre)
pp19_Pre <- connectivityBarPlot(P19_Pre_T2T,"PRE") + ggtitle("P1-9 inputs")
print(pp19_Pre)
```

Compare EPG and EPGt connectivity
```{r}
EPG_CT_PB <- getConnectionTable(getBodyIdsForList(c("EPG")),"POST","PB")
conmatPlot_EPG_PB <- plotConnectivityMatrix(EPG_CT_PB,byGroup="id")
conmatPlot_EPG_PB <- structureMatrixPlotByType(conmatPlot_EPG_PB) + ggtitle("EPG and EPGt outputs in the PB")
print(conmatPlot_EPG_PB)

EPG_CT_EB <- getConnectionTable(getBodyIdsForList(c("EPGt")),"PRE","EB")
conmatPlot_EPG_EB <- plotConnectivityMatrix(EPG_CT_EB,byGroup="id")
conmatPlot_EPG_EB <- structureMatrixPlotByType(conmatPlot_EPG_EB) + ggtitle("EPGt inputs in the EB")
print(conmatPlot_EPG_EB)
```

Put everything into one figure
```{r}
pdf("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\PB\\PB_NewNeuronSummary.pdf", width = 10, height = 10)
print(grid.arrange(pIbSpsP,
                   pSpsP,
                   pLPsP_Pre,
                   pLPsP_Post,
                   pp19_Pre,
                   ...
                   conmatPlot_EPG_EB,
                   conmatPlot_EPG_PB,
                   layout_matrix = rbind(c(1,3,5,7),c(2,4,6,7),c(8,8,8,8)
                                         )))
dev.off()
```
