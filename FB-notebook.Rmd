...
conmatPlot <- plotConnectivityMatrix(FBt_To_FBt_Post_T2T,byGroup="type")
print(conmatPlot)

FBt_To_Col_Pre_T2T <- getTypeToTypeTable(FBt_To_Col_Pre)
conmatPlot <- plotConnectivityMatrix(FBt_To_Col_Pre_T2T,byGroup="type")
print(conmatPlot)

FBt_To_Col_Post_T2T <- getTypeToTypeTable(FBt_To_Col_Post)
conmatPlot <- plotConnectivityMatrix(FBt_To_Col_Post_T2T,byGroup="type")
print(conmatPlot)

cmax = max(FBt_To_Col_Pre_T2T$weightRelative)
conmatPlot <- ggplot(FBt_To_Col_Pre_T2T) + 
    theme_classic() + theme(axis.text.x = element_text(angle = 90)) +
    scale_fill_gradient2(low="thistle", mid="blueviolet", high="black", 
                         midpoint =0.5*cmax, limits=c(0,cmax)) +
...
Format the FBt to columnar connections data frame
```{r}
FBt_To_Col_4Clust <- getTypeToTypeTable(FBt_To_Col) %>% select(type.from,type.to,weightRelative)
library(reshape2)

# cluster based on the FBts
FBt_To_Col_4Clust_FBt <- dcast(FBt_To_Col_4Clust,type.from~type.to)
FBt_To_Col_4Clust_FBt[is.na(FBt_To_Col_4Clust_FBt)] <- 0
rownames(FBt_To_Col_4Clust_FBt) <- FBt_To_Col_4Clust_FBt$type.from
FBt_To_Col_4Clust_FBt <- FBt_To_Col_4Clust_FBt %>% select(tail(colnames(FBt_To_Col_4Clust_FBt),ncol(FBt_To_Col_4Clust_FBt)-1))
FBt_To_Col_4Clust_FBt <- scale(FBt_To_Col_4Clust_FBt)

# cluster based on the columnar neurons
FBt_To_Col_4Clust_Col <- dcast(FBt_To_Col_4Clust,type.to~type.from)
FBt_To_Col_4Clust_Col[is.na(FBt_To_Col_4Clust_Col)] <- 0
rownames(FBt_To_Col_4Clust_Col) <- FBt_To_Col_4Clust_Col$type.to
FBt_To_Col_4Clust_Col <- FBt_To_Col_4Clust_Col %>% select(tail(colnames(FBt_To_Col_4Clust_Col),ncol(FBt_To_Col_4Clust_Col)-1))
FBt_To_Col_4Clust_Col <- scale(FBt_To_Col_4Clust_Col)
```

Perform hierarchical clustering
```{r}
# Dissimilarity matrix
d <- dist(FBt_To_Col_4Clust_FBt, method = "euclidean")
# Hierarchical clustering using Complete Linkage
hc1 <- hclust(d, method = "ward" )
# Plot the obtained dendrogram
plot(hc1, cex = 0.6, hang = -1)

clustOrder_FBt <- hc1$labels[hc1$order]

# Dissimilarity matrix
d <- dist(FBt_To_Col_4Clust_Col, method = "euclidean")
# Hierarchical clustering using Complete Linkage
hc1 <- hclust(d, method = "ward" )
# Plot the obtained dendrogram
plot(hc1, cex = 0.6, hang = -1)

clustOrder_Col <- hc1$labels[hc1$order]
```

Plot the clustered groups
```{r}
FBt_To_Col_T2T <- getTypeToTypeTable(FBt_To_Col)
FBt_To_Col_T2T$type.from <- factor(FBt_To_Col_T2T$type.from,levels=clustOrder_FBt)
conmatPlot <- plotConnectivityMatrix(FBt_To_Col_T2T,byGroup="type")
print(conmatPlot)

FBt_To_Col_T2T <- getTypeToTypeTable(FBt_To_Col)
FBt_To_Col_T2T$type.to <- factor(FBt_To_Col_T2T$type.to,levels=clustOrder_Col)
conmatPlot <- plotConnectivityMatrix(FBt_To_Col_T2T,byGroup="type")
print(conmatPlot)
