...
Split the different types in inputs, inner neurons, and outputs
```{r}
...
FBpreNpostDiffSEM <- aggregate(prePostDiff~type,
                     FBpreNpostRatios,
                          FUN = function(x) se = sd(x)/sqrt(length(x)))
colnames(FBpreNpostDiffSEM)[2] <- "prePostDiffSEM"
FBpreRelativeMean <- aggregate(preRelative~type,
                     FBpreNpostRatios,
                          mean)
colnames(FBpreRelativeMean)[2] <- "preRelativeMean"
FBpreRelativeSEM <- aggregate(preRelative~type,
                     FBpreNpostRatios,
                          FUN = function(x) se = sd(x)/sqrt(length(x)))
colnames(FBpreRelativeSEM)[2] <- "preRelativeSEM"
FBpostRelativeMean <- aggregate(postRelative~type,
                     FBpreNpostRatios,
                          mean)
colnames(FBpostRelativeMean)[2] <- "postRelativeMean"
FBpostRelativeSEM <- aggregate(postRelative~type,
                     FBpreNpostRatios,
                          FUN = function(x) se = sd(x)/sqrt(length(x)))
colnames(FBpostRelativeSEM)[2] <- "postRelativeSEM"


FBpreNpost <- Reduce(function(x, y) merge(x, y, all=TRUE), list(FBpreNpostDiffMean, FBpreNpostDiffSEM,FBpreRelativeMean,FBpreRelativeSEM,FBpostRelativeMean,FBpostRelativeSEM))
...
ggplot(FBpreNpost, aes(x=preRelativeMean, y=postRelativeMean)) +
  ...
  geom_errorbar(aes(ymin=postRelativeMean-postRelativeSEM, ymax=postRelativeMean+postRelativeSEM,
                    color=superType),
                width=.02, position=position_dodge(0.05),alpha=0.5) +
  geom_errorbarh(aes(xmin=preRelativeMean-preRelativeSEM, xmax=preRelativeMean+preRelativeSEM,
                     color=superType),
