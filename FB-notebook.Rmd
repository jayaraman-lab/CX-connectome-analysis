...
FBpreNpost$group[which(grepl("FC",FBpreNpost$group))] <- "columnar output"
FBpreNpost$group[which(grepl("FR",FBpreNpost$group))] <- "columnar output"
FBpreNpost$group[which(grepl("FS",FBpreNpost$group))] <- "columnar output"
...
cols <- c("input" = "red", "inner" = "blue", "output" = "darkgreen",
          ...
          "cross layer within a column" = rbow[4],"columnar output" = rbow[3],"SAF" = rbow[2],"other" = rbow[1])
...
ggplot(FBpreNpost, aes(x=preRelativeMean, y=postRelativeMean)) +
  ...
  scale_color_manual(values = cols,breaks=c("input","inner","output",
                                            "tangential","PB-FB columnar","cross columnar within a layer",
                                            "cross layer within a column","columnar output","SAF","other"))
