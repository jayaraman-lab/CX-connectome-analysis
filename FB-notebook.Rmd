...
xyLookupFBROIs <- function(ROIs){
  ...
  ROILookup[which(grepl("(L)",ROIs,fixed=TRUE)),]$x <- -1
  ROILookup[which(grepl("(R)",ROIs,fixed=TRUE)),]$x <- 1
  ...
  ROILookup[which(!grepl("FB",ROIs) &
             grepl("(",ROIs,fixed=TRUE)),]$y <-
    0.4*(rep(1:(0.5*length(which(!grepl("FB",ROIs) &
             grepl("(",ROIs,fixed=TRUE)))),each=2) - 
    (0.25*length(which(!grepl("FB",ROIs) &
             grepl("(",ROIs,fixed=TRUE)))))
...
xyLookupFBROIsIntra <- function(ROIs){
  ...
  ROILookup[which(!grepl("FB",ROIs)),]$x <- 1
  ROILookup[which(grepl("FB",ROIs)),]$x <- 0.5*sin(angs)
...
```{r}
getEdgesROIdf <- function(FBROIInfo,ROIs){
    ...
    for (us in 1:length(ROIsNow_US)){
      ...
      for (ds in 1:length(ROIsNow_DS)){
        if (rowInfo[ROIsNow_US[us]] > synThresh &
          ...
          if (!(ROIDS %in% ROIs))
            next
          edgeNow <- data.frame(to = which(ROIs == ROIUS),
                                from = which(ROIs == ROIDS),
                                weight = as.numeric(rowInfo[ROIsNow_DS[ds]]),
...
ROINames <- sapply(ROIs,function(x){ROISelection[(grepl(paste0(x,"."),ROISelection,fixed=TRUE) &
                                                    (grepl("upstream",ROISelection) | grepl("downstream",ROISelection)))]}) 
...
edges_InNOut <- getEdgesROIdf(FBROIInfo,ROIs_InNOut)

# Pull out the number of types between each ROI
edges_InNOut_numTypes <- edges_InNOut %>% group_by(to,from) %>% summarize(numTypes = length(unique(type)))
...
graph_InNOut <- tbl_graph(nodes_InNOut,
                   filter(edges_InNOut_numTypes,(!(grepl("FB",ROI.from) & grepl("FB",ROI.to)))))

gg_InNOut <-
  ggraph(graph_InNOut,layout="manual",x=nodes_InNOut$x,y=nodes_InNOut$y) + 
  geom_edge_diagonal(aes(width=numTypes,color=ROI.from),alpha=0.5,
                     strength=0.5,
                     arrow = arrow(length = unit(1, "cm")),
                     end_cap = circle(1, 'cm')) + 
  geom_edge_loop(aes(direction=45,span=90,width=numTypes,strength=0.1,color=ROI.from),alpha=0.5) +
...
# Get the table of nodes (types)
ROIs_Intra <- ROIs[which(grepl("FB",ROIs) | (ROIs %in% c("PB","EB","NO")))]
nodes_Intra <- xyLookupFBROIsIntra(ROIs_Intra)
edges_Intra <- getEdgesROIdf(FBROIInfo,ROIs_Intra)

# Pull out the number of types between each ROI
edges_Intra_numTypes <- edges_Intra %>% group_by(to,from) %>% summarize(numTypes = length(unique(type)))
...
pdf("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\FB\\FBROIConnectivity.pdf", width = 30, height = 15)
print(grid.arrange(gg_InNOut,gg_Intra,nrow=1))
dev.off()
