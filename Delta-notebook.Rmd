...
Analyze Delta 12 inputs and outputs by FB layer and R vs. L
```{r}
xThresh = 26000
zThresh = 22650
...
for (tp in 1:length(D12types)){
  # Get the ROI specific info for the Delta 12
  D12Dat <- neuprint_get_synapses(filter(D12ids,type==D12types[tp])$bodyid)
  # Get type info
  ...
  if (tp <= 11 | tp == 13){
    ...
    D12Dat$cluster <-  synClusts$cluster
  } else if (tp == 12){
    synClusts = kmeans(kmeanMat,3, iter.max = 50, nstart = 10)
    D12Dat$cluster <-  synClusts$cluster
  } else {
    synClusts = kmeans(kmeanMat,2, iter.max = 50, nstart = 10)
    D12Dat$cluster <-  synClusts$cluster
    for (clust in 2:1){
      kmeanMat = D12Dat %>% filter(cluster == clust) %>% select(x,y,z)
      synClusts = kmeans(kmeanMat,3, iter.max = 250, nstart = 200)
      clustIDs <- synClusts$cluster
      lowClustInds <- which(clustIDs == as.numeric(which.max(synClusts$centers[,3])))
      clustIDs <- vector(mode="numeric",length=length(clustIDs))+(2 + 2*clust - 2)
      clustIDs[lowClustInds] <- (1 + 2*clust - 2)
      D12Dat[which(D12Dat$cluster == clust),]$cluster <- clustIDs
    }
  }
    
  
  g <- ggplot(D12Dat, aes(x,z, color = as.factor(cluster))) + geom_point() + scale_y_reverse() + ggtitle(D12types[tp])
