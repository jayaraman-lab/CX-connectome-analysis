...
source("C:\\Users\\turnerevansd\\Documents\\FIBSEM\\neuprintR-notebooks\\supertypeUtils.R")
...
Create x and y coordinates for a manual network layout of neurons - tangential cells
```{r}

xyLookupTableTan <- function(){
  superTypes <- c("FB","PFN","Delta0","SAF","Delta6","PFR","PFG","PFL","FC","^FR","FS")
  ...
  for (st in 1:length(superTypes)){
    ...
    if (st == 1){
      ...
      ys <- ys + seq(-2,2,length.out=length(types))
    }
    typeLookup <- data.frame(type = types, x = xs, y = ys)
    xyLookup <- rbind(xyLookup,typeLookup)
  }
...
Get a weight matrix that consists of all elements up to x synapses downstream of a given neuron type
```{r}
# Step through the layers, getting the node names and connection matrix as you go
getConnectionTable_Downstream <- function(seedInput,stopLayer,ROI,synThresh){
  layer = 1
  nodes <- data.frame(name = seedInput)
  while (layer < stopLayer) {
    layer <- layer + 1
    conTab <- getConnectionTable(getBodyIdsForList(nodes$name),"POST",ROI,synThresh = synThresh)
    ...
    newNodes <- data.frame(name = newTypes)
...
graphConTab <- function(conTab,xyLookup){
  ...
  sTScale_edge <- scale_edge_colour_discrete(drop=TRUE,limits = levels(sTs))
  ...
  gg <-
    ggraph(graph,layout="manual",x=nodes$x,y=nodes$y) + 
        geom_edge_fan(aes(width=weightRelative,color=superType),alpha=0.5,
                      strength=1,
        ...
        sTScale_edge +
        sTScale +
        geom_node_text(aes(label=name),angle=40,size=12) +
...
```{r}
nronTypes <- c("PFN","Delta0","Delta6","PFR","PFG","FC","PFL","^FR","FS")
ROI = "FB"
...
gg_All <- graphConTab(conTab_Clean,xyLookup)
...
pdf("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\FB\\NetworkDiagrams\\AllColumnarConnections_Inputs.pdf", width = 40, height = 40)
...
```{r}
gg_Outputs <- list()
gg_Outputs[[1]] <- graphConTab(filter(conTab_Clean, type.to %in% getBodyIdsForList(tail(nronTypes,3))$type),
                         xyLookup)
gg_Outputs[[1]] <- gg_Outputs[[1]] + xlim(-0.75,0.75) + ylim(0.5,3.5) + scale_y_reverse()

OutTypes = getBodyIdsForList(tail(nronTypes,3))$type %>% unique() %>% sort()
for (o in 1:length(OutTypes)){
  gg_Outputs[[o+1]] <- graphConTab(conTab_Clean[which(conTab_Clean$type.to == OutTypes[o]),],
                         xyLookup)
  gg_Outputs[[o+1]] <- gg_Outputs[[o+1]] + xlim(-0.75,0.75) + ylim(0.5,3.5) + scale_y_reverse()
}

pdf("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\FB\\NetworkDiagrams\\AllColumnarConnections_Outputs.pdf", width = 60, height = 30)
print(grid.arrange(grobs = gg_Outputs,layout_matrix = rbind(c(1,13,14,15,2,3),c(4,5,6,7,8,NA),c(9,10,11,12,NA,NA))))
dev.off()
```
...
innerSuperTypes = getBodyIdsForList(nronTypes[2:6])$type %>% unique() %>% sort() %>% supertype() %>% unique()
