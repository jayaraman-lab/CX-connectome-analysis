...
corMatSum <- function(cormat){
  ...
  if (length(which(grepl("Delta12",cormat$neuron1)))>0)
    cormat[which(grepl("Delta12",cormat$neuron1)),]$type1 <- gsub("0","12",unique(cormat$type1))
  if (length(which(grepl("Delta12",cormat$neuron2)))>0)
    cormat[which(grepl("Delta12",cormat$neuron2)),]$type2 <- gsub("0","12",unique(cormat$type2))
...
quadSum <- function(synDat,prepost,D12bodyids){
  ...
  if (prepost){
    ...
    if (length(D12bodyids)>0){
      DeltaInputs[which(DeltaInputs$to %in% D12bodyids),]$type.to <- gsub("0","12",D0type)
      DeltaInputs[which(DeltaInputs$to %in% D12bodyids),]$name.to <- gsub("0","12",D0type)
    }
    ...
    if (length(D12bodyids)>0){
      DeltaOutputs[which(DeltaOutputs$from %in% D12bodyids),]$type.from <- gsub("0","12",D0type)
      DeltaOutputs[which(DeltaOutputs$from %in% D12bodyids),]$name.from <- gsub("0","12",D0type)
    }
...
for (tp in 1:length(D0types)){
  ...
  synSum[which(synSum$bodyid %in% D12bodyids),]$subtype <- gsub("0","12",synSum[which(synSum$bodyid %in% D12bodyids),]$subtype)
