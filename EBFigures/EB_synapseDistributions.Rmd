...
This notebook shows how to generate normalized synapse densitites and related plots for various types in the EB using three types of projections.The specific plots and corresponding figures are listed below.
...
styleDensity2Dhist <- function(synDens, ncols, Cmax, roioutline){
  synDens <- synDens +
    scale_fill_gradientn(colours = brewer.pal(5,"BuPu"), breaks=c(0,Cmax), limits=c(0, Cmax), oob = scales::squish) +
    facet_wrap(~type, ncol=ncols) + coord_fixed(ratio = 1) + theme_void() + guides(color=FALSE) +
...
synprojFront <- styleDensity2Dhist(synprojFront, ncolumns_xy, Cmax,outline_xy)
print(synprojFront)

# side view (zy)
Cmax = 160
synprojSide <- ggplot() + geom_hex(data=dataFilt, aes(x=z, y=y), bins=nbins) 
synprojSide <-  styleDensity2Dhist(synprojSide, ncolumns_zy, Cmax,outline_zy)
print(synprojSide)

# slice view (radial-axial)
Cmax = 300
synprojSlice <- ggplot() + geom_hex(data=dataFilt, aes(x=z, y=syn_dist), bins=nbins/2)
synprojSlice <- styleDensity2Dhist(synprojSlice, ncolumns_slice, Cmax,outline_slice)
print(synprojSlice)
...
ggsave( paste("synapseDistributions_",saveName,'_slice_levels_byRegion.pdf', sep=''), plot = synprojSlice, device='pdf', path = "plots",
...
ggplot(dataFilt %>% filter(type == "ExR1"), aes(x=x,y=y, color=angleGroup)) + geom_point()  + theme_classic()
ggsave( paste("synapseDistributions_",saveName,'_splitRad.pdf', sep=''), plot = synprojSlice, device='pdf', path = "plots",
...
ggsave( paste("synapseDistributions_",saveName,'_slice_simple_splitRad.pdf', sep=''), plot = synprojSlice, device='pdf', path = "plots",
    scale = 1.5, width = 13, height = 6, units ="cm", dpi = 600, limitsize = TRUE)
```
