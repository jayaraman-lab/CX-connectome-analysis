...
nronDat = nlapply(nronMainROI, function(nron){
  distToRoot = distances(as.ngraph(nron, weights = T), v = as.character(nron$d$PointNo[rootpoints(nron)]), to = as.character(unique(nron$d$PointNo)), mode = 'out')
  distToRoot = distToRoot[match(nron$d$PointNo, unique(nron$d$PointNo))]
  distToRoot = distToRoot/125 # converts distance to microns
  cbind(nron$d, distToRoot)
})
...
nronDatAll = data.frame(PointNo=double(),Label=double(),X=double(),Y=double(),Z=double(),W=double(),Parent=double())
for(nron in nronDat){
  nronDatAll = rbind(nronDatAll,nron)
}
...
```{r}
nsamp = 10000
inds = sample(1:dim(nronDatAll)[1], nsamp)

ggplot(nronDatAll[inds,]) +
  geom_point(aes(x = distToRoot, y = W/125))
...
nronMainROI_synapses = mapply(function(nron,syn){
  g = as.ngraph(nron,weights = T)
  diam = vertex_attr(g, 'diam', index = as.character(unique(syn$treenode_id)))
  diam = diam[match(syn$treenode_id, unique(syn$treenode_id))]
  diam = diam/125
  syn = cbind(syn, diam)
}, nron = nronMainROI, syn = nronMainROI_synapses, SIMPLIFY = F)
```
...
Do distance to rootpoint and width of arbor correlate?
```{r}
nsamp = 5000
inds = sample(1:dim(synapsesPre)[1], nsamp)

ggplot(synapsesPre[inds,]) +
  geom_point(aes(x = distToRoot, y = diam))

ggplot(filter(synapsesPre, bodyid == synapses$bodyid[10])) +
  geom_point(aes(x = distToRoot, y = diam))

```


Do distance to rootpoint and electrotonic distance correlate?
```{r}

elecVsDist = ggplot(filter(synapsesPre, modality %in% c('cv','iv','motor','ms','sleep'))) +
  geom_point(aes(x = distToRoot, y = electroDist, color = modality),alpha = 0.3) + 
...
#ggsave(paste('elecVsDist_eg', nronType, '.pdf', sep = ""), plot = elecVsDist, device='pdf', path = "../neuprintR_analysis_plots/")
...
modalityCDFPlot = ggplot(filter(synapsesPre, bodyid == nron_info_kept$bodyid[24], modality %in% c('cv','iv','motor','ms','sleep')), aes(x = normEDist, color = modality)) +
    ...
    scale_colour_manual(values = modalityCMap) +
    theme_paper() +
    labs(x = 'normalized electrotonic distance', y = 'CDF') + 
  facet_wrap(~bodyid)
} else {
...
```{r}
medOfDist_nronType = filter(synapses, prepost == 1, modality %in% c('cv','iv','motor','ms','sleep')) %>% 
...
```{r}
if(!ignore){
medOfDist_nronType = medOfDist_nronType %>% mutate(eg = case_when(
    bodyid == nron_info_kept$bodyid[2] ~ 1,
    TRUE ~ 0
  ))
}
...
medViolinPlot = ggplot(medOfDist) +
  geom_violin(aes(x = factor(modality, levels = c('motor','ms','iv','pol','cv','sleep')), y = med, fill = modality), color = '#5A5156') +
  theme_paper() +
...
medHivePlot = ggplot(medOfDist) +
  ...
  guides(fill = 'legend') +
  scale_fill_manual(values = modalityCMap) +
  scale_color_manual(values = c('black','lightgray',modalityCMap)) +
  labs(x = 'modality', y = 'median') #+
  #ylim(0.25,0.95)
...
```{r}
medHivePlot = ggplot(filter(medOfDist_nronType, partnerType %in% motor)) +
...
#ggsave(paste('medianDists_hive_jitter_', nronType, '_motor.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "../neuprintR_analysis_plots/")

medHivePlot = ggplot(filter(medOfDist_nronType, partnerType %in% ms)) +
  #geom_line(aes(x = factor(partnerType), y = med,  group = factor(bodyid), color = factor(eg)), position = position_dodge(0.5), size = 0.5) +
  geom_line(aes(x = factor(partnerType), y = med,  group = factor(bodyid)),color = 'lightgray', position = position_dodge(0.5), size = 0.5) +
  geom_point(aes(x = factor(partnerType), y = med, fill = partnerType, group = factor(bodyid)), color = 'black', pch = 21, position = position_dodge(0.5), size = 3, alpha = 0.75) +
  theme_paper() +
...
#ggsave(paste('medianDists_hive_jitter_', nronType, '_ms.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "../neuprintR_analysis_plots/")

medHivePlot = ggplot(filter(medOfDist_nronType, partnerType %in% iv)) +
...
#ggsave(paste('medianDists_hive_jitter_', nronType, '_iv.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "../neuprintR_analysis_plots/")

medHivePlot = ggplot(filter(medOfDist_nronType, partnerType %in% cv)) +
  #geom_line(aes(x = factor(partnerType, levels = c('ER3p_a','ERp_b','ER3m','ER3d_a','ER3d_b','ER3d_c','ER3d_d','ER3a_a','ER3a_b','ER3a_c','ER3a_d')), y = med,  group = factor(bodyid), color = factor(eg)), position = position_dodge(0.5), size = 0.5) +
  geom_line(aes(x = factor(partnerType, levels = c('ER3p_a','ER3p_b','ER3m','ER3d_a','ER3d_b','ER3d_c','ER3d_d','ER3a_a','ER3a_b','ER3a_c','ER3a_d')), y = med,  group = factor(bodyid)), color = 'lightgray',position = position_dodge(0.5), size = 0.5) +
  geom_point(aes(x = factor(partnerType, levels = c('ER3p_a','ER3p_b','ER3m','ER3d_a','ER3d_b','ER3d_c','ER3d_d','ER3a_a','ER3a_b','ER3a_c','ER3a_d')), y = med, fill = partnerType, group = factor(bodyid)), color = 'black', pch = 21, position = position_dodge(0.5), size = 3, alpha = 0.75) +
  theme_paper() +
  guides(fill = 'legend') +
 # scale_fill_manual(values = c('#9D49F5','#D3A4F7')) +
  scale_color_manual(values = c('lightgray','black')) +
...
#ggsave(paste('medianDists_hive_jitter_', nronType, '_cv.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "../neuprintR_analysis_plots/")
...
distBetween = distBetween %>% mutate(flag = case_when(
  ...
  betweenWho == '4-5' & orderingType == 4 ~ 1,
  TRUE ~ 0
))
}
