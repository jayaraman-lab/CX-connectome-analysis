...
nronInfo$glom <- lapply(nronInfo$id, function(id) strsplit(strsplit(as.character(id),'_')[[1]][2],'-')[[1]][1]) %>% unlist()
...
```
```{r}
numerosityPlt <- ggplot(nronsPerGlom) + geom_bar(aes(x=glom,y=numPerGlom),stat="identity",color='white') + facet_grid(rows = vars(type)) + theme_paper() + 
  xlab('glomerulus') + ylab('number of neurons per glomerulus') + theme(axis.text.x = element_text(angle = 90)) + labs(subtitle = "A")
print(numerosityPlt)
```
