...
#pape_theme_hgrid <- theme_minimal_hgrid(font_size=7,font_family="Arial",rel_small=6/7,rel_tiny = 5/7,rel_large = 8/7)
#pape_theme_grid <- theme_minimal_grid(font_size=7,font_family="Arial",rel_small=6/7,rel_tiny = 5/7,rel_large = 8/7)
#pape_theme_vgrid <- theme_minimal_hgrid(font_size=7,font_family="Arial",rel_small=6/7,rel_tiny = 5/7,rel_large = 8/7)
#pape_theme_map <- theme_map(font_size=7,font_family="Arial",rel_small=6/7,rel_tiny = 5/7,rel_large = 8/7)
```
...
PBCompIn <- ggplot(filter(PB3Comp,databaseType=="PFNa"),aes(x=L3,y=R3)) + geom_point(aes(color=supertype.from2)) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + pape_theme  + scale_color_CX_supertype(name="Partner\nsupertype") + xlab("Relative input weight in L3") + coord_fixed() +
...
PBCompOut <- ggplot(filter(PB3Comp,databaseType=="EPG" & side=="Outputs"),aes(x=R4,y=L4)) + geom_point(aes(color=supertype.to2)) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + pape_theme  + scale_color_CX_supertype(name="Supertype") + xlab("Relative output weight in R4") + coord_fixed() +
...
```{r}
PBSynapseCountDiff <- ggplot(distinct(PB3CompLong,databaseType,side,glomerulus,.keep_all = TRUE),aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2,shape=comp)) + geom_point() + scale_color_CX_supertype(name="Supertype") + pape_theme_grid + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)")+theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) + ggforce::geom_mark_circle(aes(filter = (databaseType == "EPG" & glomerulus==4 & side=="Outputs")),color="black",expand=0.03) + ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==3 & side=="Inputs")),color="black",expand=0.03) + scale_shape(name="Comparison")

PBSynapseCountDiffAlt <- ggplot(distinct(PB3CompLong,databaseType,side,glomerulus,.keep_all = TRUE),aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2,shape=comp)) + geom_point() + scale_color_CX_supertype(name="Supertype") + pape_theme + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)")+theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +  ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==3 & side=="Inputs")),color="black",expand=0.03) + scale_shape(name="Comparison") + geom_hline(yintercept = 0,lty=2)
PBSynapseCountDiffAlt
```


```{r}
PBSlopes <- ggplot(PBCompFits,aes(x=databaseType,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2,shape=comp)) + geom_point() + geom_linerange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + pape_theme_grid +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate") + scale_shape(name="Comparison")+ ggforce::geom_mark_circle(aes(filter = (databaseType == "EPG" & glomerulus==4 & side=="Outputs")),color="black",alpha=1,expand=0.03) + ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==3 & side=="Inputs")),color="black",alpha=1,expand = 0.03)

PBSlopesAlt <- ggplot(PBCompFits,aes(x=databaseType,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2,shape=comp)) + geom_point() + geom_linerange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + pape_theme +theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate") + scale_shape(name="Comparison") + ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==3 & side=="Inputs")),color="black",alpha=1,expand = 0.03) + geom_hline(yintercept = 1,lty=2)
PBSlopesAlt
```
...
```{r}
PBFrame3 <- ggdraw() + draw_image(paste0(validationFolder,"frontPB3.png"),scale=1.4,vjust=0.05,clip="on") + pape_theme_map
...
```
```{r}
PBFrame4 <- ggdraw() + draw_image(paste0(validationFolder,"frontPB4.png"),scale=1.8,clip="on") + pape_theme_map
...
```
```{r}
test <- ((PBFrame3 / (PBCompIn +guides(color=FALSE))) | (PBFrame4 / (PBCompOut + guides(color=FALSE)))) | ((PBSynapseCountDiff + theme(legend.position="none")) / 
                                                                                            (PBSlopes + guides(color=FALSE)))
save_plot(paste0(validationFolder,"test.pdf"),test,nrow=1,ncol=2,device=cairo_pdf)
```
...
PBFig <- (PBFrame3 + (PBCompIn+guides(color=FALSE)) + 
                       PBFrame4 + PBCompOut + (PBSynapseCountDiff + theme(legend.position="none")) + 
         (PBSlopes + guides(color=FALSE))) + plot_layout(design=layout,guides="keep") + plot_annotation(tag_levels="i")
PBFig

save_plot(paste0(validationFolder,"ValidationPanel2.pdf"),PBFig,nrow=1,ncol=2,device=cairo_pdf,base_width = 8.5/2)
```
...
PBFigAlt <- (PBFrame3 + (PBCompIn+guides(color=FALSE)) + 
                         (PBSynapseCountDiffAlt + theme(legend.position="none")) + 
         (PBSlopesAlt + guides(color=FALSE)))  + plot_layout(design=pbLayout,guides="keep") + plot_annotation(tag_levels="i") 
PBFigAlt

save_plot(paste0(validationFolder,"ValidationPanel2Alt.pdf"),PBFigAlt,nrow=1.3,ncol=2,device=cairo_pdf,base_width = 8.5/2)
```
...
PBCompControlIn <- ggplot(filter(PB3CompControl,databaseType=="PFNa"),aes(x=L5,y=R5)) + geom_point(aes(color=supertype.from2)) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + pape_theme  + scale_color_CX_supertype(name="Supertype") + xlab("Relative input weight in L5") + coord_fixed() +
  ylab("Relative input weight in R5") + annotate(geom="text",x=0.3,y=0.08,label=paste0("s= ",format(fitValsPFNa5$estimate,digits = 3),"\n r\u00b2= ",format(fitValsPFNa5$adj.r.squared,digits=3)))+ annotate(geom="text",x=0.35,y=0.65,label="PFNa inputs in glomeruli 5")
...
PBCompControlOut <- ggplot(filter(PB3CompControl,databaseType=="EPG" & side=="Outputs"),aes(x=R6,y=L6)) + geom_point(aes(color=supertype.to2)) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + pape_theme + scale_color_CX_supertype(name="Supertype") + xlab("Relative output weight in R6") + coord_fixed()+
  ylab("Relative output weight in L6") + annotate(geom="text",x=0.3,y=0.08,label=paste0("s= ",format(fitValsEPG6$estimate,digits = 3),"\n r\u00b2= ",format(fitValsEPG6$adj.r.squared,digits=3)))+ annotate(geom="text",x=0.18,y=0.42,label="EPG outputs in glomeruli 6")
...
```{r}
PBSynapseCountDiffControl <- ggplot(distinct(PB3CompControlLong,databaseType,side,glomerulus,.keep_all = TRUE),aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2,shape=comp)) + geom_point() +  scale_color_CX_supertype(name="Type") + pape_theme + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)")+theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5))  + scale_shape(name="Comparison")+ ggforce::geom_mark_circle(aes(filter = (databaseType == "EPG" & glomerulus==6 & side=="Outputs")),color="black",expand=0.03) + ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==5 & side=="Inputs")),color="black",expand=0.03)
PBSynapseCountDiffControl
```

```{r}
PBSlopesControl <- ggplot(PBCompControlFits,aes(x=databaseType,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2,shape=comp)) + geom_point() + geom_linerange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + pape_theme +theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate") + scale_shape(name="Comparison")+ ggforce::geom_mark_circle(aes(filter = (databaseType == "EPG" & glomerulus==6 & side=="Outputs")),color="black",alpha=1,expand=0.03) + ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==5 & side=="Inputs")),color="black",alpha=1,expand = 0.03)+geom_hline(yintercept=1,lty=2)
PBSlopesControl
```

```{r}
PBCompAllFits <- rbind(mutate(PBCompFits,type="Heavy tracing comparison (3/4)"),mutate(PBCompControlFits,type="Control (5/6)"))
PBCompSlopesComp <- ggplot(PBCompAllFits,aes(estimate,fill=type)) + geom_histogram(alpha=0.5,aes(y=..density..),position="identity")  + paletteer::scale_fill_paletteer_d("Polychrome::dark",name="Comparison") + pape_theme_grid + xlab("Slope estimate")
PBCompSlopesComp
```
```{r}
layout ="
...
PBControlFig <- 
         ...
         PBCompSlopesComp) + plot_layout(design=layout,guides="keep") + plot_annotation(tag_levels="A")
PBControlFig

save_plot(paste0(validationFolder,"ValidationPanel2SI.pdf"),PBControlFig,nrow=2,ncol=1.5,base_width = 8.5/1.5,device=cairo_pdf)
```
...
```{r}
synapseDiffC3 <- ggplot(distinct(FBC3Comp,databaseType,side,.keep_all = TRUE),aes(x=stringr::str_replace(databaseType,"Delta","\u0394"),y=known_synapses_ratio*100,color=supertype2)) + geom_point() + scale_color_CX_supertype(name="Supertype") + pape_theme + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)")+theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5))+ ggforce::geom_mark_circle(aes(filter = (databaseType == "hDeltaA" & side=="Outputs")),color="black",expand=0.03) + geom_hline(yintercept=0,lty=2)
synapseDiffC3
```
...
```{r}
FC3Slopes <- ggplot(FBCompFits,aes(x=stringr::str_replace(databaseType,"Delta","\u0394"),y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2)) + geom_point() + geom_linerange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + pape_theme +theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate")+ ggforce::geom_mark_circle(aes(filter = (databaseType == "hDeltaA" & side=="Outputs")),color="black",expand=0.03) + geom_hline(lty=2,yintercept = 1)
FC3Slopes
```

```{r}
FBComp <- ggplot(filter(FBC3Comp,databaseType=="hDeltaA" & side=="Outputs"),aes(x=CX,y=C3)) + geom_point(aes(color=supertype.to2))  + geom_smooth(formula=y~x,method="lm",color="grey") + coord_fixed()+
  geom_abline(linetype=2) + pape_theme  + scale_color_CX_supertype(name="Supertype") + ylab("Column 3 weight") + xlab("Rest of the FB weight") 
FBComp
```
...
```{r}
FBC3Frame <- ggdraw() + draw_image(paste0(validationFolder,"frontC3.png"),scale=1.55,vjust=0,clip="on") + pape_theme_map
...
FC3Panel <- (FBC3Frame + (FBComp + theme(legend.position = "none")) + 
  (synapseDiffC3 + theme(legend.position = "none")) +
  FC3Slopes) + plot_layout(design = fcLayout) + plot_annotation(tag_levels=c("i"))
FC3Panel
```

```{r}
save_plot(paste0(validationFolder,"ValidationPanel3.pdf"),FC3Panel,nrow = 1.3,ncol=2,base_width = 8.5/2,device=cairo_pdf)
```

```{r}
combo1 <- (PBFigAlt + plot_layout(tag_level = "new"))/(FC3Panel + plot_layout(tag_level="new")) + plot_annotation(tag_levels=c("A","i"),tag_sep = " ") & theme(strip.background = element_blank())
combo1

save_plot(paste0(validationFolder,"ValidationPanel2and3.pdf"),combo1,nrow = 2.6,ncol=2,base_width = 8.5/2,device=cairo_pdf)
```
...
exampleEPGNew <- getNeuronMesh(exampleEPG,cloud=TRUE,node="2b4ff")
...
```{r}
EPGViews <- ggdraw() + draw_image(paste0(validationFolder,"frontEBZoom1.png"),scale=1.6,clip="on") + draw_image(paste0(validationFolder,"frontEBZoom2.png"),scale=0.5,vjust=-0.23,hjust=0.05,clip="on") +
...
synChangeEB <- ggplot(EBAges_summary,aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2)) + geom_jitter(width=0.2) + 
     scale_color_CX_supertype(name="Supertype") + theme_minimal_hgrid() + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)") + theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),panel.border = element_rect(color = "gray 50", fill = NA)) + ylim(c(0,120))+ggforce::geom_mark_circle(aes(filter = (bodyid == exampleEPG & side=="Inputs")),color="black",expand=0.03)
synChangeEB
```

```{r}
synChangeEBTot <- ggplot(distinct(EBAges,bodyid,side,.keep_all = TRUE),aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2)) + geom_jitter(width=0.2) + scale_color_CX_supertype(name="Type") + theme_minimal_hgrid() + facet_grid(.~side,scales="free_x")+ xlab("") + ylab("Known synapses change (%)") + theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5),panel.border = element_rect(color = "gray 50", fill = NA)) + geom_hline(yintercept = 10,linetype=2,color="grey") + ggforce::geom_mark_circle(aes(filter = (bodyid == exampleEPG & side =="Inputs")),color="black",expand=0.03)
synChangeEBTot
```
```{r}
...
```{r}
EBSlopes <- ggplot(filter(EBFits,paste(bodyid,side) %in% paste(EBAges_summary$bodyid,EBAges_summary$side)),aes(x=databaseType,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2)) + geom_pointrange(position=position_jitter(width=0.2)) + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_hgrid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate") + ggforce::geom_mark_circle(aes(filter = (bodyid == exampleEPG & side=="Inputs")),color="black",expand=0.03)
EBSlopes
```

```{r}
fitValsEPG <- filter(EBFits,bodyid==exampleEPG & side=="Inputs")
EBCompExample <- ggplot(filter(EBAges_filtered,bodyid==exampleEPG & side=="Inputs"),aes(x=old,y=new)) + geom_point(aes(color=supertype.from2),size=3) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + theme_minimal_hgrid()  + scale_color_CX_supertype(name="Supertype") + xlab("Relative input weight in the old dataset") + 
  ylab("Relative input weight in the new dataset") + annotate(geom="text",x=0.015,y=0.002,label=paste0("s= ",format(fitValsEPG$estimate,digits = 3),"\n r\u00b2= ",format(fitValsEPG$adj.r.squared,digits=3)))
...
EBFig <- ((EPGViews | (EBCompExample + theme(legend.position="none"))) + 
            plot_layout(tag_level = "new",widths = c(1.8,1))) / 
  (synChangeEB ) / 
  (EBSlopes+ theme(legend.position="none")) +plot_layout(design = layout) + plot_annotation(tag_levels=c("A","i"))

save_plot(paste0(validationFolder,"ValidationPanel1.pdf"),EBFig,nrow=4,ncol=2,device=cairo_pdf)
```
...
EBCompFull <- ggplot(filter(EBAges_filtered),aes(x=old,y=new,fill=supertype2)) + geom_smooth(formula=y~x,aes(group=bodyid),method="lm",alpha=0.3,color="grey30",size=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + scale_fill_CX_supertype(name="Supertype") + xlab("Relative weight in the old dataset") + 
  ylab("Relative weight in the new dataset") + facet_wrap(supertype2~side,scales="free")+theme_minimal_hgrid()+theme(panel.border = element_rect(color = "gray 50", fill = NA))
#EBCompFull
save_plot(paste0(validationFolder,"ValidationPanel1-SI1.pdf"),EBCompFull,nrow = 3,ncol=3)
```

```{r}
incSlope <-  ggplot(filter(EBFits,paste(bodyid,side) %in% paste(EBAges_summary$bodyid,EBAges_summary$side)),aes(x=known_synapses_ratio*100,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2)) + geom_pointrange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_hgrid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5))  +xlab("Known synapse increase") +ylab("Slope estimate")
incSlope
```
```{r}
incRS <-  ggplot(filter(EBFits,paste(bodyid,side) %in% paste(EBAges_summary$bodyid,EBAges_summary$side)),aes(x=known_synapses_ratio*100,y=adj.r.squared,color=supertype2)) + geom_point() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_hgrid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("Known synapse increase") +ylab("Adjusted r\u00b2")
incRS
```


```{r}
nSlope <-  ggplot(filter(EBFits,paste(bodyid,side) %in% paste(EBAges_summary$bodyid,EBAges_summary$side)),aes(x=known_synapses.new,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2)) + geom_pointrange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_hgrid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("# synapses") +ylab("Slope estimate") 
nSlope
```
```{r}
nRS <-  ggplot(filter(EBFits,paste(bodyid,side) %in% paste(EBAges_summary$bodyid,EBAges_summary$side)),aes(x=known_synapses.new,y=adj.r.squared,color=supertype2)) + geom_point() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_hgrid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("# synapses") +ylab("Adjusted r\u00b2") 
nRS
```

```{r}
EBSI2 <- (incSlope + theme(legend.position="none",plot.margin = margin(b=100,r=50))+ggtitle("Slope estimates")| incRS+ theme(legend.position="none",plot.margin = margin(b=100))+ggtitle("Adjusted r\u00b2"))/
  (nSlope + theme(legend.position="none",plot.margin = margin(r=50))| nRS) + plot_layout(guides="collect")
EBSI2
```
```{r}
save_plot(paste0(validationFolder,"ValidationPanel1-SI2.pdf"),EBSI2,nrow=2,ncol=2,device=cairo_pdf)
```



```{r}
 ggplot(filter(EBFits,paste(bodyid,side) %in% paste(EBAges_summary$bodyid,EBAges_summary$side)),aes(x=estimate,y=std.error,color=supertype2)) + geom_point() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_hgrid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("Slope estimate") +ylab("Std Error on the estimate") 
```
