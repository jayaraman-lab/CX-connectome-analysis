...
synChangeEB <- ggplot(EBAges_summary,aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2)) + geom_jitter(width=0.2) + 
     scale_color_CX_supertype(name="Supertype") + pape_theme + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)") + theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5)) + ylim(c(0,120))+ggforce::geom_mark_circle(aes(filter = (bodyid == exampleEPG & side=="Inputs")),color="black",expand=0.03)
synChangeEB
```

```{r}
synChangeEBTot <- ggplot(distinct(EBAges,bodyid,side,.keep_all = TRUE),aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2)) + geom_jitter(width=0.2) + scale_color_CX_supertype(name="Type") + pape_theme + facet_grid(.~side,scales="free_x")+ xlab("") + ylab("Known synapses change (%)") + theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.5)) + geom_hline(yintercept = 10,linetype=2,color="grey") + ggforce::geom_mark_circle(aes(filter = (bodyid == exampleEPG & side =="Inputs")),color="black",expand=0.03)
synChangeEBTot
```
```{r}
...
```{r}
EBSlopes <- ggplot(filter(EBFits,paste(bodyid,side) %in% paste(EBAges_summary$bodyid,EBAges_summary$side)),aes(x=databaseType,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2)) + geom_pointrange(position=position_jitter(width=0.2)) + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + pape_theme +theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate") + ggforce::geom_mark_circle(aes(filter = (bodyid == exampleEPG & side=="Inputs")),color="black",expand=0.03) + geom_hline(yintercept = 1,lty=2)
EBSlopes
```

```{r}
fitValsEPG <- filter(EBFits,bodyid==exampleEPG & side=="Inputs")
EBCompExample <- ggplot(filter(EBAges_filtered,bodyid==exampleEPG & side=="Inputs"),aes(x=old,y=new)) + geom_point(aes(color=supertype.from2)) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + pape_theme  + scale_color_CX_supertype(name="Supertype") + xlab("Input weight in the old dataset") + coord_fixed() +
  ylab("Input weight in the new dataset") + annotate(geom="text",x=0.015,y=0.002,label=paste0("s= ",format(fitValsEPG$estimate,digits = 3),"\n r\u00b2= ",format(fitValsEPG$adj.r.squared,digits=3)))
...
```{r}
mainFig <- (EBFig + plot_layout(tag_level = "new")) / (PBFigAlt + plot_layout(tag_level = "new"))/(FC3Panel + plot_layout(tag_level="new")) + plot_annotation(tag_levels=c("A","i"),tag_sep = "") 
mainFig

save_plot(paste0(validationFolder,"ValidationMainFig.pdf"),mainFig,nrow = 3.9,ncol=2,base_width = 8.5/2,device=cairo_pdf)
```
...
```
```{r}
save_plot(paste0(validationFolder,"ValidationPanel1-SI2.pdf"),EBSI2,nrow=2,ncol=2,device=cairo_pdf,base_width = 8.5/2)
```
