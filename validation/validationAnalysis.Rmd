...
PBControlFig <- 
         ...
         PBCompSlopesComp + plot_spacer()) + plot_layout(design=layout,guides="keep") + plot_annotation(tag_levels="A",title= "Validation Figure 1 - SI3: comparing PB glomeruli at equivalent tracing levels",theme=theme(title=element_text(size=12,face="bold")))
PBControlFig 

save_plot(paste0(validationFolder,"validation-Figure1-SI3.pdf"),PBControlFig,nrow=1,ncol=1.5,base_width = 8.5/1.5,base_height=11,device=cairo_pdf)
```
...
EBCompFull <- ggplot(filter(EBAges_filtered),aes(x=old,y=new,fill=supertype2)) + geom_smooth(formula=y~x,aes(group=bodyid),method="lm",alpha=0.3,color="grey30",size=0.1) + 
  ...
  ylab("Relative weight in the new dataset") + facet_wrap(supertype2~side)+ pape_theme + ggtitle("Validation figure 1 - figure supplement 1: individual neuron to neuron fits")#+theme(panel.border = element_rect(color = "gray 50", fill = NA))
#EBCompFull
...
EBSI2 <- (incSlope + theme(legend.position="none",plot.margin = margin(b=50,r=50))+ggtitle("Slope estimates")| incRS+ theme(legend.position="none",plot.margin = margin(b=50))+ggtitle("Adjusted r\u00b2"))/
  (nSlope + theme(legend.position="none",plot.margin = margin(r=50))| nRS) + plot_layout(guides="collect") + plot_annotation(title="Validation Figure 1 - figure supplement 2",theme=theme(title=element_text(size=12,face="bold")))
EBSI2
```
```{r}
save_plot(paste0(validationFolder,"Validation-Figure1-SI2.pdf"),EBSI2,nrow=2,ncol=2,device=cairo_pdf,base_width = 8.5/2)
```
