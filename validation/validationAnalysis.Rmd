...
PBCompIn <- ggplot(filter(PB3Comp,databaseType=="PFNa"),aes(x=L3,y=R3)) + geom_point(aes(color=supertype2.from)) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
...
EBCompExample <- ggplot(filter(EBAges_filtered,bodyid==exampleEPG & side=="Inputs"),aes(x=old,y=new)) + geom_point(aes(color=supertype2.from)) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
