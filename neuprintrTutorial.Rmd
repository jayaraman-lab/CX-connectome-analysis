...
If it's been a while since you installed natverse, I'd recommend running ```remotes::update_packages("natverse")``` to update it and all dependencies.
...
```{r}
PFLaNames <- neuprint_search("PFL1.*")
head(PFLaNames)
```
...
```{r}
lalPostConnections  <-  neuprint_connection_table(PFLaNames[["bodyid"]],"POST","LAL(R)")
...
PFLbNames <-  neuprint_search("PFL2.*")
```
...
```{r}
PFLDelta6Connections <-  neuprint_connection_table(c(PFLbNames[["bodyid"]],Delta6aNames[["bodyid"]]),"PRE","FB")
```
...
If we want that as a matrix, we can play around with some dplyr functions, then transform it to a matrix
```{r}
PFLDelta6ConnectionsWide <-  pivot_wider(PFLDelta6Connections,id_cols=name,names_from = partnerName,values_from = weightRelative)
PFLDelta6Matrix <- data.matrix(PFLDelta6ConnectionsWide[,2:ncol(PFLDelta6ConnectionsWide)])
rownames(PFLDelta6Matrix) <- PFLDelta6ConnectionsWide$name
```

PFLDelta6Matrix is now a simple matrix you can plot using base functions.

```{r}
PFLDelta6Matrix[is.na(PFLDelta6Matrix)] <- 0
heatmap(PFLDelta6Matrix)
```
...
```{r}
no1Connect <-  neuprint_find_neurons("NO1(R)",all_segments = FALSE)
no1Table <-  neuprint_connection_table(no1Connect$bodyid,"PRE","NO1(R)")   %>%
                       mutate(partnerName = neuprint_get_meta(partner)[["name"]],name =neuprint_get_meta(bodyid)[["name"]])   %>% 
                       group_by(name) %>% 
                       mutate(weightRelative = weight/sum(weight)) %>%
...
```{r message=FALSE, warning=FALSE}
PFLaRNames <-  neuprint_search("PFL1.*_R.*")
...
```{r}
EPGLNames <-  neuprint_search("EPG.*_L.*")
R4dNames <-  neuprint_search("R4d_a.*_L.*")
```
...
```{r}
EPGtoPFLPath <- neuprint_get_shortest_paths(EPGLNames$bodyid[1],PFLaNames$bodyid[1],weightT=20)

```

If you look at this table it's already clear that to go from EPG_L1 to PFL_L1 (with at least 20 synapses on each step that is) you need to go through 2 Delta7s 

```{r}
head(EPGtoPFLPath,n = 16)
```
...
```{r}
nodeNames <- unique(c(EPGtoPFLPath$from,EPGtoPFLPath$to))
pathNodes <- neuprint_get_meta(nodeNames)
EPGtoPFLPath <- EPGtoPFLPath %>% mutate(from = sapply(from, function(f) which(f == pathNodes$bodyid)),
                        to = sapply(to, function(f) which(f == pathNodes$bodyid)))

```

```{r}
test <- tbl_graph(pathNodes,EPGtoPFLPath)
```

```{r}
ggraph(test,layout="sugiyama") + geom_edge_link(aes(colour=factor(depth),width=weight,start_cap = label_rect(name.from),end_cap = label_rect(name.to)),arrow=arrow(length = unit(4, 'mm'))) +geom_node_point(aes(color=type),size=5)+ geom_node_text(aes(label=name)) +theme_minimal()
```
