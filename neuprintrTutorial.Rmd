...
PFLDelta6Connections = PFLDelta6Connections %>%
                       ...
                       mutate(weightRelative = weight/sum(weight)) %>%
                       ungroup()
## The filter operation select partners so that they're also part of the neuron set (meaning we want connections within our neuron set, not to any neuron). The rest of the operations are the same we used before, adding neuron names and defining the weight as the proportion of inputs in the neuropile
...
```{r}
ggplot(PFLDelta6Connections) + geom_tile(aes(name,partnerName,fill=weightRelative)) + theme(axis.text.x = element_text(angle = 90))
```
...
```
```{r}
EPG_L0_synapses = EPG_L0_synapses %>%
                  mutate(x=as.numeric(x),y=as.numeric(y),z=as.numeric(z),prepost=as.logical(prepost)) ## Somehow those are by default factors

R4d_L0_synapses = R4d_L0_synapses %>%
                  mutate(x=as.numeric(x),y=as.numeric(y),z=as.numeric(z),prepost=as.logical(prepost)) 
```


```{r}
ggplot() + geom_point(data=EPG_L0_synapses,aes(x,z,color= prepost),shape=6) + geom_point(data=R4d_L0_synapses,aes(x,z,color=prepost)) +scale_color_brewer(palette="Set1")+coord_fixed(ratio = 1) + theme_void()
ggplot() + geom_point(data=EPG_L0_synapses,aes(y,z,color= prepost),shape=6) + geom_point(data=R4d_L0_synapses,aes(y,z,color=prepost)) +scale_color_brewer(palette="Set1")+coord_fixed(ratio = 1) + theme_void()
```


```{r}
R4d0ToEPG0synapes = R4d_L0_synapses %>%
                    filter(partner == EPGLNames$bodyid[1]) %>%
                    filter(prepost == 1)
```

```{r}
ggplot() + geom_point(data=EPG_L0_synapses,aes(x,z,color= prepost)) + geom_point(data=R4d0ToEPG0synapes,aes(x,z),color="red") +scale_color_brewer(palette="Greens")+coord_fixed(ratio = 1) + theme_void()
ggplot() + geom_point(data=EPG_L0_synapses,aes(x,y,color= prepost)) + geom_point(data=R4d0ToEPG0synapes,aes(x,y),color="red") +scale_color_brewer(palette="Greens")+coord_fixed(ratio = 1) + theme_void()
```
