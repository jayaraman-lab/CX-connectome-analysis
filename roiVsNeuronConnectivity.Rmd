...
if(!noPost){
  postConnections <- getConnectionTable(sourceIDs$bodyid,synapseType = "POST",slctROI, by.roi = TRUE)
  postTypes = unique(postConnections$type.to)
}

# get pre connections
preConnections <- getConnectionTable(sourceIDs$bodyid,synapseType = "PRE",slctROI, synThresh=3, by.roi = TRUE)
...
plot(myGraph,edge.curved=0.2, layout=l, edge.color = customizeGraphEdges(myGraph)) # use 0.5 curved if edges overlap
