...
```
```{r}
ggplot(my_neurons,aes(x=col,color=type, fill=type)) + geom_bar() + facet_grid(rows = vars(type),  scales="free") +
...
### TODO: Could look at synapse count breackdown per ROI to see if normalization of synapse counts occurs selectively in some compartments.