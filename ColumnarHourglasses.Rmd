...
```{r}
PFRInOutLat <- lateralizeInputOutputList(PFRInOut)
```

```{r}
PFROutputsRob <- makePyramidGraph(PFRInOutLat,c("ROB(R)"),plot=TRUE,polarity = "outputs")
```
...
Seems the left/right distinction is not the correct one for PFR (similar to PFLs?)
```{r}
...
### LAL
(will also require a slightly different subdivision of PFLs)
```{r}
PFLInOutLat <- lateralizeInputOutputList(PFLInOut)
```
