...
saveName = paste0("TB_in_", myROI)
my_types = c("TuBu")# c("ER1","ER2","ER3","ER4","ER5","ER6")# #
...
3D plot
```{r}
partnerSubset = getBodyIdsForList(paste0(c("ER"),".*")) #  "TuBu", "ER"
focustype = "TuBu06" #"ER5"
data = dataFilt %>% filter(partnerType %in% partnerSubset$type) %>% filter(type %in% c(focustype))

mycols = paletteer_d("Polychrome::palette36")[-2]
...
colPal = paletteer_d("colRoz::salt_lake")
  # warm:paletteer_c("grDevices::Heat 2", n=length(unique(toplot_2D$id)))
  #cool: paletteer_d("colRoz::salt_lake")
  #polychrome: paletteer_d("Polychrome::palette36")[-2]

xyproj <- ggplot(toplot_2D) + geom_point(aes(x=x, y=y, color=id, size=1)) + 
  ...
  coord_fixed() + theme_classic() + guides(size=FALSE)
...
zyproj <- ggplot(toplot_slice %>% filter(id %in% idfilt$id) ) + geom_point(aes(x=z, y=y, color=id, size=1)) + 
  geom_point(data=toplot_slice %>% filter(id %in% idfilt$id) %>% group_by(id) %>% summarize_each(mean), aes(x=z,y=y, color=id, size=2)) +
  scale_color_manual(values = colPal, breaks = unique(toplot_2D$id)) + 
  coord_fixed() +theme_classic() + guides(size=FALSE)

projPlot <- grid.arrange(xyproj,zyproj, nrow=1)

ggsave(paste("synapseDistributions_",saveName,'_',focustype,'_projection.pdf', sep=''), plot = projPlot, device='pdf',
        path = "../neuprintR_analysis_plots/", scale = 1, width = 25, height = 8, units ="cm", dpi = 600, useDingbats=FALSE)
```
