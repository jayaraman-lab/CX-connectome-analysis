...
For Ci choose my_types = c("TuBu"), for Cii choose my_types = c("ER1","ER2","ER3","ER4","ER5","ER6") and ajdust saveName appropriately.
```{r}
myROI = "BU(R)"
...
Select "TuBu06" if you have chosen my_types = c("TuBu") or "ER5" if  you have chosen my_types = c("ER")
...
```{r}
partnerSubset = getBodyIdsForList(paste0(c("ER"),".*")) #getBodyIdsForList(paste0(c("TuBu"),".*")) #
...
xyproj <- ggplot(toplot_2D) + geom_point(aes(x=x, y=y, color=id, size=0.25, alpha=0.8,stroke = 0,)) + 
  geom_point(data=toplot_2D %>% group_by(id) %>% summarize_each(mean), aes(x=x, y=y, color=id, size=0.5)) +
...
zyproj <- ggplot(toplot_slice %>% filter(id %in% idfilt$id) ) + geom_point(aes(x=z, y=y, color=id, size=0.25, alpha=0.8,stroke = 0,)) + 
  geom_point(data=toplot_slice %>% filter(id %in% idfilt$id) %>% group_by(id) %>% summarize_each(mean), aes(x=z,y=y, color=id, size=0.5)) +
  scale_color_manual(values = colPal, breaks = unique(toplot_2D$id)) + 
  coord_fixed() + guides(size=FALSE, alpha=FALSE) + theme_paper()
