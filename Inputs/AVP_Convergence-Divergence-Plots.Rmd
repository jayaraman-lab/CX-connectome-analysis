...
myColorMap <- read.csv(paste0("~/Documents/code/neuprintR-notebooks/colormaps/colorMap_",cmapNames[1],".csv"))
if(length(cmapNames) > 1){
  for(cmapName in cmapNames[2:length(cmapNames)]){ 
    tmp = read.csv(paste0("~/Documents/code/neuprintR-notebooks/colormaps/colorMap_",cmapName,".csv"))
...
```{r}
filtdat = conPattern %>% filter(focustypeSyn == "pre" & syndir == selectDir & partnertype == "ER5")
```
