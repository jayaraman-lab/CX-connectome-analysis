...
PBCompIn <- ggplot(filter(PB3Comp,databaseType=="PFNa"),aes(x=L3,y=R3)) + geom_point(aes(color=supertype.from2),size=3) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + theme_minimal_hgrid()  + scale_color_CX_supertype(name="Partner\nsupertype") + xlab("Relative input weight in L3") + 
...
PBCompOut <- ggplot(filter(PB3Comp,databaseType=="EPG" & side=="Outputs"),aes(x=R4,y=L4)) + geom_point(aes(color=supertype.to2),size=3) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + theme_minimal_hgrid()  + scale_color_CX_supertype(name="Supertype") + xlab("Relative output weight in R4") + 
...
R3ExampleAnat <- lapply(R3Example,getNeuronMesh,cloud=TRUE,node="2b4ff")
L3ExampleAnat <- lapply(L3Example,getNeuronMesh,cloud=TRUE,node="2b4ff")
...
R4ExampleAnat <- lapply(R4Example,getNeuronMesh,cloud=TRUE,node="2b4ff")
L4ExampleAnat <- lapply(L4Example,getNeuronMesh,cloud=TRUE,node="2b4ff")
```
...
```{r}
PBFrame3 <- ggdraw() + draw_image(paste0(validationFolder,"frontPB3.png"),scale=1.4,vjust=0.05,clip="on")
...
```
```{r}
PBFrame4 <- ggdraw() + draw_image(paste0(validationFolder,"frontPB4.png"),scale=1.8,clip="on")
...
PBFig <- (((PBFrame3 / 
            (PBCompIn+guides(color=FALSE)+ theme(plot.margin = margin(b=100,r=40))) + 
            plot_layout(tag_level = "new",heights = c(1.8,1))
          ) | 
         (PBFrame4 / 
            (PBCompOut + theme(plot.margin = margin(b=100))) + 
            plot_layout(tag_level = "new",heights=c(1.8,1))
          )) /
         (PBSynapseCountDiff + theme(legend.position="none")) / 
         (PBSlopes + guides(color=FALSE))) + plot_layout(design=layout,guides="keep") + plot_annotation(tag_levels=c("A","i"))
PBFig

save_plot(paste0(validationFolder,"ValidationPanel2.pdf"),PBFig,nrow=5,ncol=2,device=cairo_pdf)
```
...
PBCompControlIn <- ggplot(filter(PB3CompControl,databaseType=="PFNa"),aes(x=L5,y=R5)) + geom_point(aes(color=supertype.from2),size=3) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + theme_minimal_hgrid()  + scale_color_CX_supertype(name="Supertype") + xlab("Relative input weight in L5") + 
...
PBCompControlOut <- ggplot(filter(PB3CompControl,databaseType=="EPG" & side=="Outputs"),aes(x=R6,y=L6)) + geom_point(aes(color=supertype.to2),size=3) + geom_smooth(formula=y~x,method="lm",color="grey70",alpha=0.1) + 
  geom_abline(linetype=2,lwd=0.2) + theme_minimal_hgrid()  + scale_color_CX_supertype(name="Supertype") + xlab("Relative output weight in R6") + 
...
```{r}
PBSynapseCountDiffControl <- ggplot(distinct(PB3CompControlLong,databaseType,side,glomerulus,.keep_all = TRUE),aes(x=databaseType,y=known_synapses_ratio*100,color=supertype2,shape=comp)) + geom_point(size=3) +  scale_color_CX_supertype(name="Type") + theme_minimal_hgrid() + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)")+theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5))  + scale_shape(name="Comparison")+ ggforce::geom_mark_circle(aes(filter = (databaseType == "EPG" & glomerulus==6 & side=="Outputs")),color="black",expand=0.03) + ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==5 & side=="Inputs")),color="black",expand=0.03)
PBSynapseCountDiffControl
```

```{r}
PBSlopesControl <- ggplot(PBCompControlFits,aes(x=databaseType,y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2,shape=comp)) + geom_point(size=3) + geom_linerange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_hgrid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate") + scale_shape(name="Comparison")+ ggforce::geom_mark_circle(aes(filter = (databaseType == "EPG" & glomerulus==6 & side=="Outputs")),color="black",alpha=1,expand=0.03) + ggforce::geom_mark_circle(aes(filter = (databaseType == "PFNa" & glomerulus==5 & side=="Inputs")),color="black",alpha=1,expand = 0.03)
PBSlopesControl
```

```{r}
PBCompAllFits <- rbind(mutate(PBCompFits,type="Heavy tracing comparison (3/4)"),mutate(PBCompControlFits,type="Control (5/6)"))
PBCompSlopesComp <- ggplot(PBCompAllFits,aes(estimate,fill=type)) + geom_histogram(alpha=0.5,aes(y=..density..),position="identity")  + paletteer::scale_fill_paletteer_d("Polychrome::dark",name="Comparison") + theme_minimal_grid() + xlab("Slope estimate")
PBCompSlopesComp
```
```{r}
layout ="
...
PBControlFig <- 
            (((PBCompControlIn+guides(color=FALSE) + theme(plot.margin = margin(b=100,r=40))
             ) |  (PBCompControlOut + theme(plot.margin = margin(b=100)))) /
...
save_plot(paste0(validationFolder,"ValidationPanel2SI.pdf"),PBControlFig,nrow=5,ncol=2,device=cairo_pdf)
```
...
plot3d(epgInputSynapsesOld,add=T,size=15,col="#B32DB5")
plot3d(epgInputSynapsesNew,add=T,col="#348E53",size=14)
nview3d("ventral")
...
rgl.snapshot(paste0(validationFolder,"frontEBZoom3.png"))
```

```{r}
EPGViews <- ggdraw() + draw_image(paste0(validationFolder,"frontEBZoom1.png")) + draw_image(paste0(validationFolder,"frontEBZoom3.png"),scale=0.5,vjust=-0.25,hjust=0.15) 
EPGViews
```
...
EBFig <- ((EPGViews | (EBCompExample + theme(legend.position="none"))) + 
  ...
  (synChangeEB + theme(legend.position="none")) / 
  EBSlopes +plot_layout(design = layout) + plot_annotation(tag_levels=c("A","i"))
...
```{r}
synapseDiffC3 <- ggplot(distinct(FBC3Comp,databaseType,side,.keep_all = TRUE),aes(x=stringr::str_replace(databaseType,"Delta","\u0394"),y=known_synapses_ratio*100,color=supertype2)) + geom_point(size=3) + scale_color_CX_supertype(name="Supertype") + theme_minimal_grid() + facet_grid(.~side,scales="free_x") + xlab("") + ylab("Known synapses change (%)")+theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5))+ ggforce::geom_mark_circle(aes(filter = (databaseType == "hDeltaA" & side=="Outputs")),color="black",expand=0.03)
synapseDiffC3
```
...
```{r}
FC3Slopes <- ggplot(FBCompFits,aes(x=stringr::str_replace(databaseType,"Delta","\u0394"),y=estimate,ymin=estimate-std.error,ymax=estimate+std.error,color=supertype2)) + geom_point(size=3) + geom_linerange() + scale_color_CX_supertype(name="Supertype") + facet_grid(.~side,scales = "free_x") + theme_minimal_grid() +theme(panel.border = element_rect(color = "gray 50", fill = NA),axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) +xlab("") +ylab("Slope estimate")+ ggforce::geom_mark_circle(aes(filter = (databaseType == "hDeltaA" & side=="Outputs")),color="black",expand=0.03)
FC3Slopes
```

```{r}
FBComp <- ggplot(filter(FBC3Comp,databaseType=="hDeltaA" & side=="Outputs"),aes(x=CX,y=C3)) + geom_point(aes(color=supertype.to2),size=3)  + geom_smooth(formula=y~x,method="lm",color="grey") +
  geom_abline(linetype=2) + theme_minimal_hgrid()  + scale_color_CX_supertype(name="Supertype") + ylab("Relative output weight in column 3") + xlab("Relative output weight in the rest of the FB") 
FBComp
```
...
```{r}
hDeltaA_C3 <- lapply(hDeltaA_C3Id,getNeuronMesh,cloud=TRUE,node="2b4ff")
hDeltaA_CX <- lapply(hDeltaA_CXId,getNeuronMesh,cloud=TRUE,node="2b4ff")
...
plot3d(hDeltaA_CXSynapses[c("x","y","z")],add=TRUE,col="darkmagenta",size=5)
nview3d("ventral")
rgl.snapshot(paste0(validationFolder,"frontC3.png"))
```

```{r}
FBC3Frame <- ggdraw() + draw_image(paste0(validationFolder,"frontC3.png"),scale=1.55,vjust=0,clip="on")
...
FBC3Frame <- FBC3Frame + annotate(geom="text",x=0.72,y=0.92,label="hΔA outputing \n in column 3",color="SteelBlue")
FBC3Frame <- FBC3Frame + annotate(geom="point",x=0.73,y=0.8,color="orange",size=1) + 
  annotate(geom="point",x=0.73,y=0.78,color="darkmagenta",size=1) +
  annotate(geom="text",x=0.81,y=0.79,label="Output synapses")
FBC3Frame
```

```{r}
FC3Panel <- (((FBC3Frame | FBComp + theme(legend.position = "none")) + plot_layout(tag_level="new",widths=c(2,1))) /
  (synapseDiffC3 + theme(legend.position = "none")) /
  FC3Slopes) + plot_layout(heights = c(2,1,1)) + plot_annotation(tag_levels=c("A","i"))
FC3Panel
```

```{r}
save_plot(paste0(validationFolder,"ValidationPanel3.pdf"),FC3Panel,nrow = 3.5,ncol=2.5,device=cairo_pdf)
```
