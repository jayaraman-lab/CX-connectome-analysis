...
# Similarly, make a scatter plot showing the total input from left/right PFX populations onto each downstream neuron 
...
ConnectivityCriteria=data.frame(type.from=character(),type.to=character(),ColFromNum=numeric(),
                                PreProportion=numeric(),PostProportion=numeric(), OffAxisProp=numeric(), AverageWeight=numeric())
for (ppp in 1:length(Pre_Post$type.from)){
  ...
  OffAxisProp=sum(Temp_Scatter$weightRelative_R > 0 & Temp_Scatter$weightRelative_L > 0)/PostNum_All
  ...
  data.frame(type.from=Pre_Post$type.from[ppp],type.to=Pre_Post$type.to[ppp],ColFromNum=FBColfromNum,
             PreProportion=PreNum/PreNum_All,PostProportion=PostNum/PostNum_All,OffAxisProp=OffAxisProp, AverageWeight=AverageWeight))
  
  
  # Complete missing connections between these types so that they show up in connectivity matrix
  ...
  Temp_PFX_Out$weightRelative[is.na(Temp_PFX_Out$weightRelative)]=-1 # make missing connection weights negative so it shows up as white in connectivity plot
  
  
  # If enough neurons from the two populations are connected,
  # plot the connectivity matrix and the scatter plot.
  # Threshold kept low to show even weakly connected pairs
  if (FBColfromNum>5 & PreNum/PreNum_All>0.3 & PostNum/PostNum_All >0.3){
    
    
    # If pre/post type is the same, treat as recurrent type
    ...
    P1 <- ggplot(Temp_PFX_Out) + 
      ...
      scale_x_discrete(breaks  = XXX, labels = substr(XXX,1,7)) + 
      scale_y_discrete(breaks  = YYY, labels = substr(YYY,1,7)) + 
    ...
    # Make scatter plot showing input from left and right PFX neurons for each postsynaptic neuron
...
# Plot the amount of left/right lateralized input as a function of the proportion of postsynaptic neurons
# in each population and by the strength of the type-to-type connection. This shows that left/right lateralized
# connectivity matrices are from inconsistent and weak type-to-type connections.
```{r}
...
P1=ggplot() + geom_point(data=ConnectivityCriteria, aes(x=PostProportion, y=1-OffAxisProp, size=AverageWeight, color=PresynapticClass),
...
ggsave(paste(PlotDir, "Proportion_Vs_Lateralization_byWeight.pdf",sep=""),
         plot = P1, device='pdf', scale = 1, width =7, height = 6, units ="in", dpi = 500, limitsize = TRUE)
...
# If PFX neurons from the left PB target downstream cell types that PFX neurons from the right PB do not,
# then making a scatter plot of Left vs Right PFX input by each postsynaptic type should yield points on
# the x and y axes. Since this is not the case, we can rule out left/right differential targeting at the
# cell type level.
```{r}

# Average Left/Right PB input by postsynaptic type. Need to multiple by PostProportion because not all
# all postsynaptic neurons of a type receive input from the presynaptic type in question, and averaging
# would miss these 0 weights. Multiplying the average weights by the proportion of downstream neurons targeted 
# is the same as including the 0s. 
...
P1=ggplot() + geom_point(data=Scatter_All_Consistent_bytype, aes(x=weightRelative_L, y=weightRelative_R, color=PresynapticClass),alpha=0.8, shape=16) +
  scale_color_manual(values=ScatterColors, drop=FALSE)  + theme_classic() + theme(aspect.ratio = 1) + xlim(0,LIM) + ylim(0,LIM)
