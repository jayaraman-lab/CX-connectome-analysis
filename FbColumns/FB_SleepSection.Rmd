...
SW_Downstream_Filt=SW_Downstream  #subset(SW_Downstream, !is.na(SW_Downstream$databaseType.to)) # to exclude single bodyIDs with no type
...
SW_Downstream_Filt_Sum_All=OutputTyping(SW_Downstream_Filt_Sum_All)
...
# Set palette
library(prismatic)
BarPalette=rev(color(c("dodgerblue2","dodgerblue4","violetred3","darkorange1","bisque3","gray50","thistle4","black")))
...
P4=ggplot(SW_Downstream_Filt_Sum_All_Bar, aes(type.from)) +   geom_bar( aes(weight=weightRelative_path,fill = SuperType_Custom)) +
  scale_fill_manual(values=BarPalette) +  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
...
PlotData=subset(SW_Downstream_Filt_Sum_All, !startsWith(type.to,"FB") & !supertype3.to=="Unassigned")
...
P6=ggplot(PlotData) +
  ...
  theme(strip.background = element_blank(), #remove background for facet labels
        ...
        panel.spacing = unit(0, "lines"),#remove space between facets
        aspect.ratio = 1) 
ggsave(paste(PlotDir, "SW_Output_noFBt_FIGURE.pdf",sep=""),
        plot = P6, device='pdf', scale = 1, width =12, height = 6, units ="in", dpi = 500, limitsize = TRUE)
  

# remake matrix plots but with faceting by region 
PlotData=subset(SW_Downstream_Filt_Sum_All, startsWith(type.to,"FB") & !supertype3.to=="Unassigned")
MaxInput= PlotData %>% group_by(type.to) %>% slice(which.max(weightRelative_path))
MaxInput$Layer=substr(MaxInput$type.from, 3, 3) 
MaxInput=MaxInput[c("type.to","Layer")]
PlotData=merge(PlotData, MaxInput, by="type.to")
...
P7=ggplot(PlotData) +
  ...
  theme(strip.background = element_blank(), #remove background for facet labels
        ...
        panel.spacing = unit(0, "lines"),#remove space between facets
        aspect.ratio = 1) 
ggsave(paste(PlotDir, "SW_Output_FBt_FIGURE.pdf",sep=""),
...
# Get pathways, grouped by type.from and type.to
SW_Upstream_Filt=SW_Upstream  #subset(SW_Upstream, !is.na(SW_Upstream$databaseType.to)) # to exclude single bodyIDs with no type
SW_Upstream_Filt_Sum=SW_Upstream_Filt %>% group_by(type.from, type.to, supertype3.from) %>% summarize(weightRelative_path=sum(weightRelative_path))
SW_Upstream_Filt_Sum_All=subset(SW_Upstream_Filt_Sum, weightRelative_path>0.001)



# Make unformatted plots to see all the types Upstream of the sleep-wake types 
PlotData=subset(SW_Upstream_Filt_Sum_All, !supertype3.from=="Unassigned" & startsWith(type.from,"FB"))
P1=ggplot(PlotData) +  geom_tile(aes(type.to,type.from,fill=weightRelative_path)) +
  scale_fill_paletteer_c("ggthemes::Blue", limits=c(0,0.1), breaks = c(0,0.05,0.1), oob=squish) +
  theme_classic() + theme(axis.text.x = element_text(angle = 90))  + coord_fixed() 
  ggsave(paste(PlotDir, "SW_Inputs_FBt.png",sep=""),
...
P2=ggplot(PlotData) +  geom_tile(aes(type.to,type.from,fill=weightRelative_path)) +
  scale_fill_paletteer_c("ggthemes::Blue", limits=c(0,0.1), breaks = c(0,0.05,0.1), oob=squish) +
  theme_classic() + theme(axis.text.x = element_text(angle = 90))  + coord_fixed() 
  ggsave(paste(PlotDir, "SW_Inputs_NotFBt.png",sep=""),
...
SW_Upstream_Filt_Sum_All=InputTyping(SW_Upstream_Filt_Sum_All)

  
# Group Upstream types by supertype and compute average pathway weight and the number of target types
SW_Upstream_Filt_Sum_All_Bar=SW_Upstream_Filt_Sum_All %>% 
...
sort(unique(SW_Upstream_Filt_Sum_All_Bar$SuperType_Custom))

# Set palette
library(prismatic)
BarPalette=rev(color(c("dodgerblue2","dodgerblue4","violetred3","darkorange1","bisque3","gray50","black")))


# Make bar plot of Upstream targets, plotting both mean pathway weight and the number of types targeted
P4=ggplot(SW_Upstream_Filt_Sum_All_Bar, aes(type.to)) +   geom_bar( aes(weight=weightRelative_path,fill = SuperType_Custom)) +
  scale_fill_manual(values=BarPalette) +  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  ylab("Mean pathway relative weight ") + ylim(0,0.1)
  ggsave(paste(PlotDir, "SW_Input_PathwayWeight_Bar.pdf",sep=""),
         plot = P4, device='pdf', scale = 1, width =10, height =10, units ="in", dpi = 500, limitsize = TRUE)

P5=ggplot(SW_Upstream_Filt_Sum_All_Bar, aes(type.to)) +   geom_bar( aes(weight=n,fill = SuperType_Custom)) +
  scale_fill_manual(values=BarPalette) +  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  ylab("# Upstream types") + ylim(0,100)
  ggsave(paste(PlotDir, "SW_Input_NumberOfTypes_Bar.pdf",sep=""),
...
# remake matrix plots but with faceting by region 
PlotData=subset(SW_Upstream_Filt_Sum_All, !startsWith(type.from,"FB") & !startsWith(type.from,"OA") & !supertype3.from=="Unassigned")
PlotData$SuperType_Custom=factor(PlotData$SuperType_Custom, levels=rev(levels(PlotData$SuperType_Custom)))
P6=ggplot(PlotData) +
  geom_tile(aes(type.to,type.from,fill=weightRelative_path)) +
  scale_fill_paletteer_c("ggthemes::Blue", limits=c(0,0.1), breaks = c(0,0.05,0.1), oob=squish) +
  ...
  theme(strip.background = element_blank(), #remove background for facet labels
        ...
        panel.spacing = unit(0, "lines"),#remove space between facets
        aspect.ratio = 1) 
ggsave(paste(PlotDir, "SW_Input_noFBt_FIGURE.pdf",sep=""),
...
# remake matrix plots but with faceting by region 
PlotData=subset(SW_Upstream_Filt_Sum_All, (startsWith(type.from,"FB") | startsWith(type.from,"OA")) & !supertype3.from=="Unassigned")
MaxInput= PlotData %>% group_by(type.from) %>% slice(which.max(weightRelative_path))
...
P7=ggplot(PlotData) +
  ...
  theme(strip.background = element_blank(), #remove background for facet labels
        ...
        panel.spacing = unit(0, "lines"),#remove space between facets
        aspect.ratio = 1) 
ggsave(paste(PlotDir, "SW_Input_FBt_FIGURE.pdf",sep=""),
