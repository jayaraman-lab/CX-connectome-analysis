...
if (Case==1){
  ...
  NODES$Group=NA
  NODES$Group[NODES$Step==1 & startsWith(NODES$name, "FB6") ] = "Group1"
  NODES$Group[NODES$Step==1 & startsWith(NODES$name, "FB7") ] = "Group2"
  
  
  # Get max inputs of layer 2 neurons and assign that as group
  A=subset(Graph, type.to %in% NODES$name[NODES$Step==2])
  A=A %>% group_by(type.to) %>% filter(weightRelative == max(weightRelative)) 
  A=A[c('type.from','type.to')]
  A$Group=NA
  A$Group[startsWith(A$type.from,"FB6")]= "Group3"
  A$Group[startsWith(A$type.from,"FB7")]= "Group4"
  A=A[c('type.to','Group')]
  colnames(A)=c('name','Group')
  NODES=NODES %>% 
    left_join(A, by = "name") %>% 
  ...
  NODES$x[NODES$Step==1]=0
  NODES$y[NODES$Step==1]=seq(from=-l1nodes/2,l1nodes/2,by=1)*1500
  NODES$y[NODES$Step==1 & startsWith(NODES$name, "FB7") ] = NODES$y[NODES$Step==1 & startsWith(NODES$name, "FB7") ] + 6000
  NODES$y[NODES$Step==1 & startsWith(NODES$name, "FB6") ] = NODES$y[NODES$Step==1 & startsWith(NODES$name, "FB6") ] - 20000
  
  l2nodesG3=sum(NODES$Step==2  & NODES$Group=="Group3")
  NODES$x[NODES$Step==2 & NODES$Group=="Group3"]=40
  NODES$y[NODES$Step==2 & NODES$Group=="Group3"]=seq(from=-1,to=-l2nodesG3,by=-1)*1000 -1500
  
  
  l2nodesG4=sum(NODES$Step==2  & NODES$Group=="Group4")
  NODES$x[NODES$Step==2 & NODES$Group=="Group4"]=40
  NODES$y[NODES$Step==2 & NODES$Group=="Group4"]=seq(from=1,to=l2nodesG4,by=1)*1000 + 2500
  
  
  L3_G1=c("FB1A","FB1G","FB8H","FB8I","vDeltaA_b")
  NODES=NODES[ c(which(!NODES$name %in% L3_G1), which(NODES$name %in% L3_G1)),]
  
  NODES$name[NODES$Step==3]
  l3nodes=sum(NODES$Step==3)
  NODES$x[NODES$Step==3]=80
  NODES$y[NODES$Step==3]=seq(from=-l3nodes/2,l3nodes/2-1,by=1)*600 -12000
  ...
  NODES$y[NODES$Step==1 & startsWith(NODES$Group, "L1_G2") ] = rev(NODES$y[NODES$Step==1 &  startsWith(NODES$Group, "L1_G2")] + 6)
  NODES$y[NODES$Step==1 & startsWith(NODES$Group, "L1_G1") ] = rev(NODES$y[NODES$Step==1 & startsWith(NODES$Group, "L1_G1") ] - 2)
  ...
  FB_L2_G1=c("FB1I","FB1J","FB7L","FB8C","FB8G","vDeltaE","vDeltaB","FB6U","FB6T","FB6Q")
  ...
  FB_L2_G2=c("hDeltaD","FB8B")
  NODES$Group[NODES$Step==2 & NODES$name %in% FB_L2_G2] = "L2_G2"
  NODES$x[which(NODES$Group=="L2_G2")]=1
  NODES$y[which(NODES$Group=="L2_G2")]=(seq(from=-length(FB_L2_G2)/2,length(FB_L2_G2)/2-1,by=1) -6) *Scale_L2
  

  FB_L2_G3=c("FB6Y","FB7C","FB7D","FB7E","FB7H","FB7J","FB7M")
  NODES$Group[NODES$Step==2 & NODES$name %in% FB_L2_G3] = "L2_G3"
  NODES$x[which(NODES$Group=="L2_G3")]=0.9
  NODES$y[which(NODES$Group=="L2_G3")]=(seq(from=-length(FB_L2_G3)/2,length(FB_L2_G3)/2-1,by=1) + 0) *Scale_L2
  

  FB_L2_G4=c("ExR3")
  NODES$Group[NODES$Step==2 & NODES$name %in% FB_L2_G4] = "L2_G4"
  NODES$x[which(NODES$Group=="L2_G4")]=1
  NODES$y[which(NODES$Group=="L2_G4")]=-8 *Scale_L2
  ...
  NODES$y[which(NODES$Group=="L2_G5")]=(seq(from=-length(FB_L2_G5)/2,length(FB_L2_G5)/2-1,by=1) - 15) *Scale_L2
  ...
  NODES$y[which(NODES$Group=="L2_G6")]=(seq(from=-length(FB_L2_G6)/2,length(FB_L2_G6)/2-1,by=1) - 25) *Scale_L2
  ...
  NODES$y[which(NODES$Group=="L2_G7")]=(seq(from=-length(FB_L2_G7)/2,length(FB_L2_G7)/2-1,by=1) - 51) *Scale_L2
  
  
  FB_L2_G8=NODES$name[(startsWith(NODES$name,"FC") |  startsWith(NODES$name,"FS") | startsWith(NODES$name,"hDel") )
                      & NODES$Step==2 & is.na(NODES$Group)]
  NODES$Group[NODES$Step==2 & NODES$name %in% FB_L2_G8] = "L2_G8"
  NODES$x[which(NODES$Group=="L2_G8")]=1
  NODES$y[which(NODES$Group=="L2_G8")]=(seq(from=-length(FB_L2_G8)/2,length(FB_L2_G8)/2-1,by=1) - 37.5) *Scale_L2
  ...
  NODES$y[is.na(NODES$y) & NODES$Step==2]=(seq(from=-1, to=-logica, by=-1)-59)*Scale_L2
  ...
  FB_L3_G2=c("EL","EPG","ER2_a","ER2_c","ER2_d","ER3a_a","ER3a_c","ER3d_a","ER3d_b","ER3d_c","ER3d_d",
             "ER3m","ER3p_a","ER3p_b","ER4d","ER4m","ER5","ExR1","ExR2","ExR5")
  NODES$Group[NODES$Step==3 & NODES$name %in% FB_L3_G2] = "L3_G2"
  NODES$x[which(NODES$Group=="L3_G2")]=2
  NODES$y[which(NODES$Group=="L3_G2")]=ceiling(seq(from=-length(FB_L3_G2)/2,length(FB_L3_G2)/2-1,by=1) - 6)*Scale_L3
  ...
  Angles=seq(from=-pi, to=pi, by=2*pi/length(FB_L3_G5))
  NODES$x[which(NODES$Group=="L3_G5")]=(2+cos(Angles)*0.1)
  NODES$y[which(NODES$Group=="L3_G5")]=(-325 + sin(Angles)*40)
  
  
  
  FB_L3_G6=Graph$type.to[Graph$type.to %in% FB_L2_G1] 
  FB_L3_G6=FB_L3_G6[FB_L3_G6 %in% NODES$name[NODES$Step==3 & !is.na(NODES$x)]] 
  
  NODES$Group[NODES$Step==3 & NODES$name %in% FB_L3_G6] = "L3_G6"
  NODES$x[which(NODES$Group=="L3_G6")]=2
  NODES$y[which(NODES$Group=="L3_G6")]=ceiling(seq(from=-length(FB_L3_G6)/2,length(FB_L3_G6)/2 -1,by=1)-25 )*Scale_L3
  ...
  NODES$y[is.na(NODES$y) & NODES$Step==3]=(seq(from=-1, to=-logica, by=-1))*Scale_L3 -100
  ...
  NODES$y[NODES$Step==3]= NODES$y[NODES$Step==3] - mean(NODES$y[NODES$Step==3])
}
...
P1=ggraph(plotgraph,layout="manual",x=NODES$x,y=NODES$y) + 
  geom_edge_fan( aes(width=weight,colour=EDGES$COLORZ), 
                 arrow = arrow(length = unit(2, 'mm'),ends = "last", type = "closed"), end_cap = circle(0, 'cm'), alpha=0.7,strength=1) +
