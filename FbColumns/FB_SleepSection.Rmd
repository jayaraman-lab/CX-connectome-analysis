...
# Get everything downstream of 23E10 neurons
```{r}
...
save(SW_Types_Table, SW_Downstream_Raw, SW_Downstream, SW_Downstream_Graph,
     file = paste(PlotDir,"SW_Downstream.RData",sep=""))
...
SW_Downstream_Filt=subset(SW_Downstream, !is.na(SW_Downstream$databaseType.to))
SW_Downstream_Filt_Sum=SW_Downstream_Filt %>% group_by(type.from, type.to, supertype3.to) %>% summarize(weightRelative_path=sum(weightRelative_path))
SW_Downstream_Filt_Sum_All=subset(SW_Downstream_Filt_Sum, weightRelative_path>0.001)




# Make plots 
P1=ggplot(SW_Downstream_Filt_Sum_All) +  geom_tile(aes(type.to,type.from,fill=weightRelative_path)) +
  scale_fill_paletteer_c("ggthemes::Blue", limits=c(0,0.1), breaks = c(0,0.05,0.1), oob=squish) +
  theme_classic() + theme(axis.text.x = element_text(angle = 90))  + coord_fixed() 
  ggsave(paste(PlotDir, "SW_OutputAll.png",sep=""),
         plot = P1, device='png', scale = 1, width =12, height = 6, units ="in", dpi = 500, limitsize = TRUE)

P2=ggplot(subset(SW_Downstream_Filt_Sum_All, !startsWith(type.to,"FB"))) +
  geom_tile(aes(type.to,type.from,fill=weightRelative_path)) +
  scale_fill_paletteer_c("ggthemes::Blue", limits=c(0,0.1), breaks = c(0,0.05,0.1), oob=squish) +
  theme_classic() + theme(axis.text.x = element_text(angle = 90))  + coord_fixed()
  ggsave(paste(PlotDir, "SW_Output_noFBt.png",sep=""),
         plot = P2, device='png', scale = 1, width =12, height = 6, units ="in", dpi = 500, limitsize = TRUE)

P3=ggplot(subset(SW_Downstream_Filt_Sum_All, startsWith(type.to,"FB"))) +
  geom_tile(aes(type.to,type.from,fill=weightRelative_path)) +
  scale_fill_paletteer_c("ggthemes::Blue", limits=c(0,0.1), breaks = c(0,0.05,0.1), oob=squish) +
  theme_classic() + theme(axis.text.x = element_text(angle = 90)) + coord_fixed()
  ggsave(paste(PlotDir, "SW_Output_FBt.png",sep=""),
         plot = P3, device='png', scale = 1, width =12, height = 6, units ="in", dpi = 500, limitsize = TRUE)

  

# Group output types
SW_Downstream_Filt_Sum_All$SuperType_Custom=NA
SW_Downstream_Filt_Sum_All$SuperType_Custom[startsWith(SW_Downstream_Filt_Sum_All$type.to,"5-HT") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"SI") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"SL") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"SM") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"DGI") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"IB042") ]="SNP"
SW_Downstream_Filt_Sum_All$SuperType_Custom[startsWith(SW_Downstream_Filt_Sum_All$type.to,"EL") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"ER") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"Ex") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"TuB")|
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"AOT")]="EB"
SW_Downstream_Filt_Sum_All$SuperType_Custom[startsWith(SW_Downstream_Filt_Sum_All$type.to,"PF") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"hDelta") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"vDelta") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"FS") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"FR") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"FC")]="FB Columnar"
SW_Downstream_Filt_Sum_All$SuperType_Custom[startsWith(SW_Downstream_Filt_Sum_All$type.to,"FB")]="FB Tangential"
SW_Downstream_Filt_Sum_All$SuperType_Custom[startsWith(SW_Downstream_Filt_Sum_All$type.to,"LAL")]="LAL"
SW_Downstream_Filt_Sum_All$SuperType_Custom[startsWith(SW_Downstream_Filt_Sum_All$type.to,"CRE")]="CRE"
SW_Downstream_Filt_Sum_All$SuperType_Custom[startsWith(SW_Downstream_Filt_Sum_All$type.to,"ATL") |
                                       startsWith(SW_Downstream_Filt_Sum_All$type.to,"PAM09")]="Olfactory"

SW_Downstream_Filt_Sum_All$type.to[is.na(SW_Downstream_Filt_Sum_All$SuperType_Custom)]


SW_Downstream_Filt_Sum_All_Bar=SW_Downstream_Filt_Sum_All %>% 
  group_by(type.from, SuperType_Custom) %>% 
  summarize(weightRelative_path=mean(weightRelative_path), n=n())


SW_Downstream_Filt_Sum_All_Bar$SuperType_Custom=factor(SW_Downstream_Filt_Sum_All_Bar$SuperType_Custom, 
            levels=rev(c("FB Tangential","FB Columnar","EB","SNP","LAL","CRE","Olfactory")))


# Set palette
library(prismatic)
Palette2=rev(color(c("dodgerblue2","dodgerblue4","violetred3","darkorange1","bisque3","gray50","black")))


# Make bar plot of downstream targets, plotting both mean pathway weight and the number of types targeted
P4=ggplot(SW_Downstream_Filt_Sum_All_Bar, aes(type.from)) +   geom_bar( aes(weight=weightRelative_path,fill = SuperType_Custom)) +
  scale_fill_manual(values=Palette2) +  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  ylab("Mean pathway relative weight ")
  ggsave(paste(PlotDir, "SW_Output_PathwayWeight_Bar.pdf",sep=""),
         plot = P4, device='pdf', scale = 1, width =8, height =4, units ="in", dpi = 500, limitsize = TRUE)

P5=ggplot(SW_Downstream_Filt_Sum_All_Bar, aes(type.from)) +   geom_bar( aes(weight=n,fill = SuperType_Custom)) +
  scale_fill_manual(values=Palette2) +  theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  ylab("# downstream types")
  ggsave(paste(PlotDir, "SW_Output_NumberOfTypes_Bar.pdf",sep=""),
         plot = P5, device='pdf', scale = 1, width =8, height = 4, units ="in", dpi = 500, limitsize = TRUE)

  
  # remake matrix plots but with faceting
  P2=ggplot(subset(SW_Downstream_Filt_Sum_All, !startsWith(type.to,"FB"))) +
    geom_tile(aes(type.to,type.from,fill=weightRelative_path)) +
    scale_fill_paletteer_c("ggthemes::Blue", limits=c(0,0.1), breaks = c(0,0.05,0.1), oob=squish) +
    theme_classic() + theme(axis.text.x = element_text(angle = 90))  + coord_fixed()
