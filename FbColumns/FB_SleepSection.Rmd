...
# Chose only the right sleep-wake types as the starting (for downstream tracing) or stoping (for upstream tracing) points,
# Then chose whicher upstream/downstream types gives largest input
```{r}

# Get only right sleep wake types and chose whichever L/R type.to is largest
SW_Downstream=subset(SW_Downstream, grepl("_R",SW_Downstream$type.from))
SW_Downstream=SW_Downstream %>% group_by(databaseType.from, databaseType.to, type_N1, n_steps) %>%
  filter(weightRelative_path == max(weightRelative_path)) 
SW_Downstream$type.from=SW_Downstream$databaseType.from
SW_Downstream$type.to=SW_Downstream$databaseType.to

  
# Get only right sleep wake types and chose whichever L/R type.to is largest
SW_Upstream=subset(SW_Upstream, grepl("_R",SW_Upstream$type.to))
SW_Upstream=SW_Upstream %>% group_by(databaseType.from, databaseType.to, type_N1, n_steps) %>%
  filter(weightRelative_path == max(weightRelative_path)) 
SW_Upstream$type.from=SW_Upstream$databaseType.from
SW_Upstream$type.to=SW_Upstream$databaseType.to
...
# First show that steps 2 and 3 are the same (i.e. no new known types are reached; though this depends on the threshold)
SW_KnownDownstream=subset(SW_Downstream, !supertype3.to == "Unassigned" & weightRelative_path>0.001) #0.001
Downstream_Step1_Types=unique(SW_KnownDownstream$type.to[SW_KnownDownstream$n_steps==1])
Downstream_Step2_Types=unique(SW_KnownDownstream$type.to[SW_KnownDownstream$n_steps==2])
Downstream_Step3_Types=unique(SW_KnownDownstream$type.to[SW_KnownDownstream$n_steps==3])
Downstream_Step3_Types %in% Downstream_Step2_Types # All neurons reach by step3 were already in step2
...
SW_Downstream_Filt=subset(SW_Downstream, weightRelative_path>0.001) # threshold out weak paths
...
SW_KnownUpstream=subset(SW_Upstream,  weightRelative_path>0.001)
...
# Get pathways, grouped by type.from and type.to
SW_Upstream_Filt=subset(SW_Upstream, weightRelative_path>0.001) # threshold out weak paths
...
SW_EbFb_T2T=SW_EbFb_Bag$inputs
...
NODES$x=NA
NODES$y=NA
NODES[NODES$name == "ExR1",c(2,3)]=c(-140,0)
NODES[NODES$name == "ExR3",c(2,3)]=c(-90,0)
NODES[NODES$name == "hDeltaK",c(2,3)]=c(-40,0)
NODES[NODES$name == "ER3d_a",c(2,3)]=c(-150,-10)
NODES[NODES$name == "ER3d_b",c(2,3)]=c(-130,-15)
NODES[NODES$name == "ER3d_c",c(2,3)]=c(-95,-15)
NODES[NODES$name == "ER3d_d",c(2,3)]=c(-75,-10)
NODES[NODES$name == "ER5",c(2,3)]=c(-175,-5)
NODES[NODES$name == "PEN_b(PEN2)",c(2,3)]=c(-40,-10)
NODES$y=NODES$y*5+50


# NODES
NODES$COLORZ="gray50"
NODES$COLORZ[NODES$name == "ExR1" | NODES$name == "ExR3" | NODES$name == "hDeltaK"]="cyan3"
NODES$COLORZ[startsWith(as.character(NODES$name),"ER3")]="seagreen3"
NODES$COLORZ[startsWith(as.character(NODES$name),"ER5")]="plum3"
NODES$COLORZ[startsWith(as.character(NODES$name),"PEN")]="lightseagreen"
...
EDGES$to=as.character(EDGES$to)
EDGES$COLORZ=NA
EDGES$COLORZ[EDGES$from == "ExR1" | EDGES$from == "ExR3" | EDGES$from == "hDeltaK"]="cyan3"
EDGES$COLORZ[startsWith(as.character(EDGES$from),"ER3")]="seagreen3"
EDGES$COLORZ[startsWith(as.character(EDGES$from),"ER5")]="plum3"
EDGES$COLORZ[startsWith(as.character(EDGES$from),"PEN")]="lightseagreen"
...
NODES[NODES$name == "ExR3",c(2,3)]=c(-90,0)
NODES[NODES$name == "hDeltaK",c(2,3)]=c(-40,0)
NODES[NODES$name == "PFGs",c(2,3)]=c(-90-20,10)
NODES[NODES$name == "FB6A",c(2,3)]=c(-20,10)
NODES[NODES$name == "hDeltaF",c(2,3)]=c(-110,15)
NODES[NODES$name == "FB7K",c(2,3)]=c(-150-35-10,12)
NODES[NODES$name == "FB7B",c(2,3)]=c(-137.5-35-5,17)
NODES[NODES$name == "FB7A",c(2,3)]=c(-125-35,14)
NODES[NODES$name == "FB8B",c(2,3)]=c(-175,5)
NODES[NODES$name == "hDeltaD",c(2,3)]=c(-200,5)
...
# NODES
NODES$COLORZ=NA
NODES$COLORZ[startsWith(as.character(NODES$name),"hDelta")]="salmon1"
NODES$COLORZ[NODES$name == "ExR1" | NODES$name == "ExR3" | NODES$name == "hDeltaK"]="cyan3"
NODES$COLORZ[startsWith(as.character(NODES$name),"FB")]="seagreen3"
NODES$COLORZ[startsWith(as.character(NODES$name),"PFG")]="slateblue1"
NODES$COLORZ[NODES$name == "FB6A" | NODES$name=="FB7A" | NODES$name=="FB7K"]=col_vector[1]
NODES$COLORZ[NODES$name == "FB7B"]=col_vector[2]
...
EDGES$to=as.character(EDGES$to)
EDGES$COLORZ=NA
EDGES$COLORZ[startsWith(as.character(EDGES$from),"hDelta")]="salmon1"
EDGES$COLORZ[EDGES$from == "ExR1" | EDGES$from == "ExR3" | EDGES$from == "hDeltaK"]="cyan3"
EDGES$COLORZ[startsWith(as.character(EDGES$from),"FB")]="seagreen3"
EDGES$COLORZ[startsWith(as.character(EDGES$from),"PFG")]="slateblue1"
EDGES$COLORZ[EDGES$from == "FB6A" | EDGES$from=="FB7A" | EDGES$from=="FB7K"]=col_vector[1]
EDGES$COLORZ[EDGES$from == "FB7B"]=col_vector[2]
