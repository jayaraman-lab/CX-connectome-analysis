...
# Get roi hierarchy
roiH <- getRoiTree()
NonFB_ROIs=unique(roiH$level2[!roiH$level2=="FB"])


# Get direct outputs from sleep-wake types
DownBag=neuronBag(SleepWakeTypes_Meta, by.roi=TRUE, omitInputs=TRUE, omitOutputs=FALSE)

DownBag_NonFB=combineRois(DownBag, NonFB_ROIs, newRoi="NonFB")
SW_Downstream_NonFB=getTypeToTypeTable(DownBag_NonFB$outputs_raw, majorOutputThreshold = 0.8, singleNeuronThreshold = 0.01,
                                       singleNeuronThresholdN = 3, pThresh = 1, typesTable = NULL, oldTable = NULL)

DownBag_FB=combineRois(DownBag, "FB", newRoi="FB")
SW_Downstream_FB=getTypeToTypeTable(DownBag_FB$outputs_raw, majorOutputThreshold = 0.8, singleNeuronThreshold = 0.01,
                                       singleNeuronThresholdN = 3, pThresh = 1, typesTable = NULL, oldTable = NULL)


# Get direct inputs to sleep-wake types
UpBag=neuronBag(SleepWakeTypes_Meta, by.roi=TRUE, omitInputs=FALSE, omitOutputs=TRUE)

UpBag_NonFB=combineRois(UpBag, NonFB_ROIs, newRoi="NonFB")
SW_Upstream_NonFB=getTypeToTypeTable(UpBag_NonFB$inputs_raw, majorOutputThreshold = 0.8, singleNeuronThreshold = 0.01,
                                     singleNeuronThresholdN = 3, pThresh = 1, typesTable = NULL, oldTable = NULL)

UpBag_FB=combineRois(UpBag, "FB", newRoi="FB")
SW_Upstream_FB=getTypeToTypeTable(UpBag_FB$inputs_raw, majorOutputThreshold = 0.8, singleNeuronThreshold = 0.01,
                                     singleNeuronThresholdN = 3, pThresh = 1, typesTable = NULL, oldTable = NULL)
...
# Pick a relative weight threshold for input and output pathways (0.01 for figures)
```{r message=FALSE, warning=FALSE}
RelativeWeightThresh=0.01
SW_Downstream_NonFB=subset(SW_Downstream_NonFB, weightRelative >= RelativeWeightThresh)
SW_Downstream_FB=subset(SW_Downstream_FB, weightRelative >= RelativeWeightThresh)
SW_Upstream_NonFB=subset(SW_Upstream_NonFB, weightRelative >= RelativeWeightThresh)
SW_Upstream_FB=subset(SW_Upstream_FB, weightRelative >= RelativeWeightThresh)
```
...
unique(SW_Downstream_FB$type.to)
unique(SW_Upstream_NonFB$type.from)


 Pd=ggplot(SW_Downstream_FB) + geom_tile(aes(type.to,type.from,fill=weightRelative)) +
