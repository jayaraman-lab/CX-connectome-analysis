...
# Get all FB bodies and columnar/tangential types
```{r message=FALSE, warning=FALSE}
...
# Get meta information dFB dopamine neurons (FB7B and FB6H) with connections to 23E10 neurons
...
# Get neuron-to-neuron connectivity table involving just sleep-wake types
...
P4=Plot_SleepWake_Graph(GraphTable)
P4
...
# ####################################################################################################################
# ######### Section 2: Analysis of upstream/downstream of 23E10 and PPL1 circuit #####################################
# ####################################################################################################################
...
if (Method=="no thresh"){
  ...
  SW_Downstream_Raw <- get_type2typePath_raw(type.from=SleepWakeTypes_Meta, type.to = NULL, n_steps=1:2, by.roi=FALSE, renaming=lateralize_types )
  SW_Downstream=tableChain2path(SW_Downstream_Raw)
  SW_Downstream_Graph=distinct(rbind(SW_Downstream_Raw[[1]], SW_Downstream_Raw[[2]]))
...
if (Method=="no thresh"){
  ...
  SW_Upstream_Raw <- get_type2typePath_raw(type.from=NULL, type.to = SleepWakeTypes_Meta, n_steps=1:2, by.roi=FALSE, renaming=lateralize_types )
  SW_Upstream=tableChain2path(SW_Upstream_Raw)
  SW_Upstream=subset(SW_Upstream, databaseType.to %in% SleepWakeTypes_Meta$type)
  SW_Upstream_Graph=distinct(rbind(SW_Upstream_Raw[[1]], SW_Upstream_Raw[[2]]))
...
save(SleepWakeTypes_Meta, SW_Downstream_Raw, SW_Downstream, SW_Downstream_Graph,
