...
Example_Inds=c(11,23,232,14,10,45,24,3)
...
ggsave(paste(PlotDir, "PCA_Scatter_Examples.png",sep=""), plot = P3, device='png', scale = 1, width =6, height = 4, units ="in", dpi = 250, limitsize = TRUE)
...
for (ppp in 1:length(Example_Inds)){
  ...
  P2=ggplot()  + geom_point(data=Columnar_PC_Motifs,aes(x=V1, y=V2, color=Motif),alpha=0.5,size=2) + 
  ...
  # Save figure
  P4=ggarrange(P1,P2,nrow=1,ncol=2)
  ggsave(paste(PlotDir, Pre_Post$type.from[Ind],"_to_", Pre_Post$type.to[Ind], "_", as.character(Ind),"_inFB.png",sep=""), 
    plot = P4, device='png', scale = 1, width =15, height = 3.5, units ="in", dpi = 250, limitsize = TRUE)
