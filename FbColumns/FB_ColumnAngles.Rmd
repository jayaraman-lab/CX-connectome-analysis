...
# Calculate how many columns are involved
...
PrePostTypes=right_join(PrePostTypes, ColumnNums, by=c("type.from","type.to"))
...
# Why are there so many points at 67.5 & 112.5 points? Because 
# Should I be excluding hDelta(s) to avoid double band problem?
# Why so much variance for the PFR_a and PFR_b types?
# Probably makes sense to average across postsynaptic types
# Is phase shift consistent across columns?
# I should be computing the mean resultant vector length too, which could be the circle size
# Should I be excluding C0 neurons???


# Get just those neurons that reeive input from left and right populations and 
# are part of a significant T2T connections without axon+dendrite input to hDeltas
PhaseShiftData=right_join(FB_Neuron_Angles, PrePostTypes, by=c("type.from","type.to"))
PhaseShiftData=subset(FB_Neuron_Angles, !(is.na(EPG_Angle_L) | is.na(EPG_Angle_R)) & !FBcol.to=="C0")
...
P1=ggplot() +  
  ...
  geom_point(data=PhaseShiftData, aes(x=type.from, y=EPG_Angle_Diff, size=EPG_Angle_rho, color=type.to), shape=21) +
...
# TO DO:
# 1) should really restrict presynaptic neurons to those with similar phase shifts (no because phase offsets cancel)
...
# Get just those neurons with PB-FB-XX phase estimates and location data
PositionInfo=subset(PositionInfo, bodyid %in% unique(FB_Neuron_Angles$to))
FB_Neuron_Angles_Phase=subset(FB_Neuron_Angles, to %in% unique(PositionInfo$bodyid))

# Merge position and phase information
colnames(PositionInfo)[colnames(PositionInfo)=="bodyid"]="to"
PositionInfo$to=as.character(PositionInfo$to)
FB_Neuron_Angles_Phase$to=as.character(FB_Neuron_Angles_Phase$to)
Position_And_Tuning=left_join(FB_Neuron_Angles_Phase, PositionInfo, by="to")
...
# Loop over postsynaptic types and make scatter plot of phase versus position
PostType=unique(Position_And_Tuning$type.to)
ScatterColors <- paletteer_d("Polychrome::palette36")
ScatterColors = ScatterColors[c(5,17,8,6,11,33,1)]
for (ttt in 1:length(PostType)){
  
  # Get just this posttype
  Temp_Pos_And_Tune=subset(Position_And_Tuning, type.to==PostType[ttt])
  ...
  Px=ggplot() + 
     ...
     scale_x_continuous(breaks=seq(from=-AX, to=AX, by=10), limits=c(-AX, AX)) +
     ggtitle(paste(PostType[ttt], "  ", Temp_Pos_And_Tune$Layer[1])) +
     scale_color_manual(values=ScatterColors, drop=FALSE) +
     theme(text = element_text(size=16), 
           axis.text.x = element_text(color="black"),
           axis.text.y = element_text(color="black"))
  Px
  ggsave(paste(PlotDir, "MeanPhase_", PostType[ttt], ".png", sep=""),
