...
# Plot hisogram of at the synapse level?
...
ggsave(paste(PlotDir, "PhaseShift_by_AngleNumer", ".png", sep=""),
       plot = Ps, device='png', scale = 1, width = 8, height = 6, units ="in", dpi = 500, limitsize = TRUE)


# Plot histograph of PB-FB phase shift using angles from all synapses, aligned to average of 
# left PB input. 
Angles=c()
for (nnn in 1:nrow(PFN_PhaseShiftData)){
  TempAngles=unlist(PFN_PhaseShiftData$AllAngles[nnn]) - PFN_PhaseShiftData$EPG_Angle_L[nnn]
  Angles=c(Angles,TempAngles)
}
Angles=(Angles+360)%%360

DF=data.frame(Angles=Angles)
Pt=ggplot(DF, aes(x=Angles)) + 
   geom_freqpoly(binwidth = 2) + theme_classic() + xlab("PB-FB Phase Shift (deg)") +
   ylab("# of neurons")
Pt
ggsave(paste(PlotDir, "PhaseShift_by_AngleNumer_Neurons", ".png", sep=""),
       plot = Ps, device='png', scale = 1, width = 8, height = 6, units ="in", dpi = 500, limitsize = TRUE)
