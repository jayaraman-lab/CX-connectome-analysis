...
FigDirOffset="C:/Users/labadmin/Documents/Cx_EM_Analysis/BradData/FB_Analysis/Column_Positions/ContinuousOffsets/"
...
if (!dir.exists(FigDirOffset)){dir.create(FigDirOffset)}
...
FBC_SynsAll=Assign_FBcol_PBglom(FBC_SynsAll, "name", "Side", "PBglom", "FBcol", "bodyid")
...
# Plot PB-FB-XX neuron locations along with their glomerulus to column mappings
```{r}
...
for (ttt in 1:length(PbFb_Types)){
  ...
  } else if (EdgeType=="FBcol"){
    EDGES=data.frame(from=PlotData$PBglom, to=paste(PlotData$name, PlotData$bodyid,sep="_") , weight=1, COLORZ=PlotData$FBcol)
    EDGES$COLORZ=factor(EDGES$COLORZ, levels=c("C1","C2","C3","C4","C5","C6","C7","C8","C9"))
    ColumnColors=color(c("#FFF688", "#FFA010", "#FF3610", "#FBB5DE", "#7D24E7", "#5C89C7", "#C7FFEF", "#81FB35", "#FFF688"))
  }
  ...
  ggsave(paste(FigDirOffset, "ColumnLocs_", PbFb_Types[ttt], "_", as.character(CurrentLayer),"_",EdgeType, ".png", sep=""),
         plot = P1, device='png', scale = 1, width = 8, height = 6, units ="in", dpi = 500, limitsize = TRUE)
  ggsave(paste(FigDirOffset, "ColumnLocs_", PbFb_Types[ttt], "_", as.character(CurrentLayer),"_",EdgeType, ".pdf", sep=""),
         plot = P1, device='pdf', scale = 1, width = 8, height = 6, units ="in", dpi = 500, limitsize = TRUE)
