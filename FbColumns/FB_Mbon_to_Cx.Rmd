...
# Pre Nodes
MBON_Nodes=data.frame(name=sort(unique(Graph_Data$databaseType.from) %>% as.character())) 
MBON_Nodes$x=0
MBON_Nodes$y=seq(from = 1, to = length(MBON_Nodes$name), by = 1)


# Post Nodes
CX_Nodes=data.frame(name=sort(unique(Graph_Data$databaseType.to) %>% as.character())) 
CX_Nodes$x=1
CX_Nodes$y=seq(from = 1, to = length(CX_Nodes$name), by = 1)


# edges
EDGES=Graph_Data[c("databaseType.from","databaseType.to","weightRelative")]
...
P4=ggraph(graph,layout="manual",x=NODES$x,y=NODES$y) + 
  geom_edge_fan(end_cap = circle(3,"mm"),linejoin = "mitre",linemitre=3,arrow =arrow(length = unit(1, 'mm'),type = "closed")) +
  geom_node_point(color=NODES$COLORZ, size=2) +
  geom_node_text(aes(label=name),angle=0,size=2, nudge_y = c(rep(0.06,18),rep(-0.06,9)), color='black' ) +
  theme_classic() + 
  theme(legend.text=element_text(size=6),legend.title=element_text(size=6),
        axis.line=element_blank(),axis.text.x=element_blank(),
