...
# Get sum of relative weights across non-CX ROIs, which accounts for FBt polarity. 
# That is, since most FBt types receive a lot of input in the CX, the roi-independent relative weight can be very low.
roisH <- getRoiTree() %>% subset(level1!="CX") # Get list of ROIs that are not the CX
DownBag_L1 <- combineRois(DownBag_L1, unique(roisH$level1), newRoi="NonCX") # Sum synapses across all non-CX ROIs



#Romain uses the below, which restricts ROIs to those on the right, so that we don't miss crosses (see his slack message)
# But, may have to do something more complicated for 2 step pathways.
roiH <- getRoiTree()
outsideRegions <- unique(selectRoiSet(exceptions=sapply(as.character(unique(roiH$level1[grepl(roiH$level1,pattern="(R)")])),function(i) return(1),USE.NAMES = TRUE,simplify=FALSE), exceptionLevelMatch = 1)$roi[roiH$level1!="CX" & roiH$side4!="Left"])
...
# Restrict to just CX targets in the right hemisphere
MB2CX_Direct=subset(Downstream_T2T_L1, databaseType.to %in% CXTypes & grepl("_R",type.to) & weightRelative>0.01 & weight>10)
...
types_all=c(PreOrder$type.from, PostOrder$type.to)
