...
# Compute relative weight from left and right PB-FB-XX populations on single downstream neurons
...
for (ppp in 1:length(Pre_Post$type.from)){
  ...
  data.frame(type.from=Pre_Post$type.from[ppp],type.to=Pre_Post$type.to[ppp],ColFromNum=FBColfromNum,
             PreProportion=PreNum/PreNum_All,PostProportion=PostProportion,Lateralization=Lateralization, AverageWeight=AverageWeight))
...
# connectivity matrices are from inconsistent and have weak type-to-type connections.
```{r message=FALSE, warning=FALSE}
