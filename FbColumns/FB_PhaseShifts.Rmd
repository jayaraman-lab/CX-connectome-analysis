...
# Load pre-processed synapse location data (including FB column locations) and remove variables that won't be used
```{r}
...
# Plot PB-FB-XX neuron columnar locations along with their glomerulus to column mappings
```{r message=FALSE, warning=FALSE}
...
for (ttt in 1:length(PbFb_Types)){
  ...
  ggsave(paste(PlotDir, "ColumnLocs_", PbFb_Types[ttt], "_", as.character(CurrentLayer),"_",EdgeType, ".png", sep=""),
...
# Plot a few connectivity matrices to show that PB-FB phase shifts are realized in connectivity
```{r}
