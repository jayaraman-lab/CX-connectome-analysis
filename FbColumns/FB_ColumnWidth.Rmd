...
# Get PB-FB-XX (ie PFX) synapse distributions
```{r message=FALSE, warning=FALSE}

# Make PB-FB-XX  plotting directory
PFX_Dir=paste(PlotDir,"PFX_Plots_All","/",sep="")
if (!dir.exists(PFX_Dir)){dir.create(PFX_Dir)}


# Get PB-FB-XX neuron synapse distributions 
# Example synapse clouds saved to PFX_Dir
PFX_SynsAll=subset(FBC_SynsAll, !(is.na(PBglom)) & startsWith(type,"PF"))
PFX_Distribution=Get_SynLayerDistribution(PFX_SynsAll, 25, PFX_Dir) # 25 is good here, as some types have low synapse counts
...
# Plot histogram of arbor widths and inter-column spacing
```{r message=FALSE, warning=FALSE}
...
ggsave(paste(PlotDir, "PbFbXx_SynapseWidth", ".pdf",sep=""), plot = P_Hist, device='pdf',
...
ggsave(paste(PlotDir, "PFX_InterColumnDistance", ".pdf",sep=""), plot = P_Hist, device='pdf',
       scale = 1, width = 4, height = 2, units ="in", dpi = 500, limitsize = TRUE,  bg = "transparent")
