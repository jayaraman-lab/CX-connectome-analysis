...
```{r}
PFL3ToLC10Plot <- ggraph(PFL3ToLC10,layout="manual",x=x,y=y) +
  geom_edge_fan(aes(color=customSupertype.from,width=weightRelative),
  ...
  scale_edge_color_manual(values=c(customGraphPalette,customSupertypePalette)) +
  scale_edge_width(range=c(0.2,3),limits=c(0.001,NA),name="relative weight") +
  guides(color="none",edge_color="none") + geom_vline(xintercept = 1,lty=2,color="grey50") +
  theme(legend.position = c(0.8,0.2)) + coord_cartesian(clip="off")
...
```
```{r}
PFL1ToLC27Plot <- ggraph(PFL1ToLC27,layout="stress") +
  geom_edge_fan(aes(color=customSupertype.from,width=weightRelative),
                end_cap = circle(3, "mm"),linejoin = "mitre",linemitre=3,
                arrow =arrow(length = unit(1, 'mm'),type = "closed")) +
  geom_node_point(aes(color=customSupertype),size=4)+
...
```{r}
ExR8toCHPlot <- ggraph(ExR8toCH,layout="stress") +
  geom_edge_fan(aes(color=customSupertype.from,width=weightRelative),
                end_cap = circle(3, "mm"),linejoin = "mitre",linemitre=3,
                arrow =arrow(length = unit(1, 'mm'),type = "closed")) +
  geom_node_point(aes(color=customSupertype),size=4)+
  ...
  scale_edge_color_manual(values=c(customGraphPalette,customSupertypePalette)) +
  scale_edge_width(range=c(0.2,3),limits=c(0.001,NA),name="relative weight") +
  guides(color="none",edge_color="none")+scale_y_continuous() + 
  theme(legend.position = c(0.8,0.2))+coord_cartesian(clip="off")
...
```
```{r}
PFRbToLTMCPlot <- ggraph(PFRbToLTMC,layout="stress") +
  geom_edge_fan(aes(color=customSupertype.from,width=weightRelative),
                end_cap = circle(3, "mm"),linejoin = "mitre",linemitre=3,
                arrow =arrow(length = unit(1, 'mm'),type = "closed")) +
  geom_node_point(aes(color=customSupertype),size=4)+
  geom_node_text(aes(label=type))+theme_paper_map()+
  scale_color_manual(values=c(customGraphPalette,customSupertypePalette))+ scale_edge_color_manual(values=c(customGraphPalette,customSupertypePalette)) + scale_edge_width(range=c(0.2,3),limits=c(0.001,NA),name="relative weight") + guides(color="none",edge_color="none")+scale_y_continuous(trans="reverse")#+coord_flip()+scale_x_continuous(trans="reverse")
