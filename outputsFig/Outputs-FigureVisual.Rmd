...
outputsFolder <- "~/Dropbox (HHMI)/FIBSEM CX Paper Jan 2020/Figures/NewOutputs/figure9-VisualPNs/"
```
...
PFL32LC33Mat <- plotConnectivity(filter(PFL3Bag$outputs_raw,type.to=="LC33_R"),grouping="neuron",orderIn = orderPFL3BI,xaxis="outputs",replacementLabels = "name",legendName="Relative weight",theme=theme_paper_grid()) + xlab("post synaptic neuron") + ylab("pre synaptic neuron")
```
...
PFLLCOutMat <- plotConnectivity(filter(rbind(PFL3Bag$outputs_raw,filter(LC33Bag$outputs_raw,type.from=="LC33_b_R")),supertype2.to != "Unassigned"),facetInputs="type.from",facetOutputs="supertype2.to",grouping="neuron",replacementLabels = "name",legendName="Relative weight",theme=theme_paper_grid()) + xlab("pre synaptic neuron") + ylab("post synaptic neuron")+ theme(strip.text.y = element_text(angle=0),strip.text.x=element_text(angle=45),axis.text.x = element_blank(),axis.text.y=element_blank())
PFLLCOutMat
```
```{r}
layout <-  c(
