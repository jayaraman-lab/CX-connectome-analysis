...
CXPerROIRedux <- filter(CXPerROI,roi %in% outputRegions) %>% group_by(type,databaseType,n,supertype1,supertype2,supertype3) %>%
   ...
   ungroup() %>% distinct()
CXPotentialOutputs <- filter(CXPerROIRedux,downstream>20 & downstreamRatio>0.5)
...
```{r}
cxClOnOut <- connectivityCluster(outputsTable = outputTable,ROIs="Output_Regions",knownStats = TRUE,grouping="type")
n1OnCxIn <- connectivityCluster(inputsTable = outputTable,ROIs="Output_Regions",knownStats = TRUE,grouping="type")
cxClCl <- cutree(cxClOnOut$hc,h=0.8)
```

```{r}
outputTable <- outputTable %>% mutate(type.fromF = factor(type.from,levels=unique(type.from,levels=unique(type.from)[cxClCl$hc$order])))
outputKind <- ggplot(outputTable,aes(x=type.fromF,y=knownOutputContribution)) + geom_col(aes(fill=category)) + theme_paper_hgrid() + theme(axis.text.x = element_text(angle = 90,hjust = 1,vjust=0.5)) + scale_fill_paletteer_d("ggthemes::Classic_Gray_5",name="Output kind") +xlab("") + ylab("Fraction of significant output weights")
outputKind
```
```{r}
outputTable <- mutate(outputTable,cluster.from=cxClCl[type.from])
...
ggplot(outputTableClusterInfluences,aes(x=mainContribFraction)) + geom_histogram() + facet_wrap(category ~.)
...
plotClusters(n1OnCxIn,colorAxis = T,theme = theme_paper_map(),h=0.8)

clCol <- paletteer::paletteer_d("Polychrome::palette36")[cxClCl[t]+2]
plotConnectivity(filter(outputTable,category=="output"),xaxis="outputs",theme=theme_paper(panel.background=element_rect(fill="gray95"),strip.text.x=element_text(colour=clCol)),orderIn=cxClOnOut$hc$order,orderOut=n1OnCxIn$hc$order,value="knownWeightRelative",facetInputs = "cluster.from",facetOutputs = "clusterEq.to") + scale_x_discrete(labels=NULL)
plotConnectivity(filter(outputTable,category=="CX loop" | category=="self" | category=="Efference"),xaxis="outputs",theme=theme_paper(panel.background=element_rect(fill="gray95")),orderIn=cxClOnOut$hc$order,orderOut=n1OnCxIn$hc$order,value="knownWeightRelative",facetInputs = "cluster.from",facetOutputs = "clusterEq.to")
```
...
CXN1OutSyn <- dplyr::bind_rows(pbapply::pblapply(outputRegionsStrict,function(r){
   synSet <- neuprint_get_synapses($bodyid,roi=r,chunk=5,progress=FALSE) 
   if (!is.null(synSet)){
      synSet <- filter(mutate(synSet,roi=r),prepost==0)
      }                
   return(synSet)                
...
for (t in unique(CXOutputNeurons$type)){
   plot3d(filter(CXOutSyn,type==t)[,c("x","y","z")],col=paletteer::paletteer_d("Polychrome::palette36")[cxClCl[t]+2],add=T)
}
nview3d("ventral")
...
plot3d(SPSMesh,color="white",alpha=0.2,xlab="",ylab="",zlab="",box=FALSE,axes=FALSE,add = T)
par3d(scale=c(1,1,1))
for (t in unique(filter(outputTable,category="output")$type.to)){
   plot3d(filter(CXOutSyn,type==t)[,c("x","y","z")],col=paletteer::paletteer_d("Polychrome::palette36")[cxClCl[t]+2],add=T)
}
nview3d("ventral")
rgl.snapshot(paste0(outputsFolder,"frontAllOuts.png"))
