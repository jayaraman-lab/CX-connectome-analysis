...
```{r}
outputBag <- neuronBag(CXOutputNeurons,selfRef = TRUE,slctROI=outputRegions,verbose=TRUE,computeKnownRatio=TRUE,omitInputs = TRUE)
...
```{r}
cxClOnOut <- connectivityCluster(outputsTable = outputsN1Table,ROIs="Output_Regions",knownStats = TRUE,grouping="type")
n1OnCxIn <- connectivityCluster(inputsTable = outputsN1Table,ROIs="Output_Regions",knownStats = TRUE,grouping="type")
```

```{r}
plotClusters(cxClOnOut,colorAxis = T,theme = theme_paper_map(),h = 0.8)
plotClusters(n1OnCxIn,colorAxis = F,theme = theme_paper_map(),h = 0.8)
plotConnectivity(cxClOnOut,xaxis="inputs",theme=theme_paper_map(),orderOut=n1OnCxIn$hc$order,value="knownWeightRelative")+coord_cartesian()
plotConnectivity(n1OnCxIn,xaxis="outputs",orderIn=cxClOnOut$hc$order,theme=theme_paper_map())+coord_cartesian()
```
