...
```{r}
compAllTo1 <- (CX2CXFullPlot + ggtitle("All steps")) / (CX2CXFullPlot1 + ggtitle("1 step")) + plot_annotation(title="Outputs figure 3 SI2: comparison of pathway connections to 1 step connections",theme=theme(title=element_text(size=12,face="bold")))
```
```{r}
save_plot(paste0(outputsFolder,"outputs-Figure3-SI2.pdf"),compAllTo1,ncol=1,nrow=4,base_width = 8.5,base_height = 11/4,device=cairo_pdf)
```
...
```{r}
save_plot(paste0(outputsFolder,"outputs-Figure3-SI3.pdf"),figure3SI2,ncol=1,nrow=4,base_width = 8.5,base_height = 11/4,device=cairo_pdf)
```
