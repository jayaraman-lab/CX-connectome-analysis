...
allClusters <- (clusterFull + cluster1)/(cluster2 + cluster3)/(cluster5 + cluster8)+ plot_layout(guides="collect")+ plot_annotation(title="Outputs figure 5 SI1: Cosine distances for different connection dephts",theme=theme(title=element_text(size=12,face="bold")))& theme(axis.text=element_text(size=4))
...
```
```{r}
save_plot(paste0(outputsFolder,"outputs-Figure5-SI1.pdf"),allClusters,ncol=1,nrow=4,base_width = 8.5,base_height = 11/4,device=cairo_pdf)
