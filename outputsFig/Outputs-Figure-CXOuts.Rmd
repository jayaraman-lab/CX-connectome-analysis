...
```{r}
customSupertypePalette <- c(supertype2Palette()$pal,"EB Columnar"=supertype2Palette()$pal["EPG"])
```
...
firstStageSummary$supertype.from <- CXOutputNeurons$supertype[match(firstStageSummary$type.from,CXOutputNeurons$type)]
```
...
countsSummary <- ggplot(firstStageSummary,aes(x=type.from,y=totalWeightRaw,color=supertype.from)) + geom_point() + 
   ...
   facet_grid(.~supertype3.from,scales = "free",space="free") + ylab("Significant # downstream synapses") + xlab("CX type") + theme(strip.text.x=element_blank(),axis.text.x = element_text(angle=90,hjust = 1,vjust=0.5)) + scale_y_log10()
...
```{r}
outputFig1 <- (outputsInnervation_R + theme(plot.margin=margin(b=30,t=30),strip.text.x=element_text(angle=90)))/ 
   allOutViews / 
   countsSummary + plot_layout(heights=c(1,3,0.5)) + plot_annotation(tag_levels = "A",title = "Figure 1 - Outputs: CX neurons downstream synapses outside the CX")
