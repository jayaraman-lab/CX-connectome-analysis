...
```{r}
save(CXOutputNeurons,CXOutsideTypes,CXOutputTypes,outsideRegions,CXOutsideNeurons,typesCXOrder,CXNeurons,file=file.path(dataFolder,"outputsBasics.RData"))
```
...
CXoutAllSteps <- endpointConnections_raw(CXout_AdjWR,endpoint=CXOutputTypes,polarity="from_sources",maxIt=8,eps=10^-8)
...
CXoutFull <- endpointConnections(CXout_AdjWR,endpoint=CXOutputTypes,polarity="from_sources",maxIt=100,eps=10^-8)
...
CXInfluence <- endpointConnections(CXout_AdjOCWR,endpoint=CXNeuronsReached,polarity = "to_targets",maxIt=100,eps=10^-8)
...
CX_outGraphRed <- as_tbl_graph(induced_subgraph(CX_outGraph,c(mainFFConns$type.to,mainFFConns$type.from))) %N>% mutate(mainContributor=mainFFConns$mainContributor[match(type,mainFFConns$type.to)],
...
save(mainTargets,mainCXTargets,mainFFTargets,mainFFConns,allMainConns,mainFFTargetsNeurons,file=file.path(dataFolder,"mainTargetsAndConns.Rdata"))
```
