...
Try the above with the getTypesInROITable function
```{r}
AB_R_Type_Info <- getTypesInRoiTable("AB(R)",minTypePercentage = 0.2)
AB_L_Type_Info <- getTypesInRoiTable("AB(L)",minTypePercentage = 0.2)
```
```{r}
T2T_AB_R <- AB_R_Type_Info$outputs %>% as.data.frame() %>% filter(type.to %in% c(AB_R_Type_Info$names$type %>% unique()))
...
T2T_AB_L <- AB_L_Type_Info$outputs %>% as.data.frame() %>% filter(type.to %in% c(AB_L_Type_Info$names$type %>% unique()))
...
```
```{r}
#ggsave("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\FirstDraftFigs\\FB\\FIGURE_AB\\TypeToType-AB-neuronbag.pdf",TCTPlt_AB)
```
