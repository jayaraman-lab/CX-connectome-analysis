...
```{r}
FBt_to_FBt <- conTab_FBTan %>% filter(grepl("FB",type.to) & grepl("FB",type.from))
typesTo <- FBt_to_FBt$type.to %>% unique() %>% sort()
types2types <- c()
for (t in 1:length(typesTo)){
  FBt_meta <- neuprint_get_meta(typesTo[t])
  if (nrow(FBt_to_FBt %>% 
...
ggsave(paste0("C:\\Users\\turnerevansd\\Dropbox (HHMI)\\FIBSEM CX Paper Jan 2020\\Figures\\FirstDraftFigs\\FB\\FIGURE_FB_TanConnectivity\\FBtToFBt-neuron-to-neuron.pdf"),FBt_Plt)
```
